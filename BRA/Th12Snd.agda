...
-- BRA.Th12Snd -- schematic Theorem 12 for Fst.
--
-- Df_F1_Snd : Fun1 dispatches on input shape via IfLf:
--   Df_F1_Snd O           = reify encAxSndLf      (lf-case proof code)
--   Df_F1_Snd (Pair x y)  = runtime ruleInst2 proof code with cor x, cor y
--                           at the substituent slots.
--
-- Th12_F1_Snd :
--   Deriv (atomic (eqn (ap1 thmT (ap1 Df_F1_Snd (var zero)))
--                       (codeFTeq1Asym Snd (var zero))))
--
-- Construction = fromBaseAndPair on
--    Th12_F1_Snd_at_lf_v2  : at substF zero O P  (lf case)
--    Th12_F1_Snd_at_pair   : at substF zero (Pair v1 v2) P  (Pair case)
-- where  P  is the Theorem 12 statement at  var zero .

module BRA.Th12Snd where
...
open import BRA.Thm.Tag      using (tagRuleInst2 ; tagAxSnd)
...
K4 : Term
K4 = reify (encAxSnd (var zero) (var (suc zero)))

lfProofCode : Term
lfProofCode = reify encAxSndLf
...
------------------------------------------------------------------------
-- Df_F1_Snd : Fun1 dispatching on input shape.
--
--   Df_F1_Snd t = ap2 IfLf t (ap2 Pair lfProofCode (ndProofCodeFun1 t))
--
...
-- applied here.  Done in Th12_F1_Snd_at_pair via cong1 cor + axFst/axSnd.
...
ndProofCodeFun1_unfold t =
  let
    ...
    eK4 = ktRed (encAxSnd (var zero) (var (suc zero))) t
...
------------------------------------------------------------------------
-- Df_F1_Snd_unfold_pair :
--   Df_F1_Snd (Pair v1 v2) = runtimeTreeAt v1 v2
--
...
Df_F1_Snd_unfold_pair : (v1 v2 : Term) ->
  Deriv (atomic (eqn (ap1 Df_F1_Snd (ap2 Pair v1 v2)) (runtimeTreeAt v1 v2)))
Df_F1_Snd_unfold_pair v1 v2 =
  let
    t : Term
    t = ap2 Pair v1 v2
    ...
    s1 : Deriv (atomic (eqn (ap1 Df_F1_Snd t)
    ...
    -- Combine: Df_F1_Snd t -> IfLf t (Pair lfProofCode nd) -> nd -> runtime.
    sCombined : Deriv (atomic (eqn (ap1 Df_F1_Snd t)
                                     (ap2 IfLf t
                                       (ap2 Pair lfProofCode
                                         (ap1 ndProofCodeFun1 t)))))
...
------------------------------------------------------------------------
-- Th12_F1_Snd_at_lf_v2 :
--   Deriv (atomic (eqn (ap1 thmT (ap1 Df_F1_Snd O))
--                       (codeFTeq1Asym Snd O)))
--
-- The lf case of schematic Theorem 12 for Fst, using the dispatching
-- Df_F1_Snd.  Df_F1_Snd O = lfProofCode = reify encAxSndLf.  Then
-- thmTDispAxSndLf gives reify outAxSndLf.  Bridge to codeFTeq1Asym Snd O
...
Th12_F1_Snd_at_lf_v2 =
  let
    s1 : Deriv (atomic (eqn (ap1 Df_F1_Snd O) lfProofCode))
    s1 = Df_F1_Snd_unfold_lf

    s2 : Deriv (atomic (eqn (ap1 thmT (ap1 Df_F1_Snd O))
                             (ap1 thmT lfProofCode)))
    s2 = cong1 thmT s1

    s3 : Deriv (atomic (eqn (ap1 thmT lfProofCode) (reify outAxSndLf)))
    s3 = thmTDispAxSndLf

    -- Bridge from reify outAxSndLf = mkEqT (mkAp1T (codeF1 Snd) O) O
...
------------------------------------------------------------------------
-- xShape_K4 : the Fst-shape proof for K4 = reify (encAxSnd (var 0) (var 1)).
--
-- K4 = ap2 Pair (reify (natCode tagAxSnd))
--               (reify (nd (code (var 0)) (code (var 1))))
--    = ap2 Pair (ap2 Pair O (ap2 Pair O (ap2 Pair O O)))
--               (ap2 Pair K2 K3)
-- since natCode tagAxSnd = natCode 3 = nd lf (nd lf (nd lf lf)) reified.

xShape_K4 : Sigma Term (\ y' -> Sigma Term (\ x' ->
  Deriv (atomic (eqn (ap1 Fst K4) (ap2 Pair x' y')))))
xShape_K4 =
  mkSigma (ap2 Pair O (ap2 Pair O O)) (mkSigma O
    (axFst (ap2 Pair O (ap2 Pair O (ap2 Pair O O))) (ap2 Pair K2 K3)))

------------------------------------------------------------------------
-- Th12_F1_Snd_at_pair : the Pair case of schematic Theorem 12 for Fst.
--
--   Deriv (atomic (eqn (ap1 thmT (ap1 Df_F1_Snd (Pair v1 v2)))
--                       (codeFTeq1Asym Snd (Pair v1 v2))))
--
-- Pipeline:
--   (1) Df_F1_Snd_unfold_pair : Df_F1_Snd (Pair v1 v2) = runtimeTreeAt v1 v2.
--   (2) cong1 thmT (1) : thmT(Df_F1_Snd (Pair v1 v2)) = thmT(runtimeTreeAt).
--   (3) thmTDispRuleInst2_param : thmT(runtimeTreeAt) = subT2-form (with
--       cor(Fst..) and cor(Snd..) at substituent slots, K4 unevaluated).
--   (4) thmTDispAxSnd (var 0) (var 1) : thmT K4 = reify (outAxSnd (var 0) (var 1)).
--   (5) subTDef_term2 : subT2 args codeP = codedSubstT2-form.
--   (6) Agda definitional unfolding of codedSubstT2 on closed outAxSnd tree:
--       reduces to reducedT2 (closed expression in cor (Fst..) and cor (Snd..)).
--   (7) Bridge to codeFTeq1Asym Snd (Pair v1 v2) via corOfFstPair, corOfSndPair,
--       and corOfPair (ruleSym).

Th12_F1_Snd_at_pair : (v1 v2 : Term) ->
  Deriv (atomic (eqn (ap1 thmT (ap1 Df_F1_Snd (ap2 Pair v1 v2)))
                     (codeFTeq1Asym Snd (ap2 Pair v1 v2))))
Th12_F1_Snd_at_pair v1 v2 =
  let
    pairIn : Term
    ...
    -- (1) Unfold Df_F1_Snd.
    s1 : Deriv (atomic (eqn (ap1 Df_F1_Snd pairIn) runtimeTree))
    s1 = Df_F1_Snd_unfold_pair v1 v2

    -- (2) Push thmT.
    s2 : Deriv (atomic (eqn (ap1 thmT (ap1 Df_F1_Snd pairIn))
    ...
    s5 : Deriv (atomic (eqn (ap2 subT2 argsPair (ap1 thmT K4))
                             (ap2 subT2 argsPair
                               (reify (outAxSnd (var zero) (var (suc zero)))))))
    ...
    s6 : Deriv (atomic (eqn (ap2 subT2 argsPair
                             ...
                             (codedSubstT2 tA tB
                               (code (var zero))
                               (code (var (suc zero)))
                               (outAxSnd (var zero) (var (suc zero))))))
    ...
    --
    -- outAxSnd (var 0) (var 1) = codeFormula (atomic (eqn (Fst (Pair (var 0) (var 1))) (var 0)))
    ...
    --   reducedT2 = mkEqT (mkAp1T (codeF1 Snd) (mkAp2T pairT tA tB)) tA
    --   target_LHS = mkEqT (mkAp1T (codeF1 Snd) (cor (Pair v1 v2))) tA
    --   target_RHS = mkEqT (mkAp1T (codeF1 Snd) (cor (Pair v1 v2))) (cor (Fst (Pair v1 v2)))
    --              [no bridge needed for tA → cor(Fst(Pair..)) since tA = cor(Fst(Pair..))]

    bridgeLHS : Deriv (atomic (eqn
      (mkAp1T (reify (codeF1 Snd)) (mkAp2T pairT tA tB))
      (mkAp1T (reify (codeF1 Snd)) (ap1 cor (ap2 Pair v1 v2)))))
    bridgeLHS =
      congR Pair (reify tagAp1)
        (congR Pair (reify (codeF1 Snd)) bridgeMkAp2TFull)

    bridgeFinal : Deriv (atomic (eqn reducedT2 (codeFTeq1Asym Snd (ap2 Pair v1 v2))))
    bridgeFinal = congL Pair tB bridgeLHS

  in ruleTrans s2 (ruleTrans s3 (ruleTrans s5 (ruleTrans s6
       (ruleTrans s7 bridgeFinal))))

------------------------------------------------------------------------
-- Th12_F1_Snd_at : pointwise schematic Theorem 12 for Fst at any input.

Th12_F1_Snd_at_O :
  Deriv (atomic (eqn (ap1 thmT (ap1 Df_F1_Snd O))
                     (codeFTeq1Asym Snd O)))
Th12_F1_Snd_at_O = Th12_F1_Snd_at_lf_v2

Th12_F1_Snd_at_Pair : (v1 v2 : Term) ->
  Deriv (atomic (eqn (ap1 thmT (ap1 Df_F1_Snd (ap2 Pair v1 v2)))
                     (codeFTeq1Asym Snd (ap2 Pair v1 v2))))
Th12_F1_Snd_at_Pair = Th12_F1_Snd_at_pair
...
P_Th12_Snd : Formula
P_Th12_Snd = atomic (eqn (ap1 thmT (ap1 Df_F1_Snd (var zero)))
                          (codeFTeq1Asym Snd (var zero)))

-- Coerce Th12_F1_Snd_at_O to  Deriv (substF zero O P_Th12_Snd)  via thClosed.

baseLf_substF :
  Deriv (substF zero O P_Th12_Snd)
baseLf_substF =
  let
    thEqO : Eq (substF1 zero O thmT) thmT
    thEqO = thClosed zero O

    coerce : Eq (atomic (eqn (ap1 thmT (ap1 Df_F1_Snd O))
                              (codeFTeq1Asym Snd O)))
                (substF zero O P_Th12_Snd)
    coerce = eqCong (\ f -> atomic
                              (eqn (ap1 f (ap1 Df_F1_Snd O))
                                   (codeFTeq1Asym Snd O)))
                    (eqSym thEqO)
  in eqSubst (\ F -> Deriv F) coerce Th12_F1_Snd_at_lf_v2

-- Same for the Pair case.

basePair_substF : (v1Idx v2Idx : Nat) ->
  Deriv (substF zero (ap2 Pair (var v1Idx) (var v2Idx)) P_Th12_Snd)
basePair_substF v1Idx v2Idx =
  let
    ...
    coerce : Eq (atomic (eqn (ap1 thmT (ap1 Df_F1_Snd pairT))
                              (codeFTeq1Asym Snd pairT)))
                (substF zero pairT P_Th12_Snd)
    coerce = eqCong (\ f -> atomic
                              (eqn (ap1 f (ap1 Df_F1_Snd pairT))
                                   (codeFTeq1Asym Snd pairT)))
                    (eqSym thEqPair)
  in eqSubst (\ F -> Deriv F) coerce
              (Th12_F1_Snd_at_pair (var v1Idx) (var v2Idx))

-- Schematic Theorem 12 for Fst:
--   Deriv (P_Th12_Snd)
-- with var 0 free.  Combines the two cases via fromBaseAndPair.

Th12_F1_Snd : Deriv P_Th12_Snd
Th12_F1_Snd =
  fromBaseAndPair P_Th12_Snd (suc (suc zero)) (suc (suc (suc zero)))
    baseLf_substF
