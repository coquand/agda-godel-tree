...
-- BRA.SubstClosure -- structural substitution-closure lemmas.
--
...
subst_reify : (k : Nat) (r : Term) (t : Tree) ->
  Eq (subst k r (reify t)) (reify t)
subst_reify k r lf       = refl
subst_reify k r (nd a b) =
  -- subst k r (reify (nd a b))
  --   = subst k r (ap2 Pair (reify a) (reify b))
  --   = ap2 Pair (subst k r (reify a)) (subst k r (reify b))
  -- by IH twice  =  ap2 Pair (reify a) (reify b)  =  reify (nd a b) .
  eqTrans (eqCong (\ A -> ap2 Pair A (subst k r (reify b)))
                   (subst_reify k r a))
          (eqCong (\ B -> ap2 Pair (reify a) B)
                   (subst_reify k r b))
...
substF1_KT_of_reify : (k : Nat) (r : Term) (t : Tree) ->
  Eq (substF1 k r (KT (reify t))) (KT (reify t))
substF1_KT_of_reify k r lf       = refl
substF1_KT_of_reify k r (nd a b) =
  -- KT (reify (nd a b)) = KT (ap2 Pair (reify a) (reify b))
  --                     = Comp2 Pair (KT (reify a)) (KT (reify b))
  -- substF1 k r (Comp2 Pair (KT (reify a)) (KT (reify b)))
  --   = Comp2 (substF2 k r Pair) (substF1 k r (KT (reify a))) (substF1 k r (KT (reify b)))
  --   = Comp2 Pair (substF1 k r (KT (reify a))) (substF1 k r (KT (reify b)))
  -- by IH twice  =  Comp2 Pair (KT (reify a)) (KT (reify b)) .
  eqTrans (eqCong (\ A -> Comp2 Pair A (substF1 k r (KT (reify b))))
                   (substF1_KT_of_reify k r a))
          (eqCong (\ B -> Comp2 Pair (KT (reify a)) B)
                   (substF1_KT_of_reify k r b))
