...
-- BRA.Sb -- our tree-based analog of Guard's substitution functor
...
--
-- sb is built from the existing  subT  combinator (BRA.SubT) plus
...
sbDef : (codeA : Tree) (n : Nat) (codeP : Tree) ->
  Deriv (atomic (eqn (ap2 sb (ap2 Pair (reify codeA) (reify (natCode n)))
                              (reify codeP))
                      (reify (codedSubst codeA (code (var n)) codeP))))
sbDef codeA n codeP =
  let codeAT : Term
      ...
      -- Step 2: reduce repackageSb to (Pair (Pair tagVarT nT) codeAT)
      --         which is definitionally (Pair (reify (code (var n))) codeAT).
      lhsFinal : Term
      ...
      -- Step 5: subTDef closes the goal.
      --   lhsFinal = ap2 Pair (ap2 Pair tagVarT nT) codeAT
      --           = ap2 Pair (reify (code (var n))) codeAT  (definitional)
      s3 : Deriv (atomic (eqn (ap2 subT lhsFinal codePT)
                              (reify (codedSubst codeA (code (var n)) codeP))))
      s3 = subTDef n codeA codeP
