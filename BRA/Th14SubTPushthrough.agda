...
module BRA.Th14SubTPushthrough where
...
treeEqRefl : (t : Tree) -> Eq (treeEq t t) true
treeEqRefl lf       = refl
treeEqRefl (nd a b) =
  eqTrans (eqCong (\ y -> boolAnd y (treeEq b b)) (treeEqRefl a))
          (treeEqRefl b)
  ...
  let
      ...
      treeEqRedC : Deriv (atomic (eqn
        ...
        (boolCase (treeEq (nd tagVar (natCode n)) (nd tagVar (natCode n)))
                  O falseT)))
      treeEqRedC = treeEqRed (nd tagVar (natCode n)) (nd tagVar (natCode n))
...
subT_node_no_match : (n : Nat) (tT : Term) (a b : Tree) ->
...
subT_node_no_match n tT a b treeEqIsFalse =
  let
      varCode : Tree
      varCode = code (var n)
      ...
      treeEqRedC = treeEqRed varCode (nd a b)
...
natEqIsFalse_of_diff n k diff =
  let
      ...
      treeEqTagVarRefl = treeEqRefl tagVar
...
subT_preserves_tagImp n tT =
  let
      ...
      step1 = subT_node_no_match n tT (nd lf lf) (nd lf lf) refl

      -- subT (reify (nd lf lf)) = ?
      -- treeEq (code (var n)) (nd lf lf):
      --   code (var n) = nd tagVar (natCode n) = nd (nd (nd (nd lf lf) lf) lf) (natCode n)
      --   compared with nd lf lf — left children: nd (nd (nd lf lf) lf) lf vs lf — false.
      step2_inner : Deriv (atomic (eqn
        (ap2 subT p (reify (nd lf lf)))
        (ap2 Pair (ap2 subT p (reify lf)) (ap2 subT p (reify lf)))))
      step2_inner = subT_node_no_match n tT lf lf refl
...
subT_codeFormula_imp n tT P Q =
  let
      ...
      step1 = subT_node_no_match n tT tagImp (nd cP cQ)
      ...
      step2_innerSplit : Deriv (atomic (eqn
        ...
        (ap2 Pair (ap2 subT p (reify cP))
                  (ap2 subT p (reify cQ)))))
      step2_innerSplit = subT_node_no_match n tT cP cQ
...
subT_dist_Pair_tagImp n tT X Y =
  let
      ...
      treeEqRedTV = treeEqRed tagVar tagImp
...
subT_preserves_tagNeg n tT =
  let
      ...
      step1 : Deriv (atomic (eqn
        ...
        (ap2 Pair (ap2 subT p (reify (nd lf lf)))
                  (ap2 subT p (reify lf)))))
      step1 = subT_node_no_match n tT (nd lf lf) lf refl

      -- subT p (reify (nd lf lf)) by inner no-match.
      step2_inner : Deriv (atomic (eqn
        (ap2 subT p (reify (nd lf lf)))
        (ap2 Pair (ap2 subT p (reify lf)) (ap2 subT p (reify lf)))))
      step2_inner = subT_node_no_match n tT lf lf refl
  ...
  let
      ...
      step1 : Deriv (atomic (eqn
        ...
        (ap2 Pair (ap2 subT p (reify tagNeg))
                  (ap2 subT p (reify cP)))))
      step1 = subT_node_no_match n tT tagNeg cP
...
subT_codeFormula_atomic n tT a b =
  subT_node_no_match n tT (code a) (code b)
