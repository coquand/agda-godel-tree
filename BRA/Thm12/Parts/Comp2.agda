...
module BRA.Thm12.Parts.Comp2 where
...
open import BRA.Thm12.Param.AxComp2
...
module Comp2Case
  ...
  private
    ...
    y1_part : Fun1
    y1_part =
      Comp2 Pair (KT (BRA.Thm.ThmT.tagCode_axComp2))
        (Comp2 Pair (KT cF2h)
          (Comp2 Pair (KT cF1f)
    ...
    let -- ====================================================================
        ...
        red_y1 =
          let -- y1_part = Comp2 Pair (KT tagCode_axComp2) (...) etc.
              ...
              s1 = axComp2 Pair (KT (BRA.Thm.ThmT.tagCode_axComp2))
                                (Comp2 Pair (KT cF2h)
                                  (Comp2 Pair (KT cF1f)
                                    (Comp2 Pair (KT cF1g) cor))) x
              s2 = axKT (natCode (BRA.Thm.Tag.tagAxComp2)) x
          ...
          in ruleTrans s1
                (ruleTrans (congL Pair _ s2)
                           (congR Pair (BRA.Thm.ThmT.tagCode_axComp2) ri1))
        ...
        red_enc_g_num =
          let s1 = axComp2 Pair (KT (reify tagAp1)) (Comp2 Pair (KT cF1g) cor) x
              s2 = axKT lf x  -- tagAp1 = nd lf (nd lf lf), reify = Pair O (Pair O O); but axKT takes v, so axKT (reify^-1 ...) — hmm
        ...
        y1_shape : Deriv (atomic (eqn (ap1 Fst (parEncAxComp2 h f' g (ap1 cor x)))
                                        (ap2 Pair O _)))
        y1_shape = axFst (BRA.Thm.ThmT.tagCode_axComp2) _
