...
module BRA.Thm12.Parts.Post where
...
open import BRA.Cor
open import BRA.Thm.Tag using (tagAxPost ; tagCong1 ; tagRuleTrans)
open import BRA.Thm.ThmT
  using (thmT ; tagCode_axPost ; tagCode_ruleTrans ; tagCode_cong1)
open import BRA.Thm12.Param.AxPost
  using (parDispAxPost ; parEncAxPost ; parOutAxPost)
open import BRA.Thm12.Param.Cong1
...
module PostCase
  ...
  axPostPart_reduce a b =
    let codeF1f_T = reify (codeF1 f)
        ...
        s2b = axKT (natCode tagAxPost) a
        s2 = ruleTrans s2a s2b

        sInner = axFan (Lift (KT codeF1f_T)) innerH Pair a b
        sInnerL = ruleTrans (axLift (KT codeF1f_T) a b) (axKT (codeF1 f) a)

        sH = axFan (Lift (KT codeF2h_T)) innerCor Pair a b
        sHL = ruleTrans (axLift (KT codeF2h_T) a b) (axKT (codeF2 h) a)
