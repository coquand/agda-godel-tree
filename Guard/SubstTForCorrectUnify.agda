...
module Guard.SubstTForCorrectUnify where
...
stepNotVar repl tgt reifyA reifyB neqPf =
  let cstf = closedSubstTFor repl tgt ; orig = ap2 Pair reifyA reifyB
      ...
      isVarEval : Deriv (eqF (ap2 isVarF orig recT) (ap2 TreeEq reifyA (reify tagVar)))
...
closedSTFNd : (replT tgtT a b : Tree) ->
  ...
  Deriv (eqF (ap1 (closedSubstTFor (reify replT) (reify tgtT)) (ap2 Pair (reify a) (reify b))) (reify (codedSubst replT tgtT (nd a b))))
closedSTFNd replT tgtT a b neqD neqB ihA ihB =
  ...
  in ruleTrans (stepNotVar (reify replT) (reify tgtT) (reify a) (reify b) neqD)
     ...
     (eqSubst (\v -> Deriv (eqF (ap2 Pair (reify (codedSubst replT tgtT a)) (reify (codedSubst replT tgtT b))) (reify v)))
              (eqSym (codedSubstNd replT tgtT a b neqB)) (axRefl _))))

------------------------------------------------------------------------
-- Convenience wrappers

closedSTFNdF1 : (replT tgtT : Tree) (f : Fun1) (b : Tree) ->
  Deriv (eqF (ap1 (closedSubstTFor (reify replT) (reify tgtT)) (reify (codeF1 f))) (reify (codedSubst replT tgtT (codeF1 f)))) ->
  Deriv (eqF (ap1 (closedSubstTFor (reify replT) (reify tgtT)) (reify b)) (reify (codedSubst replT tgtT b))) ->
  Deriv (eqF (ap1 (closedSubstTFor (reify replT) (reify tgtT)) (ap2 Pair (reify (codeF1 f)) (reify b))) (reify (codedSubst replT tgtT (nd (codeF1 f) b))))
closedSTFNdF1 replT tgtT f b = closedSTFNd replT tgtT (codeF1 f) b (codeF1NeqTagVar f) (codeF1NotVar f)

closedSTFNdF2 : (replT tgtT : Tree) (g : Fun2) (b : Tree) ->
  Deriv (eqF (ap1 (closedSubstTFor (reify replT) (reify tgtT)) (reify (codeF2 g))) (reify (codedSubst replT tgtT (codeF2 g)))) ->
  Deriv (eqF (ap1 (closedSubstTFor (reify replT) (reify tgtT)) (reify b)) (reify (codedSubst replT tgtT b))) ->
  Deriv (eqF (ap1 (closedSubstTFor (reify replT) (reify tgtT)) (ap2 Pair (reify (codeF2 g)) (reify b))) (reify (codedSubst replT tgtT (nd (codeF2 g) b))))
closedSTFNdF2 replT tgtT g b = closedSTFNd replT tgtT (codeF2 g) b (codeF2NeqTagVar g) (codeF2NotVar g)

closedSTFNdCode : (replT tgtT : Tree) (t : Term) (b : Tree) ->
  Deriv (eqF (ap1 (closedSubstTFor (reify replT) (reify tgtT)) (reify (code t))) (reify (codedSubst replT tgtT (code t)))) ->
  Deriv (eqF (ap1 (closedSubstTFor (reify replT) (reify tgtT)) (reify b)) (reify (codedSubst replT tgtT b))) ->
  Deriv (eqF (ap1 (closedSubstTFor (reify replT) (reify tgtT)) (ap2 Pair (reify (code t)) (reify b))) (reify (codedSubst replT tgtT (nd (code t) b))))
closedSTFNdCode replT tgtT t b = closedSTFNd replT tgtT (code t) b (codeNeqTagVarGen t) (codeNotVar t)

------------------------------------------------------------------------
-- natCode and tag helpers

closedSTFNatCode : (replT tgtT : Tree) (n : Nat) ->
  Deriv (eqF (ap1 (closedSubstTFor (reify replT) (reify tgtT)) (reify (natCode n))) (reify (codedSubst replT tgtT (natCode n))))
...
closedSTFTagAp1 : (rT tT : Tree) ->
  Deriv (eqF (ap1 (closedSubstTFor (reify rT) (reify tT)) (reify tagAp1)) (reify (codedSubst rT tT tagAp1)))
...
closedSTFTagAp2 : (rT tT : Tree) ->
  Deriv (eqF (ap1 (closedSubstTFor (reify rT) (reify tT)) (reify tagAp2)) (reify (codedSubst rT tT tagAp2)))
...
treeEqReify : (a b : Tree) ->
  Deriv (eqF (ap2 TreeEq (reify a) (reify b))
...
stepIsVar repl tgt reifyB =
  let cstf = closedSubstTFor repl tgt ; orig = ap2 Pair tagVarT reifyB
      ...
      sndArgF = Post Snd Pair ; outerF = Fan innerF sndArgF Pair
      cStepF = Fan isVarF outerF IfLf
      isVarEval : Deriv (eqF (ap2 isVarF orig recT) (ap2 TreeEq tagVarT (reify tagVar)))
      isVarEval = ruleTrans (fanRed (Lift Fst) (Lift (KT (reify tagVar))) TreeEq orig recT)
      ...
      matchEval : Deriv (eqF (ap2 matchF orig recT) (ap2 TreeEq reifyB tgt))
      ...
      replOrigEval : Deriv (eqF (ap2 replOrigF orig recT) (ap2 Pair repl orig))
...
ifLfTreeEq : (a b : Tree) (x y : Term) ->
  Deriv (eqF (ap2 IfLf (ap2 TreeEq (reify a) (reify b)) (ap2 Pair x y))
                 (boolCase (treeEq a b) x y))
ifLfTreeEq a b x y =
  ruleTrans (congL IfLf (ap2 Pair x y) (treeEqReify a b))
  (ifLfBool (treeEq a b) x y)
  where
  ifLfBool : (v : Bool) (x y : Term) ->
    Deriv (eqF (ap2 IfLf (reify (boolCase v lf (nd lf lf))) (ap2 Pair x y))
                   (boolCase v x y))
...
closedSTFCode : (replT tgtT : Tree) (l : Term) ->
  Deriv (eqF (ap1 (closedSubstTFor (reify replT) (reify tgtT)) (reify (code l))) (reify (codedSubst replT tgtT (code l))))
closedSTFF1 : (replT tgtT : Tree) (f : Fun1) ->
  Deriv (eqF (ap1 (closedSubstTFor (reify replT) (reify tgtT)) (reify (codeF1 f))) (reify (codedSubst replT tgtT (codeF1 f))))
closedSTFF2 : (replT tgtT : Tree) (g : Fun2) ->
  Deriv (eqF (ap1 (closedSubstTFor (reify replT) (reify tgtT)) (reify (codeF2 g))) (reify (codedSubst replT tgtT (codeF2 g))))
...
closedSTFCode replT tgtT (var n) =
  ...
  where
  boolCaseReify : (v : Bool) (x y : Tree) ->
    Deriv (eqF (boolCase v (reify x) (reify y)) (reify (boolCase v x y)))
