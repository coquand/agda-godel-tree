...
module Guard.SubstOpUnify where
...
open import Guard.SubstTForCorrectUnify using (closedSubstTFor ; closedSTFCode)
...
substOpLf : (tC xC : Term) ->
  Deriv (eqF (ap2 substOp (ap2 Pair tC xC) O) O)
substOpLf tC xC = axRecPLf stepSubstP (ap2 Pair tC xC)

substOpNd : (tC xC aT bT : Term) ->
  Deriv (eqF (ap2 substOp (ap2 Pair tC xC) (ap2 Pair aT bT))
...
closedSubstTForLf : (tC xC : Term) ->
  Deriv (eqF (ap1 (closedSubstTFor tC xC) O) O)
...
private
  aFRed : (tC xC aT bT recs : Term) ->
    let origP = ap2 Pair (ap2 Pair tC xC) (ap2 Pair aT bT) in
    Deriv (eqF (ap2 aF origP recs) aT)
  ...
  bFRed : (tC xC aT bT recs : Term) ->
    let origP = ap2 Pair (ap2 Pair tC xC) (ap2 Pair aT bT) in
    Deriv (eqF (ap2 bF origP recs) bT)
  ...
  tCFRed : (tC xC aT bT recs : Term) ->
    let origP = ap2 Pair (ap2 Pair tC xC) (ap2 Pair aT bT) in
    Deriv (eqF (ap2 tCF origP recs) tC)
  ...
  xCFRed : (tC xC aT bT recs : Term) ->
    let origP = ap2 Pair (ap2 Pair tC xC) (ap2 Pair aT bT) in
    Deriv (eqF (ap2 xCF origP recs) xC)
  ...
  curNodeFRed : (tC xC aT bT recs : Term) ->
    let origP = ap2 Pair (ap2 Pair tC xC) (ap2 Pair aT bT) in
    Deriv (eqF (ap2 curNodeF origP recs) (ap2 Pair aT bT))
  curNodeFRed tC xC aT bT recs =
    let origP = ap2 Pair (ap2 Pair tC xC) (ap2 Pair aT bT) in
    ruleTrans (liftRed Snd origP recs)
              (axSnd (ap2 Pair tC xC) (ap2 Pair aT bT))

  recsFRed : (tC xC aT bT recs : Term) ->
    let origP = ap2 Pair (ap2 Pair tC xC) (ap2 Pair aT bT) in
    Deriv (eqF (ap2 recsF origP recs) recs)
...
stepSubstPAtPair : (tC xC aT bT recs : Term) ->
  Deriv (eqF (ap2 stepSubstP (ap2 Pair (ap2 Pair tC xC) (ap2 Pair aT bT)) recs)
                 (canonForm tC xC aT bT recs))
stepSubstPAtPair tC xC aT bT recs =
  let origP = ap2 Pair (ap2 Pair tC xC) (ap2 Pair aT bT)
      ...
      isVarEval : Deriv (eqF (ap2 isVarFP origP recs) tgtIsVar)
      isVarEval =
      ...
      matchEval : Deriv (eqF (ap2 matchFP origP recs) tgtMatch)
      matchEval =
      ...
      replOrigEval : Deriv (eqF (ap2 replOrigFP origP recs) tgtReplOrig)
      replOrigEval =
      ...
      innerEval : Deriv (eqF (ap2 innerFP origP recs) tgtInner)
      innerEval =
      ...
      outerEval : Deriv (eqF (ap2 outerFP origP recs) tgtOuter)
      outerEval =
...
private
  cAFRed : (aT bT recs : Term) ->
    Deriv (eqF (ap2 (Lift Fst) (ap2 Pair aT bT) recs) aT)
  cAFRed aT bT recs =
    ruleTrans (liftRed Fst (ap2 Pair aT bT) recs) (axFst aT bT)

  cBFRed : (aT bT recs : Term) ->
    Deriv (eqF (ap2 (Lift Snd) (ap2 Pair aT bT) recs) bT)
  cBFRed aT bT recs =
    ruleTrans (liftRed Snd (ap2 Pair aT bT) recs) (axSnd aT bT)

  cRecsFRed : (aT bT recs : Term) ->
    Deriv (eqF (ap2 (Post Snd Pair) (ap2 Pair aT bT) recs) recs)
...
cStepOfAtPair : (tC xC aT bT recs : Term) ->
  Deriv (eqF (ap2 (cStepOf tC xC) (ap2 Pair aT bT) recs)
                 (canonForm tC xC aT bT recs))
cStepOfAtPair tC xC aT bT recs =
  let origC = ap2 Pair aT bT
      ...
      tgtOuter    = ap2 Pair tgtInner recs
      -- isVarFC eval
      isVarEval : Deriv (eqF (ap2 isVarFC origC recs) tgtIsVar)
      isVarEval =
      ...
      -- matchFC eval
      matchEval : Deriv (eqF (ap2 matchFC origC recs) tgtMatch)
      matchEval =
      ...
      -- replOrigFC eval
      replOrigEval : Deriv (eqF (ap2 replOrigFC origC recs) tgtReplOrig)
      replOrigEval =
...
closedSTFAtPair : (tC xC aT bT : Term) ->
  let cstf = closedSubstTFor tC xC
      recs = ap2 Pair (ap1 cstf aT) (ap1 cstf bT)
  in Deriv (eqF (ap1 cstf (ap2 Pair aT bT))
                    (canonForm tC xC aT bT recs))
...
substOpCorrect : (t : Term) (x : Nat) (l : Term) ->
  Deriv (eqF (ap2 substOp (ap2 Pair (reify (code t)) (reify (natCode x)))
                              (reify (code l)))
                 (reify (code (subst x t l))))
substOpCorrect t x l =
  let tC       = reify (code t)
      ...
      rhsEq : Deriv (eqF (ap1 cstf (reify (code l))) (reify (code (subst x t l))))
      rhsEq = eqSubst (\c' -> Deriv (eqF (ap1 cstf (reify (code l))) (reify c')))
                      metaEq codedEq
