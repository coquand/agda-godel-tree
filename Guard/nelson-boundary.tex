...
(1)~a complete diagonal G\"odel~II proof~\cite{godel1931} (168 lines, 0 postulates);
(2)~a micro-Nelson instance~\cite{nelson1986} verifying 56 programs against a fixed target;
...
(4)~the Kritchman--Raz contradiction theorem~\cite{kritchmanraz2010} (\texttt{krClean}), fully proved.
...
The system is related to Rose's equational calculus~\cite{rose1953}
and to Leivant's intrinsic characterizations of complexity
classes~\cite{leivant1995}.
...
The Chaitin machine~\cite{chaitin1974} $C_l$ searches for proofs that individual
...
the code size~\cite{kolmogorov1965}).  For large~$l$, $|C_l| < l$, so $C_l$ itself
...
\item \textbf{The boundary is intrinsic.}
  ...
  backtracking levels in learning-based
  realizability~\cite{aschieri2011}.
...
\item \textbf{Bounded consistency is feasible.}
  ...
  self-reference.  This connects to the proof-length
  phenomena studied by Pudl\'ak~\cite{pudlak1998} and
  the feasible arithmetic of Cook~\cite{cook1975}.
\end{enumerate}
