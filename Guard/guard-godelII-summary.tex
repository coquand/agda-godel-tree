...
substitution: given a coded term~$\code{t}$, it replaces every
occurrence of variable~$x_n$ by a replacement term~$r$.
These two parameters ($r$ and~$n$) are themselves represented
by free variables in $\mathrm{substTFor}$:
variable~$x_{11}$ carries the replacement~$r$, and
variable~$x_{12}$ carries the target index~$\nat{n}$.
...
$\mathrm{closedSubstTFor}(r, \nat{n})$ applied to $\widehat{\code{t}}$
equals $\widehat{\code{t[r/x_n]}}$.
...
(with $x_{11}$, $x_{12}$ still free):
\[
  \mathrm{templateEqn} \;=\;
  \bigl(\mathrm{Eq}(\mathrm{thFunTFor}(x_0),\;
    \mathrm{substTFor}(x_1)) = \pmark\bigr)
\]
Here $x_0$ will be instantiated with a proof encoding and $x_1$
with a coded term. The key design choice: leaving $x_{11}$, $x_{12}$
free keeps the code of the template small (no nested $\mathrm{R}$
from the closed version). They are instantiated later in step~7 of
the proof.
...
(abstract, cached):
\begin{itemize}
\item $N = \mathrm{templateCode}$: the tree encoding of the template equation
\item $\hat{N}$: the numeral for $N$, i.e., $\widehat{N}$ as a term
\item $G = \mathrm{templateEqn}[\hat{N}/x_1]$: the G\"odel sentence
  (the template applied to its own code)
\item $\code{G}$: the tree encoding of~$G$
\item $\mathrm{cstf} = \mathrm{closedSubstTFor}(\widehat{\hat{N}},\;
  \nat{1})$: the substitution functor with replacement =
  code of the template and target = variable~$x_1$
\end{itemize}
