...
\title{Formalization of G\"odel's First Incompleteness Theorem\\
...
The \emph{second} incompleteness theorem ($\mathrm{Con}$ is not
provable) would additionally require formalizing the proof of
Theorem~\ref{thm:godel1} \emph{inside} the system, to derive
$\vdash \mathrm{Con} \to G$.  Then if $\vdash \mathrm{Con}$, we
get $\vdash G$, contradicting Theorem~\ref{thm:godel1}.
This internalization step is not carried out here.
In Rose's paper~\cite{rose1961}, Theorem~18 corresponds to our
result; the passage to G\"odel~II would require Rose's
Theorems~15--17 to be formalized inside the object system.
\end{remark}
...
\subsection*{From G\"odel I to G\"odel II}

Theorem~\ref{thm:godel1} shows $G$ is not provable (assuming
consistency and $\mathrm{ProofE}$).  For G\"odel's \emph{second}
incompleteness theorem ($\mathrm{Con}$ is not provable), one would
need to show that the system can derive
$\mathrm{Con} \to G$
internally.  This requires formalizing steps~1--9 above as an
object-level derivation, which amounts to proving $\mathrm{ProofE}$
and the diagonal construction inside the system.
This internalization is not carried out in the present formalization.
...
Theorems~15--16 & Cases of $\mathrm{thm14E}$ + $\mathrm{cong}_L$/$\mathrm{cong}_R$ \\
