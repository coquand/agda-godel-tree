...
-- Level 1: abstract codes and closedSTFF1 for thFunTFor/substTFor.
-- Heavy traversals happen here (once, cached in .agdai).

module Guard.SubstTForPrecomp where
...
abstract
  ...
  -- instForm for the chain equation: TreeEq(cstf(reify TC), substTFor(reify TC)) = poo
  -- After ruleInst v11/v12, substTFor becomes cstf. cstf and reify TC are invariant.
  instForm2 :
    Eq (substEq v12' (reify (natCode v1)) (substEq v11' (reify crTC)
        (eqn (ap2 TreeEq (ap1 cstf (reify templateCode)) (ap1 substTFor (reify templateCode))) poo)))
       (eqn (ap2 TreeEq (ap1 cstf (reify templateCode)) (ap1 cstf (reify templateCode))) poo)
  instForm2 = refl
