...
recUnfoldIHV3 :
  (hCode ra rb : Term) (a b : Term) ->
  Deriv (eqF (ap1 (thmT hCode) a) ra) ->
  Deriv (eqF (ap1 (thmT hCode) b) rb) ->
  Deriv (eqF (ap1 (thmT hCode) (ap2 Pair a b))
...
guardNdV3 : (hCode tag x y recs : Term) ->
  Deriv (eqF (ap2 (thmTStep hCode) (ap2 Pair tag (ap2 Pair x y)) recs)
...
guardLfV3 : (hCode tag recs : Term) ->
  Deriv (eqF (ap2 (thmTStep hCode) (ap2 Pair tag O) recs)
...
case26Match : (hCode tag recs : Term) ->
  (Deriv (eqF (ap2 TreeEq hCode hCode) O)) ->
  Deriv (eqF (ap2 (case26 hCode) (ap2 Pair tag hCode) recs) hCode)
...
intermediateGenericV3 :
  ...
  (sp2a sp2b : Term)
  -> ((x recs : Term) ->
      Deriv (eqF (ap2 (ndDispatchV3 hCode) (ap2 Pair tagT x) recs)
                     (ap2 sndArg2 (ap2 Pair tagT x) recs)))
  -> Deriv (eqF (ap1 (thmT hCode) (ap2 Pair tagT (ap2 Pair sp2a sp2b)))
