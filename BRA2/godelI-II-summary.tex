...
not what step~3 uses.  The actual specification is purely projective
on Term shape:
...
\begin{quote}
  ...
  $\mathit{thmT}(\mathit{arg})$ dispatches to
  $\mathit{Snd}(\mathit{Snd}(\mathit{thmT}\,y_1))$, provided the
  outer-shape discriminant on $\mathit{thmT}\,y_1$ matches
  $\mathit{ap}_2\;\mathit{Pair}\;\mathit{tagImp}\;(\mathit{ap}_2\;
   \mathit{Pair}\;X\;Y)$ for some Terms $X, Y$; otherwise it returns
  $\mathit{codeTriv}$.
\end{quote}

The discriminant only inspects the outer $\mathit{Fst}$/$\mathit{Snd}$
projections.  Whatever sits in the inner slots --- $\mathit{codeFormula}\,P$
and $\mathit{codeFormula}\,Q$, or $\mathit{cor}\,x$ interleaved with
tag constants, or arbitrary value-shaped Terms with the right outer
shape --- gets carried through the $\mathit{Snd} \circ \mathit{Snd}$
projection unchanged.

This is exactly what makes step~3 work.  The substituted-code
$\widehat{\mathit{th}\,\hat x = \mathit{sub}\,j\,j}$ has outer shape
$\mathit{ap}_2\;\mathit{Pair}\;\mathit{tagImp}\;(\mathit{ap}_2\;
 \mathit{Pair}\;\_\;\_)$ even though it is not $\mathit{codeFormula}\,P$
for any $P$.  The $\mathit{mp}$-dispatch consults only the outer
$\mathit{Fst}$/$\mathit{Snd}$ projections; the $\mathit{cor}\,x$
sitting in some inner slot is \emph{carried} by the projection, not
\emph{understood} by it.  The chain's two $\mathit{mp}$-internalised
steps inside $g\_\mathit{part}$ produce a Term of the form
\[
...
\]
equationally, with no claim about Formula content.  The result is
again a substituted-code, and feeds into the next step the same way.
