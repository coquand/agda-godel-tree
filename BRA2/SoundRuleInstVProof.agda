...
module BRA2.SoundRuleInstVProof where
...
----------------------------------------------------------------------
-- Body and helpers (formerly in BRA2.SoundRuleInstProto).
...
body_ruleInst_v_eval_pass a bb argsPair t1 codeP fstPart sndPart
  ...
  let
      ...
      e3 = axKT (codeFormula (atomic (eqn O O))) (codeFormula_isValue (atomic (eqn O O))) tprime
...
-- replaced by its lifted counterpart from BRA2.Thm.EvalHelpers; closed
...
body_ruleInst_v_eval_pass_l P a bb argsPair t1 codeP fstPart sndPart
  ...
  let
      ...
      e3A = axKT (codeFormula (atomic (eqn O O))) (codeFormula_isValue (atomic (eqn O O))) tprime
