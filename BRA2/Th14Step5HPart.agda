...
module BRA2.Th14Step5HPart where
...
open import BRA2.Thm.EvalHelpers using (liftAxiom)
...
open import BRA2.Thm14Numerals using (t'_formula)
open import BRA2.GoedelII using (cG ; G ; i ; j ; j_isValue ; PrAtX)
...
private
  ...
  -- bot from BRA2.Thm11Abstract = atomic (eqn O (ap2 Pair O O)).
...
private
  ...
  bA'_codeO = subT_preserves_via_meta (suc zero) encoded_sub_ii (code O) (code_isValue O) refl
  ...
  bA'_codeFalse = subT_preserves_via_meta (suc zero) encoded_sub_ii (code (ap2 Pair O O))
                                          (code_isValue (ap2 Pair O O)) refl
...
private
  ...
  bB'_subII x =
    ...
    in subT_preserves_via_meta zero (encoded_th_x_at x) (code (ap2 sub i i))
  ...
  bB'_bot : (x : Term) ->
    Deriv (atomic (eqn (ap2 subT (vc0sub x) (reify (codeFormula bot_local)))
                        (reify (codeFormula bot_local))))
  bB'_bot x = subT_preserves_via_meta zero (encoded_th_x_at x) (codeFormula bot_local)
                                       (codeFormula_isValue bot_local) refl
