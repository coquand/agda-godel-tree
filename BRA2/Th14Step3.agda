...
open import BRA2.Thm14Numerals using
  ( t_term ; t_term_isValue ; t_formula ; t_witness ; eq01 ; eq02 ; eq12 )

open import BRA2.GoedelII using (i ; cG ; G ; j ; j_isValue)
...
private
  pair_unfold_kt : (tagV : Term) -> IsValue tagV -> (innerF : Fun1) (x : Term) ->
    Deriv (atomic (eqn
      (ap1 (Comp2 Pair (KT (reify tagV)) innerF) x)
      (ap2 Pair (reify tagV) (ap1 innerF x))))
  pair_unfold_kt tagV tagV_iv innerF x =
    let
        s1 = axComp2 Pair (KT (reify tagV)) innerF x
        s2_kt = ktRed tagV tagV_iv x
...
module Th14Step3
  ...
  f_part_inner2_unfold x =
    let
        ...
        kt_v2 = ktRed (code (var (suc (suc zero)))) (code_isValue (var (suc (suc zero)))) x

        kt_sub : Deriv (atomic (eqn (ap1 (KT sub_ii_subst) x) sub_ii_subst))
        kt_sub = ktRed (code (reify (codeFormula G))) (code_isValue (reify (codeFormula G))) x
        ...
        kt_t = ktRed t_term t_term_isValue x
        ...
        kt_tag = ktRed (natCode tagRuleInst) (natCode_isValue tagRuleInst) x
  ...
  f_part_inner1_unfold x =
    let
        ...
        kt_v1 = ktRed (code (var (suc zero))) (code_isValue (var (suc zero))) x

        kt_sub : Deriv (atomic (eqn (ap1 (KT encoded_sub_ii) x) encoded_sub_ii))
        kt_sub = ktRed (code (ap2 sub i i)) (code_isValue (ap2 sub i i)) x
  ...
  encoded_th_x_unfold x =
    let
        ...
        kt_codef1 : Deriv (atomic (eqn (ap1 (KT (reify (codeF1 thmT))) x)
                                        (reify (codeF1 thmT))))
        kt_codef1 = ktRed (codeF1 thmT) (codeF1_isValue thmT) x
        ...
        kt_tagap1 = ktRed tagAp1 tagAp1_isValue x
  ...
  f_part_unfold x =
    let
        ...
        kt_v0 = ktRed (code (var zero)) (code_isValue (var zero)) x
  ...
  step3_pre x =
    let
        ...
        layer1_shape = subTOfCodeFormulaImp_isPair (suc (suc zero))
                         ...
                         (code_isValue (reify (codeFormula G)))
                         eq02 (eq12 imp eq01)
        layer1_fp : Term
        ...
        layer2_shape = subTOfCodeFormulaImp_isPair (suc zero)
                         (code (ap2 sub i i)) (code_isValue (ap2 sub i i)) F1Q F1R
        layer2_fp : Term
  ...
  -- (Stage A + B + C from BRA2.Th14Step3Canon, plumbed via Comp2/KT bridges)
  ...
  open import BRA2.Cor      using (corOfReify)
  open import BRA2.Thm.ThmT using
    ( thmTDispMp_param ; thmTDispMp_param_l ; tagCode_mp )
  open import BRA2.Thm14Constr5Final using ()
  open Th14Constr5Final r12_th r12_sub
    using (g_part_inner ; g_part ; D_thmT ; D_sub_at_ii ; D_sub)
  open import BRA2.Th14GPartUnfolds using (module Th14GPartUnfolds)
  open Th14GPartUnfolds r12_th r12_sub
    using (g_part_inner_unfold ; g_part_unfold)

  open import BRA2.Th14Step3Canon
  ...
  open import BRA2.GoedelII using (PrAtX)
  open import BRA2.CorCGBridge using (corCGBridge)
  open import BRA2.Th14Constr5
  ...
  open import BRA2.Thm.Tag using (tagMp ; tagAxContrapos)
  ...
  step3_l x =
    let
        ...
        cor_i_eq = corOfReify j j_isValue
