...
-- BRA2.TreeEqReflParam
--
-- Parametric reflexivity:
--   treeEqReflParam : (t : Term) ->
--     Deriv (atomic (eqn (ap2 TreeEq t t) O))
--
-- Generalises BRA2.SubT.treeEqRed (which works only for closed Tree
-- inputs of the form  reify a , reify b ) to an arbitrary Term t.
-- Used by  body_mp_v_eval_pass  (BRA2.SoundMpVProof) when its inner
...
-- / liftedCongLTwo from BRA2.Thm.EvalHelpers (Hilbert/Carneiro-style
...
module BRA2.TreeEqReflParam where
