...
explicitly.  We also discuss an architectural simplification that would
eliminate roughly ten thousand lines of substitution-machinery currently
used for convenience.  Throughout we follow Shoenfield's notation
...
Set
\[
h(\mathbf{x}) \;:=\; \seq{\mathbf{k}_{\mathrm{tag\_sb_2}}, \sigma(\mathbf{x}), t'},
\]
where $\sigma(\mathbf{x})$ is the two-variable substitution
specification
\[
\sigma(\mathbf{x}) \;:=\; \seq{\seq{\mathbf{k}_0, \code{\thmT(\num(\mathbf{x}))}},\, \seq{\mathbf{k}_1, \code{\sub(\mathbf{k}_i, \mathbf{k}_i)}}}.
\]
Then by the $\subtwo$-closure of $\thmT$ and the per-shape closures of
the two-variable formula substitution $\sbftwo$, writing
$E := \thmT(\num(\mathbf{x})) = \sub(\mathbf{k}_i, \mathbf{k}_i)$,
\[
...
\item In the description of $h(\mathbf{x})$, Guard does not commit to
a representation of the two-variable substitution specification.  The
formalisation must fix one
($\seq{\seq{\mathbf{k}_0, S_0}, \seq{\mathbf{k}_1, S_1}}$) and
prove that the $\sbftwo$ closures are universal in $S_0, S_1$.
\end{itemize}

\section{The simultaneous-substitution machinery: where it is, and is not, needed}
\label{sec:sb2}

The Agda development uses a dedicated two-variable simultaneous
substitution $\subtwo$ for $h(\mathbf{x})$ in Step~5a, and a
three-variable $\mathbf{sub_3}$ for the encoded equational congruence
axioms feeding Theorem~12.  This requires $\sbttwo, \sbftwo,
\mathbf{sbt_3}, \mathbf{sbf_3} \in \Fun_2$ by course-of-values, the
contract records \verb|SbContract2|/\verb|SbContract3| (8--11 closure
lemmas each), and two extra discriminator levels
($\mathbf{k}_{\mathrm{tag\_sb_2}}$, $\mathbf{k}_{\mathrm{tag\_sb_3}}$) in
the $\thmT$ cascade --- roughly $10\,000$ lines across a dozen files.  The
natural question is whether this is necessary.  The answer is subtle, and
worth recording carefully because it pinpoints the one place Guard's text
leaves the argument incomplete.

\paragraph{Iterated single substitution does not eliminate it.}  The
tempting factorisation of a two-variable simultaneous substitution into
single ones with a fresh-variable rename,
\[
\sbttwo((\mathbf{a}/\mathbf{x}_0, \mathbf{b}/\mathbf{x}_1),\, \mathbf{c}) \;=\; \sbt(\mathbf{b}/\mathbf{x}_k)\,(\sbt(\mathbf{a}/\mathbf{x}_0)\,(\sbt(\code{\mathbf{x}_k}/\mathbf{x}_1)\,\mathbf{c})),
\]
requires the outer single substitution to leave the already-plopped
substituent $\mathbf{a}$ unchanged --- the encoded freshness identity
$\vdash \sbt(\seq{\mathbf{k}_k, S},\, \mathbf{a}) = \mathbf{a}$ when
$\mathbf{x}_k \notin \mathbf{a}$.  In the theorems the substituents are
$\num\,X$ for \emph{universal} $X$, and for \emph{open} $X$ (e.g.\
$X = \mathbf{x}_1$) the term $\num(\mathbf{x}_1)$ does not reduce:
$\num = \mathbf{C}\,\mathit{numAux}\,\mathbf{o}\,\mathbf{u}$ with
$\mathit{numAux} = \mathbf{R}\,\mathbf{o}\,\mathit{numStep}\,\mathbf{v}$,
whose $\mathbf{R}$-recursion fires only when its second argument is
syntactically $\mathbf{O}$ or $\mathbf{s}(\cdot)$; a variable is neither,
so $\num(\mathbf{x}_1)$ stays a raw open application.  Since $\sbt$
dispatches on $\mathit{get\_tag}(\mathit{input})$ first, and that tag is
an \emph{open} expression on the stuck term, the dispatch never settles:
$\sbt(\seq{\mathbf{k}_k, S}, \num(\mathbf{x}_1))$ is not provably equal to
anything --- \emph{even with an identity (passthrough) otherwise clause},
since one cannot prove the otherwise branch is reached.  The freshness
identity \emph{is} provable for \emph{var-free} $X$ (closed numerals:
$\num\,X$ reduces to a $\code{\cdot}$-tree with no
$\seq{\mathbf{k}_{\mathrm{tag\_var}}, \cdot}$ leaf), but not for open $X$.

\paragraph{What Guard actually does (guard15.pdf).}  Reading Guard's own
definitions settles the question.  His verifier $\mathbf{th}$
(Definition~16) has \emph{only a single-variable substitution clause}:
\[
\mathbf{th}(4y{+}1) = \mathbf{sb}(\seq{KKy, IKy},\, \mathbf{th}(Ly)), \qquad
\mathbf{sb}(J(\code{A}, n), \code{P}) = \code{S^{\mathbf{x}_n}_{A}\,P}.
\]
The congruence axioms are produced by his $\mathbf{ax}$ functor
(Exercise~24[6]) as \emph{schemas} with free $\mathbf{x}_0,\mathbf{x}_1,
\mathbf{x}_2$, by \emph{direct assembly} from the functor's G\"odel
number, e.g.\ $\mathbf{ax}(9\code{f}{+}5) = \code{\mathbf{x}_0 =
\mathbf{x}_1 \supset f(\mathbf{x}_0) = f(\mathbf{x}_1)}$.  And in his
\emph{explicit} Theorem~14 construction (p.~17) the multi-variable effect
is obtained by \emph{nested single-$\mathbf{sb}$ steps with the open
substituent applied last}:
\[
f(\mathbf{x}) = 4J[\,J(\code{\thmT(\underline{\mathbf{x}})},0),\ 4J[\,J(\code{\sub(\mathbf{k}_i,\mathbf{k}_i)},1),\ 4J[\,J(\num\,j,2),\,t\,]{+}1\,]{+}1\,]{+}1,
\]
which substitutes $\mathbf{x}_2 := \num\,j$ (closed), then
$\mathbf{x}_1 := \sub(\mathbf{k}_i,\mathbf{k}_i)$ (closed), then
$\mathbf{x}_0 := \thmT(\underline{\mathbf{x}})$ (open) \emph{outermost}.
The closed substituents are re-scanned but pass through (var-free); the
open one is plopped last and never re-scanned.  This is exactly the
ordering trick --- and it is Guard's actual proof.

\paragraph{Hence Theorem~14 does not need $\subtwo/\mathbf{sub_3}$.}  Step
5a's $h(\mathbf{x})$ has a single open substituent
($Y_0 = \code{\thmT(\num\,\mathbf{x})}$; $Y_1 =
\code{\sub(\mathbf{k}_i,\mathbf{k}_i)}$ is var-free), so single
$\mathbf{sb}$ with the open substituent last suffices, exactly as in
Guard's $f,g,h$.  Our use of $\subtwo$ there is a uniformity choice, not a
necessity.

\paragraph{Theorem~12's compound cases are the real locus --- and Guard
leaves them to the reader.}  Theorem~12 (``\emph{meta-induction on the
length of the definition of a functor; the details are left to the
reader}'') builds the derivation handle $D_f$ by induction on the functor.
The base cases $\mathbf{s},\mathbf{o},\mathbf{u},\mathbf{v}$ are routine.
The \emph{compound} cases $\mathbf{C}(f,g,h)$ and $\mathbf{R}(\cdots)$ are
not: building $D_{\mathbf{C}(f,g,h)}$ needs, from
$g(\underline{\mathbf{x}}) = g(\mathbf{x})$ and
$h(\underline{\mathbf{x}}) = h(\mathbf{x})$, the congruence
\[
f(g(\underline{\mathbf{x}}), h(\underline{\mathbf{x}})) = f(g(\mathbf{x}), h(\mathbf{x})),
\]
i.e.\ the binary congruence schemas specialised at
$g(\underline{\mathbf{x}}), g(\mathbf{x}), h(\underline{\mathbf{x}}),
h(\mathbf{x})$ --- \emph{all open} (each involves the object variable via
$\underline{\mathbf{x}} = \num\,\mathbf{x}$).  With \emph{several}
independent open substituents, no ordering puts them all last, and
iterated single-$\mathbf{sb}$-with-rename re-scans an open $\num\,\mathbf{x}$
and stalls.  Guard escapes this \emph{meta-level}: his $\mathbf{sb},
\mathbf{th}$ are total primitive-recursive functions reasoned about via
representability (no internal tag-dispatch to stall), and he even defines
the multi-variable $\mathbf{sbt}/\mathbf{sbf}$ in Exercise~24[2,3].  But an
\emph{internal, explicit} construction of $D_f$ and a proof of its
$\thmT$-closure has no such escape: it must add a genuine multi-variable
substitution clause to $\thmT$.  That is precisely what $\subtwo/
\mathbf{sub_3}$ are.  So the single place a simultaneous-substitution
primitive is genuinely required is the \emph{internal formalisation of
Theorem~12's compound-functor induction step} --- exactly the step Guard
never wrote out.

\paragraph{These primitives are sound, because simultaneous instantiation
is an admissible rule.}  ``Primitive at the encoded-functor level'' must
not be read as ``unjustified axiom''.  The closures of $\mathtt{SbContract2}$
and $\mathtt{SbContract3}$ are the \emph{encoded-level counterparts} of a
\emph{derived} admissible rule of the Hilbert calculus,
\[
\mathbf{ruleInst2}\;:\; k_1 \neq k_2 \;\Longrightarrow\; \vdash \mathbf{P} \;\Longrightarrow\; \vdash \mathit{simSubstF}(k_1, t_1, k_2, t_2, \mathbf{P}),
\]
\emph{fully derived} (not postulated) in \verb|BRA3.RuleInst2| --- Church's
Standard Metatheorem~VII --- from ordinary single-variable substitution by
alpha-renaming through a fresh internal variable; the three-variable
$\mathbf{ruleInst3}$ likewise in \verb|BRA4.RuleInst3|.  Thus
$\subtwo/\mathbf{sub_3}$ encode a rule the system already admits and add no
proof-theoretic strength.  The contrast is the whole point of this
section: the \emph{abstract} rule $\mathbf{ruleInst2}$ \emph{is} derivable
from single substitution (on raw formulas, where the fresh rename works),
whereas its \emph{encoded functor} $\sbttwo$ is not reducible to iterated
encoded $\sbt$ --- the encoded re-scan stalls on the open $\num\,X$.  The
admissibility lives at the abstract level; the encoded functor is its
faithful internalisation, needed exactly where Guard reasons abstractly
(Theorem~12) rather than constructing explicitly (Theorem~14).

\paragraph{Refactor, not elimination.}  The only honest simplification is
to \emph{unify} $\subtwo, \mathbf{sub_3}$ (and any $\mathbf{sub_n}$) into a
single $n$-ary $\mathbf{sub\_multi}$ primitive --- a spec-list of
$(\text{var}, \text{substituent})$ pairs plopped simultaneously ---
collapsing the dozen files to one functor, one contract, and one cascade
level.  This consolidates the $\sim 10\,000$ lines and removes the
duplicated $2$- versus $3$-variable code, but it remains a primitive
simultaneous substitution; it does not reduce to single-variable $\sbt$.
...
machinery (closedness side conditions, predicate-Leibniz, Carneiro
lift, validating-decoder invariant, the two-forms-of-$\mathbf{G}$
distinction) on which Guard's text relies without comment.  As {\S}\ref{sec:sb2}
documents, Theorem~14 itself needs only single-variable substitution with
the open substituent applied last --- exactly Guard's explicit
construction (guard15.pdf p.~17); the simultaneous-substitution machinery
$\subtwo/\mathbf{sub_3}$ becomes \emph{necessary only for the internal
formalisation of Theorem~12's compound-functor induction step}, the one
step Guard leaves to the reader, where a congruence must be specialised at
several open terms at once.  That machinery is sound (the encoded
counterpart of the derived rules $\mathbf{ruleInst2}/\mathbf{ruleInst3}$,
Church's Standard Metatheorem~VII) and the natural future refactor is to
\emph{unify} the $2$- and $3$-variable functors into a single $n$-ary
$\mathbf{sub\_multi}$ primitive, not to eliminate them.
