...
-- BRA4.SbT2AtVar -- discharge of the SbContract2 sbt2 closures at the
-- three var-shape inputs:
--
--   sbt2_at_var_match_one :
--     ap2 sbt2 (pi (pi (natCode k1) S1) (pi (natCode k2) S2))
--              (pi (natCode tag_var) (natCode k1))                    =  S1
--
--   sbt2_at_var_match_two :
--     Eq (natEq k1 k2) false ->
--     ap2 sbt2 (pi (pi (natCode k1) S1) (pi (natCode k2) S2))
--              (pi (natCode tag_var) (natCode k2))                    =  S2
--
--   sbt2_at_var_nomatch :
...
--   1.  sbt2_unfold      :  sbt2 spec t = readOff_spec (sbt2State spec t)
--   2.  readOff_at_pi_natCode  :  reduce sbt2State ... t to stateMeta (cantor 1 m).
--   3.  cantor_var_succ_decomp :  cantor 1 m = suc (cantorVarPred m).
--   4.  readOff_stateMeta_succ_via_natCode :  = stepFun_sbt2 (natCode K_pred) (Snd ...).
--   5.  Unfold stepFun_sbt2 = Post stepBody_sbt2 pi.  Dispatch through
...
private
  ...
  -- ap1 get_spec2Fst (pi A Y) = ap1 Fst (ap1 Snd (ap1 Fst Y)).
  get_spec2Fst_at_pi :
    (A Y : Term) ->
    Deriv (eqF (ap1 get_spec2Fst (ap2 pi A Y)) (ap1 Fst (ap1 Snd (ap1 Fst Y))))
  ...
  -- ap1 get_spec2Snd (pi A Y) = ap1 Snd (ap1 Snd (ap1 Fst Y)).
  get_spec2Snd_at_pi :
    (A Y : Term) ->
    Deriv (eqF (ap1 get_spec2Snd (ap2 pi A Y)) (ap1 Snd (ap1 Snd (ap1 Fst Y))))
...
private
  -- ap1 stepBody_sbt2 input = condFork (C pi var_branch ap1_or_above input) (isVar input).
  ...
  -- ap1 var_branch_match2 input = condFork (C pi get_spec2Snd get_newK input) (isVarMatch2 input).
  var_branch_match2_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 var_branch_match2 input)
                (ap2 condFork
                  (ap1 (C pi get_spec2Snd get_newK) input)
                  (ap1 isVarMatch2 input)))
  var_branch_match2_unfold input =
    ax_C condFork (C pi get_spec2Snd get_newK) isVarMatch2 input

  pi_spec2Snd_newK_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 (C pi get_spec2Snd get_newK) input)
                (ap2 pi (ap1 get_spec2Snd input) (ap1 get_newK input)))
  pi_spec2Snd_newK_unfold input = ax_C pi get_spec2Snd get_newK input
...
--   Fst Y = spec2 = pi (pi (natCode k1) S1) (pi (natCode k2) S2) .

private
  spec2Term : (k1 k2 : Nat) (S1 S2 : Term) -> Term
  spec2Term k1 k2 S1 S2 =
    ap2 pi (ap2 pi (natCode k1) S1) (ap2 pi (natCode k2) S2)

  -- get_spec1Fst input = natCode k1, given Fst Y = spec2.
  get_spec1Fst_value :
    (A Y : Term) (k1 k2 : Nat) (S1 S2 : Term) ->
    Deriv (eqF (ap1 Fst Y) (spec2Term k1 k2 S1 S2)) ->
    Deriv (eqF (ap1 get_spec1Fst (ap2 pi A Y)) (natCode k1))
  get_spec1Fst_value A Y k1 k2 S1 S2 fstY_eq =
    let s1 :
          Deriv (eqF (ap1 get_spec1Fst (ap2 pi A Y)) (ap1 Fst (ap1 Fst (ap1 Fst Y))))
        s1 = get_spec1Fst_at_pi A Y

        -- Fst Y -> spec2 = pi (pi nck1 S1) (pi nck2 S2).
        s2 :
          Deriv (eqF (ap1 Fst (ap1 Fst Y))
                      (ap1 Fst (spec2Term k1 k2 S1 S2)))
        s2 = cong1 Fst fstY_eq

        -- Fst (pi (pi nck1 S1) (pi nck2 S2)) = pi nck1 S1.
        s3 :
          Deriv (eqF (ap1 Fst (spec2Term k1 k2 S1 S2)) (ap2 pi (natCode k1) S1))
        s3 = axFst (ap2 pi (natCode k1) S1) (ap2 pi (natCode k2) S2)

        -- Fst (pi nck1 S1) = nck1.
        s4 :
          Deriv (eqF (ap1 Fst (ap2 pi (natCode k1) S1)) (natCode k1))
        s4 = axFst (natCode k1) S1
    in ruleTrans s1
         (ruleTrans (cong1 Fst s2)
           (ruleTrans (cong1 Fst s3) s4))

  -- get_spec1Snd input = S1.
  get_spec1Snd_value :
    (A Y : Term) (k1 k2 : Nat) (S1 S2 : Term) ->
    Deriv (eqF (ap1 Fst Y) (spec2Term k1 k2 S1 S2)) ->
    Deriv (eqF (ap1 get_spec1Snd (ap2 pi A Y)) S1)
  get_spec1Snd_value A Y k1 k2 S1 S2 fstY_eq =
    let s1 :
          Deriv (eqF (ap1 get_spec1Snd (ap2 pi A Y)) (ap1 Snd (ap1 Fst (ap1 Fst Y))))
    ...
    let s1 :
        ...
        s2 :
          Deriv (eqF (ap1 Snd (ap1 Fst Y))
                      (ap1 Snd (spec2Term k1 k2 S1 S2)))
        s2 = cong1 Snd fstY_eq

        s3 :
          Deriv (eqF (ap1 Snd (spec2Term k1 k2 S1 S2)) (ap2 pi (natCode k2) S2))
        s3 = axSnd (ap2 pi (natCode k1) S1) (ap2 pi (natCode k2) S2)

        s4 :
          Deriv (eqF (ap1 Fst (ap2 pi (natCode k2) S2)) (natCode k2))
        s4 = axFst (natCode k2) S2
    in ruleTrans s1
         (ruleTrans (cong1 Fst s2)
           (ruleTrans (cong1 Fst s3) s4))

  -- get_spec2Snd input = S2.
  get_spec2Snd_value :
    (A Y : Term) (k1 k2 : Nat) (S1 S2 : Term) ->
    Deriv (eqF (ap1 Fst Y) (spec2Term k1 k2 S1 S2)) ->
    Deriv (eqF (ap1 get_spec2Snd (ap2 pi A Y)) S2)
  get_spec2Snd_value A Y k1 k2 S1 S2 fstY_eq =
    let s1 :
          Deriv (eqF (ap1 get_spec2Snd (ap2 pi A Y)) (ap1 Snd (ap1 Snd (ap1 Fst Y))))
        ...
        s4 :
          Deriv (eqF (ap1 Snd (ap2 pi (natCode k2) S2)) S2)
        s4 = axSnd (natCode k2) S2
    in ruleTrans s1
         (ruleTrans (cong1 Snd s2)
           (ruleTrans (cong1 Snd s3) s4))
...
private
  -- Step: under isVar input = sO, stepBody_sbt2 input = var_branch input.
  stepBody_to_var_branch :
    (input : Term) ->
    Deriv (eqF (ap1 isVar input) (ap1 s O)) ->
    Deriv (eqF (ap1 stepBody_sbt2 input) (ap1 var_branch input))
        ...
        isVM2_subst :
          Deriv (eqF (ap2 condFork
                      ...
                      (ap2 condFork
                       ...
                       (ap1 s O)))
        isVM2_subst =
          congR condFork (ap1 (C pi get_spec2Snd get_newK) input) isVM2_sO

        condFork_to_Fst :
          Deriv (eqF (ap2 condFork
                       (ap1 (C pi get_spec2Snd get_newK) input) (ap1 s O))
                      (ap1 Fst (ap1 (C pi get_spec2Snd get_newK) input)))
        condFork_to_Fst =
          condFork_true_nc (ap1 (C pi get_spec2Snd get_newK) input) O

        pi_eq :
          Deriv (eqF (ap1 (C pi get_spec2Snd get_newK) input)
                      (ap2 pi (ap1 get_spec2Snd input) (ap1 get_newK input)))
        pi_eq = pi_spec2Snd_newK_unfold input

        Fst_pi :
          Deriv (eqF (ap1 Fst (ap2 pi (ap1 get_spec2Snd input) (ap1 get_newK input)))
                      (ap1 get_spec2Snd input))
        Fst_pi = axFst (ap1 get_spec2Snd input) (ap1 get_newK input)
    in ruleTrans e1
         (ruleTrans isVM2_subst
           (ruleTrans condFork_to_Fst
             (ruleTrans (cong1 Fst pi_eq) Fst_pi)))

  -- Step: under isVarMatch2 input = O, var_branch_match2 input = get_newK input.
  match2_to_newK :
    (input : Term) ->
    Deriv (eqF (ap1 isVarMatch2 input) O) ->
    Deriv (eqF (ap1 var_branch_match2 input) (ap1 get_newK input))
  match2_to_newK input isVM2_O =
    let e1 :
          Deriv (eqF (ap1 var_branch_match2 input)
                      (ap2 condFork
                        (ap1 (C pi get_spec2Snd get_newK) input)
        ...
        isVM2_subst :
          Deriv (eqF (ap2 condFork
                      ...
                      (ap2 condFork
                       ...
                       O))
        isVM2_subst =
          congR condFork (ap1 (C pi get_spec2Snd get_newK) input) isVM2_O

        condFork_to_Snd :
          Deriv (eqF (ap2 condFork
                       (ap1 (C pi get_spec2Snd get_newK) input) O)
                      (ap1 Snd (ap1 (C pi get_spec2Snd get_newK) input)))
        condFork_to_Snd =
          condFork_false (ap1 (C pi get_spec2Snd get_newK) input)
        ...
        Snd_pi :
          Deriv (eqF (ap1 Snd (ap2 pi (ap1 get_spec2Snd input) (ap1 get_newK input)))
                      (ap1 get_newK input))
        Snd_pi = axSnd (ap1 get_spec2Snd input) (ap1 get_newK input)
    in ruleTrans e1
         (ruleTrans isVM2_subst
           (ruleTrans condFork_to_Snd
             (ruleTrans (cong1 Snd pi_eq) Snd_pi)))
...
--
--   stepFun_sbt2_at_var_match_one_core :
--     (k1 k2 : Nat) (S1 S2 : Term) (Y : Term) ->
--     Deriv (eqF (ap1 Fst Y) (spec2Term k1 k2 S1 S2)) ->
--     Deriv (eqF (ap2 stepFun_sbt2 (natCode (cantorVarPred k1)) Y) S1)
--
-- And similarly for _match_2 / _nomatch.

stepFun_sbt2_at_var_match_one_core :
  (k1 k2 : Nat) (S1 S2 Y : Term) ->
  Deriv (eqF (ap1 Fst Y) (spec2Term k1 k2 S1 S2)) ->
  Deriv (eqF (ap2 stepFun_sbt2 (natCode (cantorVarPred k1)) Y) S1)
stepFun_sbt2_at_var_match_one_core k1 k2 S1 S2 Y fstY_eq =
  let A : Term
      ...
      ----------------------------------------------------------------
      -- Step 1: stepFun_sbt2 (A, Y) = stepBody_sbt2 input.
      e1 : Deriv (eqF (ap2 stepFun_sbt2 A Y) (ap1 stepBody_sbt2 input))
      e1 = axPost stepBody_sbt2 pi A Y

      ----------------------------------------------------------------
      -- Step 2: isVar input = sO.
      ...
      ----------------------------------------------------------------
      -- Step 3: isVarMatch1 input = sO.
      --   get_body input = natCode k1, get_spec1Fst input = natCode k1.
      e3_unfold :
        Deriv (eqF (ap1 isVarMatch1 input)
                    (ap2 natEqF (ap1 get_body input) (ap1 get_spec1Fst input)))
      ...
      spec1Fst_value = get_spec1Fst_value A Y k1 k2 S1 S2 fstY_eq
      ...
      ----------------------------------------------------------------
      -- Step 4: var_branch input = get_spec1Snd input.
      var_to_s1Snd :
        Deriv (eqF (ap1 var_branch input) (ap1 get_spec1Snd input))
      var_to_s1Snd = var_branch_to_spec1Snd input isVM1_value

      ----------------------------------------------------------------
      -- Step 5: get_spec1Snd input = S1.
      s1Snd_value : Deriv (eqF (ap1 get_spec1Snd input) S1)
      s1Snd_value = get_spec1Snd_value A Y k1 k2 S1 S2 fstY_eq
  in ruleTrans e1
       (ruleTrans to_var_branch
         (ruleTrans var_to_s1Snd s1Snd_value))

stepFun_sbt2_at_var_match_two_core :
  (k1 k2 : Nat) (S1 S2 Y : Term) ->
  Eq (natEq k1 k2) false ->
  Deriv (eqF (ap1 Fst Y) (spec2Term k1 k2 S1 S2)) ->
  Deriv (eqF (ap2 stepFun_sbt2 (natCode (cantorVarPred k2)) Y) S2)
stepFun_sbt2_at_var_match_two_core k1 k2 S1 S2 Y k1_neq_k2 fstY_eq =
  let A : Term
      ...
      ----------------------------------------------------------------
      -- isVar input = sO (same as match_1 case but at index k2).
      isVar_value : Deriv (eqF (ap1 isVar input) (ap1 s O))
      ...
      ----------------------------------------------------------------
      -- isVarMatch1 input = O.
      --   get_body input = natCode k2, get_spec1Fst input = natCode k1.
      --   natEqF (natCode k2) (natCode k1) = O via natEqF_at_neq.
      e3_unfold :
        Deriv (eqF (ap1 isVarMatch1 input)
                    (ap2 natEqF (ap1 get_body input) (ap1 get_spec1Fst input)))
      e3_unfold = isVarMatch1_unfold input
      ...
      ----------------------------------------------------------------
      -- var_branch input = var_branch_match2 input.
      var_to_match2 :
        Deriv (eqF (ap1 var_branch input) (ap1 var_branch_match2 input))
      var_to_match2 = var_branch_to_match2 input isVM1_value

      ----------------------------------------------------------------
      -- isVarMatch2 input = sO.
      --   get_body input = natCode k2, get_spec2Fst input = natCode k2.
      e5_unfold :
        Deriv (eqF (ap1 isVarMatch2 input)
                    (ap2 natEqF (ap1 get_body input) (ap1 get_spec2Fst input)))
      ...
      spec2Fst_value = get_spec2Fst_value A Y k1 k2 S1 S2 fstY_eq
      ...
      ----------------------------------------------------------------
      -- var_branch_match2 input = get_spec2Snd input.
      match2_to_s2Snd :
        Deriv (eqF (ap1 var_branch_match2 input) (ap1 get_spec2Snd input))
      ...
      s2Snd_value = get_spec2Snd_value A Y k1 k2 S1 S2 fstY_eq
  in ruleTrans e1
       (ruleTrans to_var_branch
         (ruleTrans var_to_match2
           (ruleTrans match2_to_s2Snd s2Snd_value)))

stepFun_sbt2_at_var_nomatch_core :
  (k1 k2 m : Nat) (S1 S2 Y : Term) ->
  Eq (natEq k1 m) false -> Eq (natEq k2 m) false ->
  Deriv (eqF (ap1 Fst Y) (spec2Term k1 k2 S1 S2)) ->
  Deriv (eqF (ap2 stepFun_sbt2 (natCode (cantorVarPred m)) Y)
              (ap1 s (natCode (cantorVarPred m))))
stepFun_sbt2_at_var_nomatch_core k1 k2 m S1 S2 Y k1_neq_m k2_neq_m fstY_eq =
  let A : Term
      A = natCode (cantorVarPred m)
      ...
      match2_to_newK_eq :
        Deriv (eqF (ap1 var_branch_match2 input) (ap1 get_newK input))
      match2_to_newK_eq = match2_to_newK input isVM2_value
  ...
  in ruleTrans e1
       (ruleTrans to_var_branch
         (ruleTrans var_to_match2
           (ruleTrans match2_to_newK_eq newK_eq)))
...
sbt2_at_var_match_one :
  ...
  Deriv (eqF (ap2 sbt2 (spec2Term k1 k2 S1 S2)
                       (ap2 pi (natCode tag_var) (natCode k1))) S1)
sbt2_at_var_match_one k1 k2 S1 S2 =
  let spec : Term
      spec = spec2Term k1 k2 S1 S2
      ...
      step1 = sbt2_unfold spec t

      step2 :
        Deriv (eqF (ap1 readOff_spec (ap2 sbt2State spec t))
                    (ap1 readOff_spec
      ...
      step6 = stepFun_sbt2_at_var_match_one_core k1 k2 S1 S2 Y fstY_eq
  in ruleTrans step1
       (ruleTrans step2
         (ruleTrans step3
...
sbt2_at_var_match_two k1 k2 S1 S2 k1_neq_k2 =
  let spec : Term
      spec = spec2Term k1 k2 S1 S2
      ...
      step6 = stepFun_sbt2_at_var_match_two_core k1 k2 S1 S2 Y k1_neq_k2 fstY_eq
  in ruleTrans step1
       (ruleTrans step2
         (ruleTrans step3
           (ruleTrans step4 step6)))
...
sbt2_at_var_nomatch :
  ...
  Deriv (eqF (ap2 sbt2 (spec2Term k1 k2 S1 S2)
              ...
              (ap2 pi (natCode tag_var) (natCode m)))
sbt2_at_var_nomatch k1 k2 m S1 S2 k1_neq_m k2_neq_m =
  let spec : Term
      spec = spec2Term k1 k2 S1 S2
      ...
      step6 = stepFun_sbt2_at_var_nomatch_core k1 k2 m S1 S2 Y k1_neq_m k2_neq_m fstY_eq

      ----------------------------------------------------------------
      -- Bridge:  s (natCode K_pred) = natCode (cantor 1 m) = pi (natCode tag_var) (natCode m).
      step7a :
