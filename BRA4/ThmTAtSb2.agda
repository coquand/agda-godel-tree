...
-- BRA4.ThmTAtSb2 -- discharge of the  sb2  closure of  thmT  :
--
--   thmT_at_sb2 :
--     (cSpec2 cProofIdx : Term) ->
--     Deriv (eqF (ap1 thmT (ap2 pi (natCode tag_sb2) (ap2 pi cSpec2 cProofIdx)))
--                 (ap2 sbf2 cSpec2 (ap1 thmT cProofIdx)))
--
-- Universal in cSpec2, cProofIdx : Term -- no Closed premise.
--
-- Architecture mirrors  BRA4.ThmTAtSb  but the cascade descent walks
-- one extra level :  isAx=O , isSb=O , isSb3=O  (false branches via Snd) ,
-- then  isSb2=sO  (true branch via Fst) reaches  sb2_branch_thmT  =
-- C sbf2 get_sb_spec get_sb_proof_val .

module BRA4.ThmTAtSb2 where
...
------------------------------------------------------------------------
-- NatNeqWitnesses : tag_sb2 /= tag_ax / tag_sb / tag_sb3 .
...
private
  ...
  witness_sb2_neq_ax : NatNeqWitness tag_sb2 tag_ax
  witness_sb2_neq_ax = natEqFalse_to_witness tag_ax tag_sb2 refl

  witness_sb2_neq_sb : NatNeqWitness tag_sb2 tag_sb
  witness_sb2_neq_sb = natEqFalse_to_witness tag_sb tag_sb2 refl

  witness_sb2_neq_sb3 : NatNeqWitness tag_sb2 tag_sb3
  witness_sb2_neq_sb3 = natEqFalse_to_witness tag_sb3 tag_sb2 refl
...
private
  ...
  isSb2_unfold input =
    let s1 = ax_C natEqF get_tag (constN tag_sb2) input
...
------------------------------------------------------------------------
-- Tag-firing helpers : tag = natCode tag_sb2 .

private
  isAx_at_natCodeSb2_O :
    (input : Term) ->
    Deriv (eqF (ap1 get_tag input) (natCode tag_sb2)) ->
    Deriv (eqF (ap1 isAx input) O)
  isAx_at_natCodeSb2_O input tag_eq_pf =
    let s1 = isAx_unfold input
        s2 = congL natEqF (natCode tag_ax) tag_eq_pf
        s3 = natEqF_at_neq tag_sb2 tag_ax witness_sb2_neq_ax
    in ruleTrans s1 (ruleTrans s2 s3)

  isSb_at_natCodeSb2_O :
    (input : Term) ->
    Deriv (eqF (ap1 get_tag input) (natCode tag_sb2)) ->
    Deriv (eqF (ap1 isSb input) O)
  isSb_at_natCodeSb2_O input tag_eq_pf =
    let s1 = isSb_unfold input
        s2 = congL natEqF (natCode tag_sb) tag_eq_pf
        s3 = natEqF_at_neq tag_sb2 tag_sb witness_sb2_neq_sb
    in ruleTrans s1 (ruleTrans s2 s3)

  isSb3_at_natCodeSb2_O :
    (input : Term) ->
    Deriv (eqF (ap1 get_tag input) (natCode tag_sb2)) ->
    Deriv (eqF (ap1 isSb3 input) O)
  isSb3_at_natCodeSb2_O input tag_eq_pf =
    let s1 = isSb3_unfold input
        s2 = congL natEqF (natCode tag_sb3) tag_eq_pf
        s3 = natEqF_at_neq tag_sb2 tag_sb3 witness_sb2_neq_sb3
    in ruleTrans s1 (ruleTrans s2 s3)

  isSb2_at_natCodeSb2_sO :
    (input : Term) ->
    Deriv (eqF (ap1 get_tag input) (natCode tag_sb2)) ->
    Deriv (eqF (ap1 isSb2 input) (ap1 s O))
  isSb2_at_natCodeSb2_sO input tag_eq_pf =
    let s1 = isSb2_unfold input
...
private
  ...
  sb3_or_above_to_sb2_or_above input isSb3_O =
    let e1 = sb3_or_above_unfold input
        sub_isSb3 = congR condFork (ap1 (C pi sb3_branch_thmT sb2_or_above) input) isSb3_O
        cf_to_Snd = condFork_false (ap1 (C pi sb3_branch_thmT sb2_or_above) input)
  ...
  sb2_or_above_to_sb2 :
    (input : Term) ->
    Deriv (eqF (ap1 isSb2 input) (ap1 s O)) ->
    Deriv (eqF (ap1 sb2_or_above input) (ap1 sb2_branch_thmT input))
  sb2_or_above_to_sb2 input isSb2_sO =
    let e1 = sb2_or_above_unfold input
        sub_isSb2 = congR condFork (ap1 (C pi sb2_branch_thmT mp_or_above) input) isSb2_sO
        cf_to_Fst = condFork_true_nc (ap1 (C pi sb2_branch_thmT mp_or_above) input) O
        pi_eq = pi_sb2_mpor_unfold input
        Fst_pi = axFst (ap1 sb2_branch_thmT input) (ap1 mp_or_above input)
...
------------------------------------------------------------------------
-- Leq lemma : leq cProofIdx (pi_succ_outer A_outer (pi cSpec2 cProofIdx)).
-- A_outer is generic ; we instantiate at  natCode (predecessor of tag_sb2) .

leq_cProofIdx_P_outer_sb2 :
  (cSpec2 cProofIdx : Term) ->
  Deriv (leq cProofIdx
            (pi_succ_outer (natCode (suc (suc (suc (suc zero)))))
                           (ap2 pi cSpec2 cProofIdx)))
leq_cProofIdx_P_outer_sb2 cSpec2 cProofIdx =
  let A : Term
      A = natCode (suc (suc (suc (suc zero)))) -- natCode 4
      Y : Term
      Y = ap2 pi cSpec2 cProofIdx
      X : Term
      X = ap2 sigma (ap2 sigma A Y) (ap1 tau (ap2 sigma A Y))
      l1 : Deriv (leq cProofIdx Y)
      l1 = leq_pi_right cSpec2 cProofIdx
      l2 : Deriv (leq Y (ap2 sigma X Y))
      l2 = leq_sigma_right X Y
  in leq_trans cProofIdx Y (ap2 sigma X Y) l1 l2

------------------------------------------------------------------------
-- sb2_branch_thmT body unfolding.

private
  sb2_branch_thmT_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 sb2_branch_thmT input)
                (ap2 sbf2 (ap1 get_sb_spec input) (ap1 get_sb_proof_val input)))
  sb2_branch_thmT_unfold input =
    ax_C sbf2 get_sb_spec get_sb_proof_val input
...
thmT_at_sb2 :
  ...
  Deriv (eqF (ap1 thmT (ap2 pi (natCode tag_sb2) (ap2 pi cSpec2 cProofIdx)))
              (ap2 sbf2 cSpec2 (ap1 thmT cProofIdx)))
thmT_at_sb2 cSpec2 cProofIdx =
  let input : Term
      input = ap2 pi (natCode tag_sb2) (ap2 pi cSpec2 cProofIdx)

      Y_body : Term
      Y_body = ap2 pi cSpec2 cProofIdx

      -- tag_sb2 = 5 = suc 4 ; natCode 5 = s (natCode 4) , so
      -- A_outer = natCode 4 .
      A_outer : Term
      A_outer = natCode (suc (suc (suc (suc zero))))
      ...
      Fst_input :
        Deriv (eqF (ap1 Fst input) (natCode tag_sb2))
      Fst_input = axFst (natCode tag_sb2) Y_body
      ...
      get_tag_value :
        Deriv (eqF (ap1 get_tag input_pkg') (natCode tag_sb2))
      ...
      isAx_value = isAx_at_natCodeSb2_O input_pkg' get_tag_value

      isSb_value : Deriv (eqF (ap1 isSb input_pkg') O)
      isSb_value = isSb_at_natCodeSb2_O input_pkg' get_tag_value

      isSb3_value : Deriv (eqF (ap1 isSb3 input_pkg') O)
      isSb3_value = isSb3_at_natCodeSb2_O input_pkg' get_tag_value

      isSb2_value : Deriv (eqF (ap1 isSb2 input_pkg') (ap1 s O))
      isSb2_value = isSb2_at_natCodeSb2_sO input_pkg' get_tag_value
      ...
      sb3or_to_sb2or  = sb3_or_above_to_sb2_or_above  input_pkg' isSb3_value
      sb2or_to_sb2    = sb2_or_above_to_sb2           input_pkg' isSb2_value

      stepBody_to_sb2 :
        Deriv (eqF (ap1 stepBody_thmT input_pkg') (ap1 sb2_branch_thmT input_pkg'))
      stepBody_to_sb2 =
        ruleTrans stepBody_to_sbor
          (ruleTrans sbor_to_sb3or
            (ruleTrans sb3or_to_sb2or sb2or_to_sb2))

      -- sb2_branch unfolding.
      sb2_branch_value :
        Deriv (eqF (ap1 sb2_branch_thmT input_pkg')
                    (ap2 sbf2 (ap1 get_sb_spec input_pkg')
                            (ap1 get_sb_proof_val input_pkg')))
      sb2_branch_value = sb2_branch_thmT_unfold input_pkg'

      -- Compute get_sb_spec input_pkg' = cSpec2.
      Snd_sP_eq :
      ...
      Snd_input_eq_Yb = axSnd (natCode tag_sb2) Y_body
      ...
      leq_cP_P = leq_cProofIdx_P_outer_sb2 cSpec2 cProofIdx
      ...
      -- Assemble sbf2 cSpec2 (ap1 thmT cProofIdx).
      sb2_branch_to_sbf2 :
        Deriv (eqF (ap1 sb2_branch_thmT input_pkg')
                    (ap2 sbf2 cSpec2 (ap1 thmT cProofIdx)))
      sb2_branch_to_sbf2 =
        ruleTrans sb2_branch_value
          (ruleTrans (congL sbf2 (ap1 get_sb_proof_val input_pkg') get_sb_spec_value)
                      (congR sbf2 cSpec2 val_to_thmT))
      ...
      chain_to_sb2_branch :
        Deriv (eqF (ap1 thmT input) (ap1 sb2_branch_thmT input_pkg'))
      chain_to_sb2_branch = ruleTrans chain_to_stepBody stepBody_to_sb2
  in ruleTrans chain_to_sb2_branch sb2_branch_to_sbf2
