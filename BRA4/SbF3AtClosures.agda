...
-- BRA4.SbF3AtClosures -- discharge of the three formula closures of
--  SbContract :
--
--   sbf3_at_atomic :  ap2 sbf3 spec (pi (natCode tag_eq) (pi ca cb))
--                       = pi (natCode tag_eq) (pi (sbt3 spec ca) (sbt3 spec cb))
--   sbf3_at_neg    :  ap2 sbf3 spec (pi (natCode tag_neg) cP)
--                       = pi (natCode tag_neg) (sbf3 spec cP)
--   sbf3_at_imp    :  ap2 sbf3 spec (pi (natCode tag_imp) (pi cP cQ))
--                       = pi (natCode tag_imp) (pi (sbf3 spec cP) (sbf3 spec cQ))
--
...
-- + readOff + Post + stepBody_sbf3 cascade.  For tag_neg / tag_imp the
-- sub-positions are formulas and we use the sbf3 cov-table lookup +
-- stability bridge (HP_sbf_eq_sbf_under_leq).  For tag_eq the sub-positions
-- are TERMS and we use  C sbt3 get_spec get_ca_atom  + spec-preservation
-- (fstSnd_cov_spec_eq) -- no stability needed.

module BRA4.SbF3AtClosures where
...
-- stepFun and work the same for sbf3 as for sbt.
...
private
  stepBody_sbf3_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 stepBody_sbf3 input)
                (ap2 condFork
                  (ap1 (C pi atomic_branch_sbf3 neg_or_above) input)
                  (ap1 isEq input)))
  stepBody_sbf3_unfold input =
    ax_C condFork (C pi atomic_branch_sbf3 neg_or_above) isEq input

  pi_atomic_neg_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 (C pi atomic_branch_sbf3 neg_or_above) input)
                (ap2 pi (ap1 atomic_branch_sbf3 input) (ap1 neg_or_above input)))
  pi_atomic_neg_unfold input =
    ax_C pi atomic_branch_sbf3 neg_or_above input

  neg_or_above_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 neg_or_above input)
                (ap2 condFork
                  (ap1 (C pi neg_branch_sbf3 imp_or_else) input)
                  (ap1 isNeg input)))
  neg_or_above_unfold input =
    ax_C condFork (C pi neg_branch_sbf3 imp_or_else) isNeg input

  pi_neg_imp_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 (C pi neg_branch_sbf3 imp_or_else) input)
                (ap2 pi (ap1 neg_branch_sbf3 input) (ap1 imp_or_else input)))
  pi_neg_imp_unfold input =
    ax_C pi neg_branch_sbf3 imp_or_else input

  imp_or_else_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 imp_or_else input)
                (ap2 condFork
                  (ap1 (C pi imp_branch_sbf3 else_branch_sbf3) input)
                  (ap1 isImp input)))
  imp_or_else_unfold input =
    ax_C condFork (C pi imp_branch_sbf3 else_branch_sbf3) isImp input

  pi_imp_else_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 (C pi imp_branch_sbf3 else_branch_sbf3) input)
                (ap2 pi (ap1 imp_branch_sbf3 input) (ap1 else_branch_sbf3 input)))
  pi_imp_else_unfold input =
    ax_C pi imp_branch_sbf3 else_branch_sbf3 input
...
private
  stepBody_sbf3_to_atomic :
    (input : Term) ->
    Deriv (eqF (ap1 isEq input) (ap1 s O)) ->
    Deriv (eqF (ap1 stepBody_sbf3 input) (ap1 atomic_branch_sbf3 input))
  stepBody_sbf3_to_atomic input isEq_sO =
    let e1 :
          Deriv (eqF (ap1 stepBody_sbf3 input)
                      (ap2 condFork
                        (ap1 (C pi atomic_branch_sbf3 neg_or_above) input)
                        (ap1 isEq input)))
        e1 = stepBody_sbf3_unfold input
        ...
        isEq_subst :
          Deriv (eqF (ap2 condFork
                      ...
                      (ap2 condFork
                       ...
                       (ap1 s O)))
        isEq_subst =
          congR condFork (ap1 (C pi atomic_branch_sbf3 neg_or_above) input) isEq_sO

        condFork_to_Fst :
          Deriv (eqF (ap2 condFork
                       (ap1 (C pi atomic_branch_sbf3 neg_or_above) input) (ap1 s O))
                      (ap1 Fst (ap1 (C pi atomic_branch_sbf3 neg_or_above) input)))
        condFork_to_Fst =
          condFork_true_nc (ap1 (C pi atomic_branch_sbf3 neg_or_above) input) O
        ...
        Fst_pi :
          Deriv (eqF (ap1 Fst (ap2 pi (ap1 atomic_branch_sbf3 input) (ap1 neg_or_above input)))
                      (ap1 atomic_branch_sbf3 input))
        Fst_pi = axFst (ap1 atomic_branch_sbf3 input) (ap1 neg_or_above input)
    in ruleTrans e1
         (ruleTrans isEq_subst
           (ruleTrans condFork_to_Fst
             (ruleTrans (cong1 Fst pi_eq) Fst_pi)))

  stepBody_sbf3_to_neg_or_above :
    (input : Term) ->
    Deriv (eqF (ap1 isEq input) O) ->
    Deriv (eqF (ap1 stepBody_sbf3 input) (ap1 neg_or_above input))
  stepBody_sbf3_to_neg_or_above input isEq_O =
    let e1 :
          Deriv (eqF (ap1 stepBody_sbf3 input)
                      (ap2 condFork
                        (ap1 (C pi atomic_branch_sbf3 neg_or_above) input)
        ...
        isEq_subst :
          Deriv (eqF (ap2 condFork
                      ...
                      (ap2 condFork
                       ...
                       O))
        isEq_subst =
          congR condFork (ap1 (C pi atomic_branch_sbf3 neg_or_above) input) isEq_O

        condFork_to_Snd :
          Deriv (eqF (ap2 condFork
                       (ap1 (C pi atomic_branch_sbf3 neg_or_above) input) O)
                      (ap1 Snd (ap1 (C pi atomic_branch_sbf3 neg_or_above) input)))
        condFork_to_Snd =
          condFork_false (ap1 (C pi atomic_branch_sbf3 neg_or_above) input)
        ...
        Snd_pi :
          Deriv (eqF (ap1 Snd (ap2 pi (ap1 atomic_branch_sbf3 input) (ap1 neg_or_above input)))
                      (ap1 neg_or_above input))
        Snd_pi = axSnd (ap1 atomic_branch_sbf3 input) (ap1 neg_or_above input)
    in ruleTrans e1
         (ruleTrans isEq_subst
  ...
  neg_or_above_to_neg :
    (input : Term) ->
    Deriv (eqF (ap1 isNeg input) (ap1 s O)) ->
    Deriv (eqF (ap1 neg_or_above input) (ap1 neg_branch_sbf3 input))
  neg_or_above_to_neg input isNeg_sO =
    let e1 :
        ...
        isNeg_subst :
          Deriv (eqF (ap2 condFork
                      ...
                      (ap2 condFork
                       ...
                       (ap1 s O)))
        isNeg_subst =
          congR condFork (ap1 (C pi neg_branch_sbf3 imp_or_else) input) isNeg_sO

        condFork_to_Fst :
          Deriv (eqF (ap2 condFork
                       (ap1 (C pi neg_branch_sbf3 imp_or_else) input) (ap1 s O))
                      (ap1 Fst (ap1 (C pi neg_branch_sbf3 imp_or_else) input)))
        condFork_to_Fst =
          condFork_true_nc (ap1 (C pi neg_branch_sbf3 imp_or_else) input) O
        ...
        Fst_pi :
          Deriv (eqF (ap1 Fst (ap2 pi (ap1 neg_branch_sbf3 input) (ap1 imp_or_else input)))
                      (ap1 neg_branch_sbf3 input))
        Fst_pi = axFst (ap1 neg_branch_sbf3 input) (ap1 imp_or_else input)
    in ruleTrans e1
  ...
  neg_or_above_to_imp :
    ...
    Deriv (eqF (ap1 isNeg input) O) ->
    Deriv (eqF (ap1 isImp input) (ap1 s O)) ->
    Deriv (eqF (ap1 neg_or_above input) (ap1 imp_branch_sbf3 input))
  neg_or_above_to_imp input isNeg_O isImp_sO =
    let e1 :
        ...
        isNeg_subst :
          Deriv (eqF (ap2 condFork
                      ...
                      (ap2 condFork
                       ...
                       O))
        isNeg_subst =
          congR condFork (ap1 (C pi neg_branch_sbf3 imp_or_else) input) isNeg_O

        condFork_to_Snd :
          Deriv (eqF (ap2 condFork
                       (ap1 (C pi neg_branch_sbf3 imp_or_else) input) O)
                      (ap1 Snd (ap1 (C pi neg_branch_sbf3 imp_or_else) input)))
        condFork_to_Snd =
          condFork_false (ap1 (C pi neg_branch_sbf3 imp_or_else) input)
        ...
        Snd_pi :
          Deriv (eqF (ap1 Snd (ap2 pi (ap1 neg_branch_sbf3 input) (ap1 imp_or_else input)))
                      (ap1 imp_or_else input))
        Snd_pi = axSnd (ap1 neg_branch_sbf3 input) (ap1 imp_or_else input)
        ...
        isImp_subst :
          Deriv (eqF (ap2 condFork
                      ...
                      (ap2 condFork
                       ...
                       (ap1 s O)))
        isImp_subst =
          congR condFork (ap1 (C pi imp_branch_sbf3 else_branch_sbf3) input) isImp_sO

        condFork_to_Fst_imp :
          Deriv (eqF (ap2 condFork
                       (ap1 (C pi imp_branch_sbf3 else_branch_sbf3) input) (ap1 s O))
                      (ap1 Fst (ap1 (C pi imp_branch_sbf3 else_branch_sbf3) input)))
        condFork_to_Fst_imp =
          condFork_true_nc (ap1 (C pi imp_branch_sbf3 else_branch_sbf3) input) O
        ...
        Fst_pi_imp :
          Deriv (eqF (ap1 Fst (ap2 pi (ap1 imp_branch_sbf3 input) (ap1 else_branch_sbf3 input)))
                      (ap1 imp_branch_sbf3 input))
        Fst_pi_imp = axFst (ap1 imp_branch_sbf3 input) (ap1 else_branch_sbf3 input)

        imp_or_else_to_imp :
          Deriv (eqF (ap1 imp_or_else input) (ap1 imp_branch_sbf3 input))
        imp_or_else_to_imp =
          ruleTrans e2
...
private
  neg_branch_sbf3_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 neg_branch_sbf3 input)
                (ap2 pi (ap1 (constN tag_neg) input) (ap1 (lookupAt get_body) input)))
  neg_branch_sbf3_unfold input =
    ax_C pi (constN tag_neg) (lookupAt get_body) input

  atomic_branch_sbf3_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 atomic_branch_sbf3 input)
                (ap2 pi (ap1 (constN tag_eq) input)
                        (ap1 (C pi (C sbt3 get_spec get_ca_atom)
                                    (C sbt3 get_spec get_cb_atom)) input)))
  atomic_branch_sbf3_unfold input =
    ax_C pi (constN tag_eq)
            (C pi (C sbt3 get_spec get_ca_atom) (C sbt3 get_spec get_cb_atom)) input

  atomic_pair_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 (C pi (C sbt3 get_spec get_ca_atom) (C sbt3 get_spec get_cb_atom)) input)
                (ap2 pi (ap1 (C sbt3 get_spec get_ca_atom) input)
                        (ap1 (C sbt3 get_spec get_cb_atom) input)))
  atomic_pair_unfold input =
    ax_C pi (C sbt3 get_spec get_ca_atom) (C sbt3 get_spec get_cb_atom) input

  C_sbt_ca_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 (C sbt3 get_spec get_ca_atom) input)
                (ap2 sbt3 (ap1 get_spec input) (ap1 get_ca_atom input)))
  C_sbt_ca_unfold input = ax_C sbt3 get_spec get_ca_atom input

  C_sbt_cb_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 (C sbt3 get_spec get_cb_atom) input)
                (ap2 sbt3 (ap1 get_spec input) (ap1 get_cb_atom input)))
  C_sbt_cb_unfold input = ax_C sbt3 get_spec get_cb_atom input

  imp_branch_sbf3_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 imp_branch_sbf3 input)
                (ap2 pi (ap1 (constN tag_imp) input)
                        (ap1 (C pi (lookupAt get_cP_imp) (lookupAt get_cQ_imp)) input)))
  imp_branch_sbf3_unfold input =
...
private
  HP_sbf_eq_sbf_under_leq :
    (spec ct K : Term) ->
    Deriv (leq ct K) ->
    Deriv (eqF (HPsbt baseValue_sbf3 stepFun_sbf3 spec ct K) (ap2 sbf3 spec ct))
  HP_sbf_eq_sbf_under_leq spec ct K leq_ct_K =
    let stab :
          Deriv (eqF (HPsbt baseValue_sbf3 stepFun_sbf3 spec ct K)
                      (HPsbt baseValue_sbf3 stepFun_sbf3 spec ct ct))
        stab = mp (stabilityP_sbt_at baseValue_sbf3 stepFun_sbf3 spec ct K) leq_ct_K

        subCT_O : Deriv (eqF (ap2 sub ct ct) O)
        subCT_O = sub_self ct

        iter_arg :
          Deriv (eqF (ap2 (iter Snd) (HistP_sbt baseValue_sbf3 stepFun_sbf3 spec ct) (ap2 sub ct ct))
                      (ap2 (iter Snd) (HistP_sbt baseValue_sbf3 stepFun_sbf3 spec ct) O))
        iter_arg = congR (iter Snd) (HistP_sbt baseValue_sbf3 stepFun_sbf3 spec ct) subCT_O

        iter_base :
          Deriv (eqF (ap2 (iter Snd) (HistP_sbt baseValue_sbf3 stepFun_sbf3 spec ct) O)
                      (HistP_sbt baseValue_sbf3 stepFun_sbf3 spec ct))
        iter_base = iter_base_univ Snd (HistP_sbt baseValue_sbf3 stepFun_sbf3 spec ct)
        ...
        HP_at_ct :
          Deriv (eqF (HPsbt baseValue_sbf3 stepFun_sbf3 spec ct ct)
                      (ap1 Fst (HistP_sbt baseValue_sbf3 stepFun_sbf3 spec ct)))
        HP_at_ct = cong1 Fst iter_full

        readOff_eq :
          Deriv (eqF (ap1 readOff_spec (ap2 (cov_spec baseValue_sbf3 stepFun_sbf3) spec ct))
                      (ap1 Fst (HistP_sbt baseValue_sbf3 stepFun_sbf3 spec ct)))
        readOff_eq = readOff_spec_eq (ap2 (cov_spec baseValue_sbf3 stepFun_sbf3) spec ct)

        readOff_eq_sym :
          Deriv (eqF (ap1 Fst (HistP_sbt baseValue_sbf3 stepFun_sbf3 spec ct))
                      (ap1 readOff_spec (ap2 (cov_spec baseValue_sbf3 stepFun_sbf3) spec ct)))
        readOff_eq_sym = ruleSym readOff_eq

        sbf_eq :
          Deriv (eqF (ap2 sbf3 spec ct) (ap1 readOff_spec (ap2 sbf3State spec ct)))
        sbf_eq = sbf3_unfold spec ct

        sbf_eq_sym :
          Deriv (eqF (ap1 readOff_spec (ap2 sbf3State spec ct)) (ap2 sbf3 spec ct))
...
------------------------------------------------------------------------
-- sbf3_at_neg :  the SbContract closure for neg.

sbf3_at_neg :
  (k1 k2 k3 : Nat) (S1 S2 S3 cP : Term) ->
  Deriv (eqF (ap2 sbf3 (ap2 pi (ap2 pi (natCode k1) S1)
                                (ap2 pi (ap2 pi (natCode k2) S2)
                                        (ap2 pi (natCode k3) S3)))
               (ap2 pi (natCode tag_neg) cP))
              (ap2 pi (natCode tag_neg)
                (ap2 sbf3 (ap2 pi (ap2 pi (natCode k1) S1)
                                   (ap2 pi (ap2 pi (natCode k2) S2)
                                           (ap2 pi (natCode k3) S3))) cP)))
sbf3_at_neg k1 k2 k3 S1 S2 S3 cP =
  let spec : Term
      spec = ap2 pi (ap2 pi (natCode k1) S1)
                    (ap2 pi (ap2 pi (natCode k2) S2)
                            (ap2 pi (natCode k3) S3))
      ...
      prev : Term
      prev = ap2 (cov_spec baseValue_sbf3 stepFun_sbf3) spec P_outer

      input_pkg' : Term
      input_pkg' = ap2 pi P_outer (ap1 Snd prev)

      step1 :
        Deriv (eqF (ap2 sbf3 spec input) (ap1 readOff_spec (ap2 sbf3State spec input)))
      step1 = sbf3_unfold spec input
      ...
      cov_lift :
        Deriv (eqF (ap2 (cov_spec baseValue_sbf3 stepFun_sbf3) spec input)
                    (ap2 (cov_spec baseValue_sbf3 stepFun_sbf3) spec (ap1 s P_outer)))
      cov_lift = congR (cov_spec baseValue_sbf3 stepFun_sbf3) spec input_eq_sP_outer

      cov_step :
        Deriv (eqF (ap2 (cov_spec baseValue_sbf3 stepFun_sbf3) spec (ap1 s P_outer))
                    (ap1 (state_step_spec stepFun_sbf3) prev))
      cov_step = cov_spec_step_univ baseValue_sbf3 stepFun_sbf3 spec P_outer

      sbf3State_eq :
        Deriv (eqF (ap2 sbf3State spec input) (ap1 (state_step_spec stepFun_sbf3) prev))
      sbf3State_eq = ruleTrans cov_lift cov_step

      readOff_lift :
        Deriv (eqF (ap1 readOff_spec (ap2 sbf3State spec input))
                    (ap1 readOff_spec (ap1 (state_step_spec stepFun_sbf3) prev)))
      readOff_lift = cong1 readOff_spec sbf3State_eq

      readOff_eval :
        Deriv (eqF (ap1 readOff_spec (ap1 (state_step_spec stepFun_sbf3) prev))
                    (ap2 stepFun_sbf3 (ap1 Fst prev) (ap1 Snd prev)))
      readOff_eval = readOff_state_step_univ stepFun_sbf3 prev

      Fst_prev_eq :
        Deriv (eqF (ap1 Fst prev) P_outer)
      Fst_prev_eq = fst_cov_spec_eq baseValue_sbf3 stepFun_sbf3 spec P_outer

      stepFun_lift :
        Deriv (eqF (ap2 stepFun_sbf3 (ap1 Fst prev) (ap1 Snd prev))
                    (ap2 stepFun_sbf3 P_outer (ap1 Snd prev)))
      stepFun_lift = congL stepFun_sbf3 (ap1 Snd prev) Fst_prev_eq

      Post_eq :
        Deriv (eqF (ap2 stepFun_sbf3 P_outer (ap1 Snd prev))
                    (ap1 stepBody_sbf3 (ap2 pi P_outer (ap1 Snd prev))))
      Post_eq = axPost stepBody_sbf3 pi P_outer (ap1 Snd prev)
      ...
      stepBody_to_neg_or :
        Deriv (eqF (ap1 stepBody_sbf3 input_pkg') (ap1 neg_or_above input_pkg'))
      stepBody_to_neg_or = stepBody_sbf3_to_neg_or_above input_pkg' isEq_value

      negor_to_neg :
        Deriv (eqF (ap1 neg_or_above input_pkg') (ap1 neg_branch_sbf3 input_pkg'))
      negor_to_neg = neg_or_above_to_neg input_pkg' isNeg_value

      stepBody_to_neg :
        Deriv (eqF (ap1 stepBody_sbf3 input_pkg') (ap1 neg_branch_sbf3 input_pkg'))
      stepBody_to_neg = ruleTrans stepBody_to_neg_or negor_to_neg

      -- neg_branch unfolding.
      negbr_unfold :
        Deriv (eqF (ap1 neg_branch_sbf3 input_pkg')
                    (ap2 pi (ap1 (constN tag_neg) input_pkg')
                            (ap1 (lookupAt get_body) input_pkg')))
      negbr_unfold = neg_branch_sbf3_unfold input_pkg'
      ...
      -- lookupAt get_body input_pkg' = HPsbf spec cP P_outer = sbf3 spec cP.
      ...
      get_table_value :
        Deriv (eqF (ap1 get_table input_pkg') (HistP_sbt baseValue_sbf3 stepFun_sbf3 spec P_outer))
      ...
      iter_full :
        Deriv (eqF (ap2 (iter Snd) (ap1 get_table input_pkg')
                          (ap2 sub (ap1 get_K input_pkg') (ap1 get_body input_pkg')))
                    (ap2 (iter Snd) (HistP_sbt baseValue_sbf3 stepFun_sbf3 spec P_outer)
                          (ap2 sub P_outer cP)))
      iter_full = ruleTrans (congL (iter Snd) (ap2 sub (ap1 get_K input_pkg') (ap1 get_body input_pkg')) get_table_value)
                   (congR (iter Snd) (HistP_sbt baseValue_sbf3 stepFun_sbf3 spec P_outer) iter_arg)

      val_cP_unfold :
        Deriv (eqF (ap1 (lookupAt get_body) input_pkg')
                    (ap1 Fst (ap2 (iter Snd) (HistP_sbt baseValue_sbf3 stepFun_sbf3 spec P_outer)
                              (ap2 sub P_outer cP))))
      ...
      HP_to_sbf :
        Deriv (eqF (HPsbt baseValue_sbf3 stepFun_sbf3 spec cP P_outer) (ap2 sbf3 spec cP))
      HP_to_sbf = HP_sbf_eq_sbf_under_leq spec cP P_outer leq_cP_P

      val_cP_value :
        Deriv (eqF (ap1 (lookupAt get_body) input_pkg') (ap2 sbf3 spec cP))
      val_cP_value = ruleTrans val_cP_unfold HP_to_sbf

      -- Assemble pi (natCode tag_neg) (sbf3 spec cP).
      negbr_value :
        Deriv (eqF (ap1 neg_branch_sbf3 input_pkg')
                    (ap2 pi (natCode tag_neg) (ap2 sbf3 spec cP)))
      negbr_value =
        ruleTrans negbr_step1 (congR pi (natCode tag_neg) val_cP_value)

      chain_to_stepBody :
        Deriv (eqF (ap2 sbf3 spec input) (ap1 stepBody_sbf3 input_pkg'))
      chain_to_stepBody =
        ruleTrans step1
      ...
      chain_to_negbr :
        Deriv (eqF (ap2 sbf3 spec input) (ap1 neg_branch_sbf3 input_pkg'))
      chain_to_negbr = ruleTrans chain_to_stepBody stepBody_to_neg
  in ruleTrans chain_to_negbr negbr_value

------------------------------------------------------------------------
-- sbf3_at_imp :  the SbContract closure for imp.
--
  ...
  Deriv (eqF (ap2 sbf3 (ap2 pi (ap2 pi (natCode k1) S1)
              ...
              (ap2 pi (natCode tag_imp)
                (ap2 pi
                  ...
                  (ap2 sbf3 (ap2 pi (ap2 pi (natCode k1) S1)
                                     (ap2 pi (ap2 pi (natCode k2) S2)
                                             (ap2 pi (natCode k3) S3))) cQ))))
sbf3_at_imp k1 k2 k3 S1 S2 S3 cP cQ =
  let spec : Term
      spec = ap2 pi (ap2 pi (natCode k1) S1)
                    (ap2 pi (ap2 pi (natCode k2) S2)
                            (ap2 pi (natCode k3) S3))
      ...
      iter_full_cP = ruleTrans (congL (iter Snd) (ap2 sub (ap1 get_K input_pkg') (ap1 get_cP_imp input_pkg')) get_table_value)
                      (congR (iter Snd) (HistP_sbt baseValue_sbf3 stepFun_sbf3 spec P_outer) iter_arg_cP)

      val_cP_to_HP :
        Deriv (eqF (ap1 (lookupAt get_cP_imp) input_pkg')
                    (HPsbt baseValue_sbf3 stepFun_sbf3 spec cP P_outer))
      ...
      val_cP_value :
        Deriv (eqF (ap1 (lookupAt get_cP_imp) input_pkg') (ap2 sbf3 spec cP))
      ...
      iter_full_cQ = ruleTrans (congL (iter Snd) (ap2 sub (ap1 get_K input_pkg') (ap1 get_cQ_imp input_pkg')) get_table_value)
                      (congR (iter Snd) (HistP_sbt baseValue_sbf3 stepFun_sbf3 spec P_outer) iter_arg_cQ)

      val_cQ_to_HP :
        Deriv (eqF (ap1 (lookupAt get_cQ_imp) input_pkg')
                    (HPsbt baseValue_sbf3 stepFun_sbf3 spec cQ P_outer))
      ...
      val_cQ_value :
        Deriv (eqF (ap1 (lookupAt get_cQ_imp) input_pkg') (ap2 sbf3 spec cQ))
      val_cQ_value = ruleTrans val_cQ_to_HP
                       (HP_sbf_eq_sbf_under_leq spec cQ P_outer leq_cQ_P)

      -- Assemble pi (sbf3 spec cP) (sbf3 spec cQ).
      pair_val_value :
        Deriv (eqF (ap1 (C pi (lookupAt get_cP_imp) (lookupAt get_cQ_imp)) input_pkg')
                    (ap2 pi (ap2 sbf3 spec cP) (ap2 sbf3 spec cQ)))
      pair_val_value =
        ruleTrans inner_C_unfold
          (ruleTrans (congL pi (ap1 (lookupAt get_cQ_imp) input_pkg') val_cP_value)
                      (congR pi (ap2 sbf3 spec cP) val_cQ_value))
      ...
      chain_to_impbr :
        Deriv (eqF (ap2 sbf3 spec input) (ap1 imp_branch_sbf3 input_pkg'))
      chain_to_impbr = ruleTrans chain_to_stepBody stepBody_to_imp
  in ruleTrans chain_to_impbr impbr_value

------------------------------------------------------------------------
-- sbf3_at_atomic :  the SbContract closure for atomic.
--
-- Direct sbt3 invocation in sub-positions via  C sbt3 get_spec get_ca_atom .
...
sbf3_at_atomic :
  ...
  Deriv (eqF (ap2 sbf3 (ap2 pi (ap2 pi (natCode k1) S1)
              ...
              (ap2 pi (natCode tag_eq)
                (ap2 pi
                  (ap2 sbt3 (ap2 pi (ap2 pi (natCode k1) S1)
                                     (ap2 pi (ap2 pi (natCode k2) S2)
                                             (ap2 pi (natCode k3) S3))) ca)
                  (ap2 sbt3 (ap2 pi (ap2 pi (natCode k1) S1)
                                     (ap2 pi (ap2 pi (natCode k2) S2)
                                             (ap2 pi (natCode k3) S3))) cb))))
sbf3_at_atomic k1 k2 k3 S1 S2 S3 ca cb =
  let spec : Term
      spec = ap2 pi (ap2 pi (natCode k1) S1)
                    (ap2 pi (ap2 pi (natCode k2) S2)
                            (ap2 pi (natCode k3) S3))
      ...
      stepBody_to_atom :
        Deriv (eqF (ap1 stepBody_sbf3 input_pkg') (ap1 atomic_branch_sbf3 input_pkg'))
      stepBody_to_atom = stepBody_sbf3_to_atomic input_pkg' isEq_value

      -- atomic_branch unfolding.
      atombr_unfold :
        Deriv (eqF (ap1 atomic_branch_sbf3 input_pkg')
                    (ap2 pi (ap1 (constN tag_eq) input_pkg')
                            (ap1 (C pi (C sbt3 get_spec get_ca_atom)
                                       (C sbt3 get_spec get_cb_atom)) input_pkg')))
      atombr_unfold = atomic_branch_sbf3_unfold input_pkg'
      ...
      atombr_step1 = ruleTrans atombr_unfold
                       (congL pi (ap1 (C pi (C sbt3 get_spec get_ca_atom)
                                                (C sbt3 get_spec get_cb_atom)) input_pkg') constN_at)

      inner_C_unfold :
        Deriv (eqF (ap1 (C pi (C sbt3 get_spec get_ca_atom)
                              (C sbt3 get_spec get_cb_atom)) input_pkg')
                    (ap2 pi (ap1 (C sbt3 get_spec get_ca_atom) input_pkg')
                            (ap1 (C sbt3 get_spec get_cb_atom) input_pkg')))
      inner_C_unfold = atomic_pair_unfold input_pkg'

      -- C sbt3 get_spec get_ca_atom input_pkg' = sbt3 (get_spec input_pkg') (get_ca_atom input_pkg').
      sbt_ca_unfold :
        Deriv (eqF (ap1 (C sbt3 get_spec get_ca_atom) input_pkg')
                    (ap2 sbt3 (ap1 get_spec input_pkg') (ap1 get_ca_atom input_pkg')))
      sbt_ca_unfold = C_sbt_ca_unfold input_pkg'

      sbt_cb_unfold :
        Deriv (eqF (ap1 (C sbt3 get_spec get_cb_atom) input_pkg')
                    (ap2 sbt3 (ap1 get_spec input_pkg') (ap1 get_cb_atom input_pkg')))
      ...
      fstSnd_prev = fstSnd_cov_spec_eq baseValue_sbf3 stepFun_sbf3 spec P_outer
      ...
      -- sbt3 (get_spec ipkg) (get_ca_atom ipkg) = sbt3 spec ca.
      sbt_ca_value :
        Deriv (eqF (ap1 (C sbt3 get_spec get_ca_atom) input_pkg') (ap2 sbt3 spec ca))
      sbt_ca_value =
        ruleTrans sbt_ca_unfold
          (ruleTrans (congL sbt3 (ap1 get_ca_atom input_pkg') get_spec_value)
                      (congR sbt3 spec get_ca_value))

      sbt_cb_value :
        Deriv (eqF (ap1 (C sbt3 get_spec get_cb_atom) input_pkg') (ap2 sbt3 spec cb))
      sbt_cb_value =
        ruleTrans sbt_cb_unfold
          (ruleTrans (congL sbt3 (ap1 get_cb_atom input_pkg') get_spec_value)
                      (congR sbt3 spec get_cb_value))

      pair_val_value :
        Deriv (eqF (ap1 (C pi (C sbt3 get_spec get_ca_atom)
                              (C sbt3 get_spec get_cb_atom)) input_pkg')
                    (ap2 pi (ap2 sbt3 spec ca) (ap2 sbt3 spec cb)))
      pair_val_value =
        ruleTrans inner_C_unfold
          (ruleTrans (congL pi (ap1 (C sbt3 get_spec get_cb_atom) input_pkg') sbt_ca_value)
                      (congR pi (ap2 sbt3 spec ca) sbt_cb_value))
      ...
      chain_to_atombr :
        Deriv (eqF (ap2 sbf3 spec input) (ap1 atomic_branch_sbf3 input_pkg'))
