...
-- BRA4.SbT2AtAp -- discharge of the SbContract sbt closures at the two
-- ap-shape inputs:
--
--   sbt2_at_ap1 :
--     (k : Nat) (S : Term) (f : Fun1) (ct : Term) ->
--     ap2 sbt2 (ap2 pi (natCode k) S)
--             (ap2 pi (natCode tag_ap1) (ap2 pi (codeFun1 f) ct))
--       =  ap2 pi (natCode tag_ap1)
--                (ap2 pi (codeFun1 f) (ap2 sbt2 (ap2 pi (natCode k) S) ct))
--
--   sbt2_at_ap2 :  (analogous, with two recursive lookups)
--
...
-- stepFun_sbt2 dispatch + stabilityP_sbt_at to bridge the table lookup
-- to sbt2 spec ct.

module BRA4.SbT2AtAp where
...
------------------------------------------------------------------------
-- Specialized stepFun_sbt2 unfolding lemmas at packaged input.

private
  stepBody_sbt2_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 stepBody_sbt2 input)
                (ap2 condFork
                  (ap1 (C pi var_branch ap1_or_above) input)
                  (ap1 isVar input)))
  stepBody_sbt2_unfold input =
...
------------------------------------------------------------------------
-- Leq monotonicity at the specific shape used by sbt2_at_ap1 / sbt2_at_ap2.
...
------------------------------------------------------------------------
-- Stability bridge: under  leq ct K , the table-lookup value equals  sbt2 spec ct .
--
--   HP_sbt base step spec ct K = sbt2 spec ct
-- under  leq ct K .
--
-- Direct chain:
...
--                        = sbt2 spec ct                                  [sbt2_unfold reverse]

private
  HP_sbt_eq_sbt_under_leq :
    (spec ct K : Term) ->
    Deriv (leq ct K) ->
    Deriv (eqF (HPsbt baseValue_sbt2 stepFun_sbt2 spec ct K) (ap2 sbt2 spec ct))
  HP_sbt_eq_sbt_under_leq spec ct K leq_ct_K =
    let -- Step A: stability lemma.
        stab :
          Deriv (eqF (HPsbt baseValue_sbt2 stepFun_sbt2 spec ct K)
                      (HPsbt baseValue_sbt2 stepFun_sbt2 spec ct ct))
        stab = mp (stabilityP_sbt_at baseValue_sbt2 stepFun_sbt2 spec ct K) leq_ct_K
        ...
        iter_arg :
          Deriv (eqF (ap2 (iter Snd) (HistP_sbt baseValue_sbt2 stepFun_sbt2 spec ct) (ap2 sub ct ct))
                      (ap2 (iter Snd) (HistP_sbt baseValue_sbt2 stepFun_sbt2 spec ct) O))
        iter_arg = congR (iter Snd) (HistP_sbt baseValue_sbt2 stepFun_sbt2 spec ct) subCT_O

        iter_base :
          Deriv (eqF (ap2 (iter Snd) (HistP_sbt baseValue_sbt2 stepFun_sbt2 spec ct) O)
                      (HistP_sbt baseValue_sbt2 stepFun_sbt2 spec ct))
        iter_base = iter_base_univ Snd (HistP_sbt baseValue_sbt2 stepFun_sbt2 spec ct)
        ...
        HP_at_ct :
          Deriv (eqF (HPsbt baseValue_sbt2 stepFun_sbt2 spec ct ct)
                      (ap1 Fst (HistP_sbt baseValue_sbt2 stepFun_sbt2 spec ct)))
        ...
        readOff_eq :
          Deriv (eqF (ap1 readOff_spec (ap2 (cov_spec baseValue_sbt2 stepFun_sbt2) spec ct))
                      (ap1 Fst (HistP_sbt baseValue_sbt2 stepFun_sbt2 spec ct)))
        readOff_eq = readOff_spec_eq (ap2 (cov_spec baseValue_sbt2 stepFun_sbt2) spec ct)

        readOff_eq_sym :
          Deriv (eqF (ap1 Fst (HistP_sbt baseValue_sbt2 stepFun_sbt2 spec ct))
                      (ap1 readOff_spec (ap2 (cov_spec baseValue_sbt2 stepFun_sbt2) spec ct)))
        readOff_eq_sym = ruleSym readOff_eq

        -- Step D: ap1 readOff_spec (cov_spec spec ct) = ap2 sbt2 spec ct.
        sbt_eq :
          Deriv (eqF (ap2 sbt2 spec ct) (ap1 readOff_spec (ap2 sbt2State spec ct)))
        sbt_eq = sbt2_unfold spec ct

        sbt_eq_sym :
          Deriv (eqF (ap1 readOff_spec (ap2 sbt2State spec ct)) (ap2 sbt2 spec ct))
        sbt_eq_sym = ruleSym sbt_eq

    in ruleTrans stab (ruleTrans HP_at_ct (ruleTrans readOff_eq_sym sbt_eq_sym))

------------------------------------------------------------------------
-- The core firing lemma for sbt2_at_ap1.
--
--   stepFun_sbt22_at_ap1_core :
--     (spec ct : Term) (f : Fun1) ->
--     Deriv (eqF (ap1 Fst Y) spec) ->                  -- spec invariant
--     Deriv (eqF (ap2 stepFun_sbt2 P_outer Y)
--                 (ap2 pi (natCode tag_ap1)
--                       (ap2 pi (codeFun1 f) (sbt2 spec ct))))
--
...
-- sbt2 spec ct directly using HP_sbt_eq_sbt_under_leq.  The hypothesis
-- "Fst Y = spec" is used to dispatch through get_specFst etc.

-- For convenience, sbt2_at_ap1 is built directly without an intermediate
...
------------------------------------------------------------------------
-- Dispatch through stepBody_sbt2's outer condFork.

private
  stepBody_sbt2_to_ap1_or_above :
    (input : Term) ->
    Deriv (eqF (ap1 isVar input) O) ->
    Deriv (eqF (ap1 stepBody_sbt2 input) (ap1 ap1_or_above input))
  stepBody_sbt2_to_ap1_or_above input isVar_O =
    let e1 :
          Deriv (eqF (ap1 stepBody_sbt2 input)
                      (ap2 condFork
                        (ap1 (C pi var_branch ap1_or_above) input)
                        (ap1 isVar input)))
        e1 = stepBody_sbt2_unfold input
...
------------------------------------------------------------------------
-- sbt2_at_ap1 :  the SbContract closure for ap1.
--
--   ap2 sbt2 (pi (natCode k) S) (pi (natCode tag_ap1) (pi (codeFun1 f) ct))
...
sbt2_at_ap1 :
  ...
  Deriv (eqF (ap2 sbt2 (ap2 pi (ap2 pi (natCode k1) S1) (ap2 pi (natCode k2) S2))
               (ap2 pi (natCode tag_ap1) (ap2 pi (codeFun1 f) ct)))
              (ap2 pi (natCode tag_ap1)
                (ap2 pi (codeFun1 f)
                  (ap2 sbt2 (ap2 pi (ap2 pi (natCode k1) S1) (ap2 pi (natCode k2) S2)) ct))))
sbt2_at_ap1 k1 k2 S1 S2 f ct =
  let spec : Term
      ...
      prev : Term
      prev = ap2 (cov_spec baseValue_sbt2 stepFun_sbt2) spec P_outer
      ...
      -- Step 1: sbt2_unfold.
      step1 :
        Deriv (eqF (ap2 sbt2 spec input) (ap1 readOff_spec (ap2 sbt2State spec input)))
      step1 = sbt2_unfold spec input
      ...
      cov_lift :
        Deriv (eqF (ap2 (cov_spec baseValue_sbt2 stepFun_sbt2) spec input)
                    (ap2 (cov_spec baseValue_sbt2 stepFun_sbt2) spec (ap1 s P_outer)))
      cov_lift = congR (cov_spec baseValue_sbt2 stepFun_sbt2) spec input_eq_sP_outer

      -- cov_spec spec (s P_outer) = state_step_spec stepFun_sbt2 prev.
      cov_step :
        Deriv (eqF (ap2 (cov_spec baseValue_sbt2 stepFun_sbt2) spec (ap1 s P_outer))
                    (ap1 (state_step_spec stepFun_sbt2) prev))
      cov_step = cov_spec_step_univ baseValue_sbt2 stepFun_sbt2 spec P_outer

      sbt2State_eq :
        Deriv (eqF (ap2 sbt2State spec input) (ap1 (state_step_spec stepFun_sbt2) prev))
      sbt2State_eq = ruleTrans cov_lift cov_step

      ----------------------------------------------------------------
      -- Step 3: readOff_state_step_univ gives stepFun_sbt2 (Fst prev) (Snd prev).
      readOff_lift :
        Deriv (eqF (ap1 readOff_spec (ap2 sbt2State spec input))
                    (ap1 readOff_spec (ap1 (state_step_spec stepFun_sbt2) prev)))
      readOff_lift = cong1 readOff_spec sbt2State_eq

      readOff_eval :
        Deriv (eqF (ap1 readOff_spec (ap1 (state_step_spec stepFun_sbt2) prev))
                    (ap2 stepFun_sbt2 (ap1 Fst prev) (ap1 Snd prev)))
      readOff_eval = readOff_state_step_univ stepFun_sbt2 prev

      ----------------------------------------------------------------
      -- Step 4: bridge Fst prev = P_outer via fst_cov_spec_eq.
      Fst_prev_eq :
        Deriv (eqF (ap1 Fst prev) P_outer)
      Fst_prev_eq = fst_cov_spec_eq baseValue_sbt2 stepFun_sbt2 spec P_outer

      stepFun_lift :
        Deriv (eqF (ap2 stepFun_sbt2 (ap1 Fst prev) (ap1 Snd prev))
                    (ap2 stepFun_sbt2 P_outer (ap1 Snd prev)))
      stepFun_lift = congL stepFun_sbt2 (ap1 Snd prev) Fst_prev_eq

      ----------------------------------------------------------------
      -- Step 5: stepFun_sbt2 = Post stepBody_sbt2 pi.  So
      --   stepFun_sbt2 P_outer (Snd prev) = stepBody_sbt2 (pi P_outer (Snd prev)).
      Post_eq :
        Deriv (eqF (ap2 stepFun_sbt2 P_outer (ap1 Snd prev))
                    (ap1 stepBody_sbt2 (ap2 pi P_outer (ap1 Snd prev))))
      Post_eq = axPost stepBody_sbt2 pi P_outer (ap1 Snd prev)
      ...
      stepBody_to_ap1or :
        Deriv (eqF (ap1 stepBody_sbt2 input_pkg') (ap1 ap1_or_above input_pkg'))
      stepBody_to_ap1or = stepBody_sbt2_to_ap1_or_above input_pkg' isVar_value
      ...
      -- Combine:  stepBody_sbt2 input_pkg' = ap1_branch input_pkg'.
      stepBody_to_ap1 :
        Deriv (eqF (ap1 stepBody_sbt2 input_pkg') (ap1 ap1_branch input_pkg'))
      ...
      --   = pi (natCode tag_ap1) (pi (codeFun1 f) (sbt2 spec ct))
      ...
      ----------------------------------------------------------------
      -- Step 11: get_val_ct_ap1 input_pkg' = sbt2 spec ct (under leq ct P_outer).
      ...
      -- get_table input_pkg' = Snd (Snd prev) = HistP_sbt baseValue_sbt2 stepFun_sbt2 spec P_outer.
      get_table_value :
        Deriv (eqF (ap1 get_table input_pkg') (HistP_sbt baseValue_sbt2 stepFun_sbt2 spec P_outer))
      ...
      iter_full :
        Deriv (eqF (ap2 (iter Snd) (ap1 get_table input_pkg')
                          (ap2 sub (ap1 get_K input_pkg') (ap1 get_ct_ap1 input_pkg')))
                    (ap2 (iter Snd) (HistP_sbt baseValue_sbt2 stepFun_sbt2 spec P_outer)
                          (ap2 sub P_outer ct)))
      iter_full = ruleTrans (congL (iter Snd) (ap2 sub (ap1 get_K input_pkg') (ap1 get_ct_ap1 input_pkg')) get_table_value)
                   (congR (iter Snd) (HistP_sbt baseValue_sbt2 stepFun_sbt2 spec P_outer) iter_arg)

      -- Wrap with Fst: get HP_sbt form.
      val_ct_unfold :
        Deriv (eqF (ap1 get_val_ct_ap1 input_pkg')
                    (ap1 Fst (ap2 (iter Snd) (HistP_sbt baseValue_sbt2 stepFun_sbt2 spec P_outer)
                              (ap2 sub P_outer ct))))
      val_ct_unfold = ruleTrans lookup_unfold (cong1 Fst iter_full)
      -- The RHS is HPsbt baseValue_sbt2 stepFun_sbt2 spec ct P_outer.

      -- Step 11b: leq ct P_outer.
      leq_ct_P : Deriv (leq ct P_outer)
      leq_ct_P = leq_ct_P_outer_ap1 f ct

      -- Step 11c: HP_sbt → sbt2 spec ct.
      HP_to_sbt :
        Deriv (eqF (HPsbt baseValue_sbt2 stepFun_sbt2 spec ct P_outer) (ap2 sbt2 spec ct))
      HP_to_sbt = HP_sbt_eq_sbt_under_leq spec ct P_outer leq_ct_P

      val_ct_value :
        Deriv (eqF (ap1 get_val_ct_ap1 input_pkg') (ap2 sbt2 spec ct))
      ...
      chain_to_stepBody :
        Deriv (eqF (ap2 sbt2 spec input)
                    (ap1 stepBody_sbt2 input_pkg'))
      chain_to_stepBody =
        ruleTrans step1
      ...
      chain_to_ap1br :
        Deriv (eqF (ap2 sbt2 spec input) (ap1 ap1_branch input_pkg'))
      chain_to_ap1br = ruleTrans chain_to_stepBody stepBody_to_ap1
  in ruleTrans chain_to_ap1br ap1br_value

------------------------------------------------------------------------
-- sbt2_at_ap2 :  the SbContract closure for ap2.
--
-- Same skeleton as sbt2_at_ap1, but with tag_ap2 instead of tag_ap1, and
...
sbt2_at_ap2 k1 k2 S1 S2 g ca cb =
  let spec : Term
      spec = ap2 pi (ap2 pi (natCode k1) S1) (ap2 pi (natCode k2) S2)
      ...
      stepBody_to_ap2 :
        Deriv (eqF (ap1 stepBody_sbt2 input_pkg') (ap1 ap2_branch input_pkg'))
      ...
      ----------------------------------------------------------------
      -- get_val_ca input_pkg' = sbt2 spec ca (under leq ca P_outer).
      -- get_val_cb input_pkg' = sbt2 spec cb (under leq cb P_outer).
      ...
      iter_full_ca = ruleTrans (congL (iter Snd) (ap2 sub (ap1 get_K input_pkg') (ap1 get_ca_ap2 input_pkg')) get_table_value)
                      (congR (iter Snd) (HistP_sbt baseValue_sbt2 stepFun_sbt2 spec P_outer) iter_arg_ca)

      val_ca_to_HP :
        Deriv (eqF (ap1 get_val_ca input_pkg')
                    (HPsbt baseValue_sbt2 stepFun_sbt2 spec ca P_outer))
      ...
      val_ca_value :
        Deriv (eqF (ap1 get_val_ca input_pkg') (ap2 sbt2 spec ca))
      ...
      iter_full_cb = ruleTrans (congL (iter Snd) (ap2 sub (ap1 get_K input_pkg') (ap1 get_cb_ap2 input_pkg')) get_table_value)
                      (congR (iter Snd) (HistP_sbt baseValue_sbt2 stepFun_sbt2 spec P_outer) iter_arg_cb)

      val_cb_to_HP :
        Deriv (eqF (ap1 get_val_cb input_pkg')
                    (HPsbt baseValue_sbt2 stepFun_sbt2 spec cb P_outer))
      ...
      val_cb_value :
        Deriv (eqF (ap1 get_val_cb input_pkg') (ap2 sbt2 spec cb))
      ...
      pair_val_value =
        ruleTrans inner_C_unfold
          (ruleTrans (congL pi (ap1 get_val_cb input_pkg') val_ca_value)
                      (congR pi (ap2 sbt2 spec ca) val_cb_value))

      -- Assemble pi (codeFun2 g) (pi val_ca val_cb).
      inner_value :
        Deriv (eqF (ap1 (C pi get_bodyFst_ap2 (C pi get_val_ca get_val_cb)) input_pkg')
                    (ap2 pi (codeFun2 g) (ap2 pi (ap2 sbt2 spec ca) (ap2 sbt2 spec cb))))
      inner_value =
        ruleTrans inner_unfold
      ...
      chain_to_ap2br :
        Deriv (eqF (ap2 sbt2 spec input) (ap1 ap2_branch input_pkg'))
