...
-- T4.ChaitinG1DischargeKdefAlph -- the  checkAlphN -guard analog of
-- T4.ChaitinG1DischargeKdef .   Re-pointed from  hitKdef Lstar (outKdef
-- Lstar) / Kcode Lstar / predFlipDef Lstar  to the Alph recogniser
-- ( hitKdefAlph outKdefAlph / KcodeAlph / predFlipDefAlph ).  Bodies generic.

open import T4.Base

module T4.ChaitinG1DischargeKdefAlph (Lstar_meta : Nat) where

open import T4.ThmT            using ( thmT )
open import T4.KdefRecogAlph Lstar_meta
  using ( hitKdefAlph ; hitKdefAlph_le_one ; hitKdefAlph_fires
        ; dNeg_from_hitKdefAlph ; outKdefAlph )
...
------------------------------------------------------------------------
-- The Alph-shape Discharge module.

module DischargeKdefAlph
  (w        : Term)
  (x        : Term)
  ...
  ----------------------------------------------------------------------
  -- 5.  dNeg in num-raw KcodeAlph form, at the read-off subject  x' .

  x' : Term
  x' = ap1 outKdefAlph k_max

  dNeg_at_kmax : Deriv (eqF (ap1 thmT k_max) (ap1 KcodeAlph x'))
  dNeg_at_kmax = dNeg_from_hitKdefAlph outKdefAlph k_max isHit_recog
