...
-- T4.StepU2 -- Layer 1 of the two-layer CK-machine: the operational
-- semantics for Fun1 (no mu).
--
-- ARCHITECTURAL NOTE.  The Fun1 step-combinator `step` IS the universal
-- step combinator `stepU` from T4.EvalUStep.  Although `stepU`
...
-- the mu body.  Layer-1 completeness for Fun1 (T4.StepU2Correct) is
...
-- Term-fuel Reaches relation in T4.StepU2Correct.
--
...
open import T4.EvalUStep public
  using ( stepU
...
open import T4.Base using ( Fun1 )
