...
-- T4.ChaitinG1ChainKdefImp -- Carneiro-lifted (imp Rf) variant of
-- T4.ChaitinG1ChainKdef.
--
-- The original ChainKdef chain is:
--   seg1 ; seg2_mu (= runs_mu) ; seg3 ; seg4 ; seg5 ; seg6 ; seg7 ; seg8
-- only  seg2_mu  is h-dependent (it consumes  isHit / missSucc / ...
-- from DischargeKdef, all derived from the h-dep firing witness).
--
-- For the imp-lifted variant we take  imp_runs_mu  as an explicit input
-- (a Carneiro-lifted variant of  runs_mu  -- to be supplied either by
-- the future  ImpConstruct  in T4.StepU2MuCorrectImp or, in the
-- interim, by manually building it for specific k_max instances).
--
-- All other segments are closed and threaded through via impLift.
--
-- Output:
--   imp_dEval_witness :
--     Deriv (imp Rf (eqF (ap2 evalU (ap1 parse (enc (gLcodeDef Lstar))) nTerm)
--                         (ap1 s (ap1 (outKdef Lstar) k_max))))

module T4.ChaitinG1ChainKdefImp where

open import T4.Base
open import T4.ThmT            using ( thmT )
open import T4.Kdef            using ( Kcode )
open import T4.KdefRecog       using ( outKdef )
open import T4.KdefDiag        using ( gLcodeDef ; gCodeOfDef ; dRT_gLDef )
...
import T4.ChaitinG1DischargeKdefImp
...
module ChainKdefImp
  ...
  open T4.ChaitinG1DischargeKdefImp.DischargeKdefImp
         Rf w x hyp_imp sub0_w sub1_w sim_w
    using ( k_max ; gFun ; imp_dNeg_at_kmax )

  -- The h-dependent fuelMu_fun is computed inside Construct (from the
  -- h-dep params).  For the imp-lifted chain it must also be supplied
  -- alongside  imp_runs_mu .   We expose it abstractly.
  ...
  module Chain
    ...
    --------------------------------------------------------------------
    -- Per-segment derivations.  All but  seg2_mu  are CLOSED (no h).
    ...
    --------------------------------------------------------------------
    -- Closed iter_step1 / compStep (these don't depend on  hyp ).
    ...
    --------------------------------------------------------------------
    -- imp-lifted compStep variant:  e1 is  imp Rf -wrapped, e2 closed.
    ...
    --------------------------------------------------------------------
    -- imp-lifted compStep variant:  e1 closed, e2 is  imp Rf -wrapped.
    ...
    --------------------------------------------------------------------
    -- Build the fuel sigma-pile and the imp-lifted run chain.
    ...
    --------------------------------------------------------------------
    -- Readout + initF + parse round-trip.
    ...
    imp_evalU_at_gL :
      Deriv (imp Rf (eqF (ap2 evalU (gLcodeDef Lstar) fuelN)
                          (ap1 s (ap1 outL k_max))))
    imp_evalU_at_gL =
      let unfold : Deriv (eqF (ap2 evalU (gLcodeDef Lstar) fuelN)
                              (ap1 readout (ap2 (iter step)
                                              (ap1 initF (gLcodeDef Lstar))
                                              fuelN)))
          unfold = evalU_unfold (gLcodeDef Lstar) fuelN
          iterRw : Deriv (eqF (ap2 (iter step) (ap1 initF (gLcodeDef Lstar))
                                                fuelN)
          ...
          closed_pre :
            Deriv (eqF (ap2 evalU (gLcodeDef Lstar) fuelN)
                       (ap1 readout (ap2 (iter step) c0 fuelN)))
          closed_pre = ruleTrans unfold readRw
      in impEqTrans (ap2 evalU (gLcodeDef Lstar) fuelN)
           (ap1 readout (ap2 (iter step) c0 fuelN))
           (ap1 s (ap1 outL k_max))
           (impLift {Rf} closed_pre) imp_readout_chain

    imp_evalU_at_parse :
      Deriv (imp Rf (eqF (ap2 evalU (ap1 parse (enc (gLcodeDef Lstar))) fuelN)
                          (ap1 s (ap1 outL k_max))))
    imp_evalU_at_parse =
      let parseEq : Deriv (eqF (ap1 parse (enc (gLcodeDef Lstar))) (gLcodeDef Lstar))
          parseEq = dRT_gLDef Lstar
          evalRw : Deriv (eqF (ap2 evalU (ap1 parse (enc (gLcodeDef Lstar))) fuelN)
                              (ap2 evalU (gLcodeDef Lstar) fuelN))
          evalRw = congL evalU fuelN parseEq
      in impEqTrans (ap2 evalU (ap1 parse (enc (gLcodeDef Lstar))) fuelN)
           (ap2 evalU (gLcodeDef Lstar) fuelN) (ap1 s (ap1 outL k_max))
