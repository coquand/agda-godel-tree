...
-- T4.KOut -- Phase E2 (CHAITIN-G1-STANDARD-DIRECTION.md SS5): the search
-- PROJECTOR  out_L  that reads the subject  z0  back off a matched proof.
-- The standard-route analog of T4.ChaitinG1Out, simpler: the K-formula
-- skeleton (T4.KFormula) is a PURE right-spine (every constant on the
...
open import T4.KFormula  using ( Kgt ; kgtConsts )
open import T4.NegAtomCode using ( NVList ; nvnil ; nvcons ; skelOf )
