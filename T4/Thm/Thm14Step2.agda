...
-- T4.Thm.Thm14Step2 -- Step 2 of Guard's Theorem 14 proof , EXACTLY
-- matching guard15.pdf p.17 line 2) :
--
--     th( Dsub(i,i) )  =  "sub(i,i) = j"   ,    by Th 13 .
--
-- (Guard writes "sub(i,i) = j" ; the underline-on-j is implicit per
--  Definition 12 + the Thm 13 typo correction noted in T4.Thm12.Thm13 ;
--  the actual encoded form has  num j  =  j_  on the RHS slot .)
--
-- T4 form .  UNCONDITIONAL .  With  i  =  natCode (codeFormulaNat H) ,
-- j = natCode (codeFormulaNat G)  ( T4.Thm.Thm14F ) :
--
...
module T4.Thm.Thm14Step2 where
