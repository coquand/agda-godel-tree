...
-- T4.Thm.Thm11 -- Goedel's First Incompleteness Theorem in the
-- T4 Pair-encoding, parametric on:
--
--   sb   : Fun2  satisfying  SbContract   sb     (T4.SbContract)
--   prov : Fun1  satisfying  ProvContract prov   (T4.Provability)
--
-- The numeral coding  num : Fun1  is the concrete one shipped in
-- T4.Num  (T4.NumContract.numContract), so it is NOT a module
-- parameter.
--
...
module T4.Thm.Thm11 where

open import T4.Base
open import T4.Code
open import T4.Num using ( num )
open import T4.NumContract using ( numContract )
open import T4.NatBridge using ( codeFormulaNat ; codeFormula_eq )
open import T4.SbContract using ( SbContract ; module SbContract )
open import T4.Provability using ( provFormula ; ProvContract ; module ProvContract )
open import T4.ExFalso using ( exFalso )
open import T4.Diagonal using ( module Diagonal )
