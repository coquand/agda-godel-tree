...
-- T4.Thm.Thm14Step5 -- Step 5 of Guard's Theorem 14 proof in T4 ,
-- EXACTLY matching guard15.pdf p.17 line 5) :
--
--     th(x) = j   ⊃   th(big_term(x))  =  "0 = 1"   ,    by mp on 3) , 4) , 5a) .
--
-- T4 form (non-parametric ;  F  fixed in T4.Thm.Thm14F ).
-- Universal in  x : Term .
--
--     step5 :  (x : Term) ->
--              Deriv (imp (P_x x)
...
--  T4.Thm12.EncodedMp.imp_encoded_mp  (Carneiro-lifted) .
--
...
module T4.Thm.Thm14Step5 where
...
open import T4.Thm.Thm14Step3    using ( g ; step3 )
open import T4.Thm.Thm14Step4    using ( K_part ; step4 )
open import T4.Thm.Thm14HPart
...
--    with T4 convention "Pair tag_mp (Pair impl ant)" :
