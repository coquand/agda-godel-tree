...
-- T4.SurpriseG2.DischargeKdefConj -- conj-shape Discharge for the
-- new CGI_core (per T4/NEXT-SESSION-CGICONJ-BODY.md).   Mechanical
-- parallel of  T4.ChaitinG1DischargeKdef  at the new shape :
--
--   Kcode Lstar           ->  KcodeConj M enum
--   hitKdef Lstar         ->  hitKdefConj M enum
--   outKdef Lstar         ->  outKdefConj M enum
--   predFlipDef Lstar     ->  predFlipDefConj M enum
--
-- The proof skeletons are IDENTICAL ; only the parametric Fun1 changes .
--
-- KEY trick (same as the OLD): we open the  Construct  module of
-- T4.StepU2MuCorrect  with  gFun := predFlipDefConj M enum  ;  the
-- abstract shielding inside StepU2MuCorrect keeps Agda from walking
-- the deep Fun1 structure of  gFun .

module T4.SurpriseG2.DischargeKdefConj where

open import T4.Base
open import T4.ThmT            using ( thmT )
open import T4.SurpriseG2.KdefRecogConj
  using ( hitKdefConj ; hitKdefConj_le_one ; hitKdefConj_fires
        ; dNeg_from_hitKdefConj ; outKdefConj )
open import T4.SurpriseG2.KcodeConj using ( KcodeConj )
open import T4.SurpriseG2.KdefDiagConj using ( predFlipDefConj )
...
------------------------------------------------------------------------
-- The conj-shape Discharge module .

module DischargeKdefConj
  (M        : Nat)
  (enum     : Fun1)
  (w        : Term)
  (x        : Term)
  (h        : Deriv (eqF (ap1 thmT w) (ap1 (KcodeConj M enum) x)))
  -- Substitution-stability of  w  at the two var positions used by
  -- StepU2MuCorrect.Construct (var 0, var 1) .   Replaces a stronger
  -- Closed w  hypothesis ;  T4.CloseW.cl_w_sub0/sub1/sim discharge
  ...
  ----------------------------------------------------------------------
  -- 3.  isHit / missSucc -- identical bodies to the OLD Kdef-shape
  -- Discharge ,  since they only depend on  p_recog 's 0/1 behaviour ,
  -- not on the recogniser's internals .
  ...
  ----------------------------------------------------------------------
  -- 5.  dNeg in num-raw KcodeConj form , at the read-off subject  x' .

  x' : Term
  x' = ap1 (outKdefConj M enum) k_max

  dNeg_at_kmax : Deriv (eqF (ap1 thmT k_max) (ap1 (KcodeConj M enum) x'))
  dNeg_at_kmax = dNeg_from_hitKdefConj M enum (outKdefConj M enum) k_max isHit_recog
