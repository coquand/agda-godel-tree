...
-- T4.SurpriseG2.ChainKdefConj -- conj-shape Chain (gLcodeDefConj-based) ,
-- parallel to  T4.ChaitinG1ChainKdef  (per T4/NEXT-SESSION-CGICONJ-BODY.md) .
--
...
--   * stepU_at_evU       (u(O) = O) .
--   * stepU_at_rtApp2    (frmApp2 pops, evaluate g = Lift1 outKdefConj at pi k_max O) .
--   * runs2 (Lift1 outKdefConj) k_max O konEmpty -- yields outKdefConj k_max .
--   * stepU_at_rtEmpty   (konEmpty pops, halt) .
--
-- and bridges to evalU (parse (enc (gLcodeDefConj M enum))) nTerm
-- via initF_eq + evalU_unfold + dRT_gLDefConj .

module T4.SurpriseG2.ChainKdefConj where

open import T4.Base
open import T4.ThmT            using ( thmT )
open import T4.SurpriseG2.KcodeConj   using ( KcodeConj )
open import T4.SurpriseG2.KdefRecogConj using ( outKdefConj )
open import T4.SurpriseG2.KdefDiagConj
  using ( gLcodeDefConj ; gCodeOfDefConj ; dRT_gLDefConj )
...
------------------------------------------------------------------------
-- The Chain module -- parametric in the same setup as DischargeKdefConj .
...
module ChainKdefConj
  ...
  open T4.SurpriseG2.DischargeKdefConj.DischargeKdefConj
         M enum w x h sub0_w sub1_w sim_w
  ...
  outL : Fun1
  outL = outKdefConj M enum

  g_outer_code : Term
  g_outer_code = gCodeOfDefConj M enum       -- = mcode2 (Lift1 outL)

  h1c : Term
  h1c = mcodeMu
          (mcode1
            (T4.SurpriseG2.DischargeKdefConj.DischargeKdefConj.gFun
              M enum w x h sub0_w sub1_w sim_w))
  ...
  c0  = cfgEV (gLcodeDefConj M enum) O konEmpty
  c1  = cfgEV h1c O K1
  c2  = cfgRT k_max K1
  ...
  initF_bridge : Deriv (eqF (ap1 initF (gLcodeDefConj M enum)) c0)
  initF_bridge = initF_eq (gLcodeDefConj M enum)

  evalU_at_gL :
    Deriv (eqF (ap2 evalU (gLcodeDefConj M enum) fuelN)
                (ap1 s (ap1 outL k_max)))
  evalU_at_gL =
    let unfold = evalU_unfold (gLcodeDefConj M enum) fuelN
        iterRw = congL (iter step) fuelN initF_bridge
        readRw = cong1 readout iterRw
    in ruleTrans unfold (ruleTrans readRw readout_chain)

  evalU_at_parse :
    Deriv (eqF (ap2 evalU (ap1 parse (enc (gLcodeDefConj M enum))) fuelN)
                (ap1 s (ap1 outL k_max)))
  evalU_at_parse =
    let parseEq = dRT_gLDefConj M enum
  ...
  dEval_witness :
    Deriv (eqF (ap2 evalU (ap1 parse (enc (gLcodeDefConj M enum))) nTerm)
                (ap1 s (ap1 (outKdefConj M enum) k_max)))
