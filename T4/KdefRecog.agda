...
-- T4.KdefRecog -- CGI num-raw step (b): the recogniser  hitKdef , the subject
-- projector  outKdef , and the firing bridge  dNeg_from_hitKdef .  The num-raw
-- analogs of T4.KRecog / T4.KOut, re-pointed from  negKgtCodeOf / Kgt  to
-- the open  Kcode / Kdef  (T4.Kdef).
--
...
--       -- directly (T4.Decode.decode_num_id_at).  Cleaner than KOut.out_L_correct.
--
...
module T4.KdefRecog where

open import T4.Base
open import T4.ThmT        using ( thmT )
open import T4.Decode      using ( decode ; decode_num_id_at )
open import T4.Num         using ( num )
open import T4.Kdef        using ( Kdef ; Kcode ; Kcode_eval ; kdefConsts ; kdefSkel )
...
-- Mirrors  T4.KFire.fireAtProof , but sourced from the provability fact
