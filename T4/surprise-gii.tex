...
$\vdash \thmT(\code{d}) = \code{A}$ whenever $d$ codes a derivation of
...
system. We write $\vdash A$ (or $\mathit{Deriv}(A)$) for ``$A$ is a
theorem of $T$''.
...
\[
  \boxed{\;\vdash \Con_T \;\Longrightarrow\; \vdash (\mathbf{O}=\mathbf{s}\,\mathbf{O}),\;}
\]
...
judgements, realised in Agda as a total function
$\mathit{Deriv}(\Con_T) \to \mathit{Deriv}(\mathbf{O}=\mathbf{s}\,\mathbf{O})$.
...
\[
  \vdash\;\Bigl(\thmT(w) = \code{\,\Kc(\mathbf{out}(w))>\Lstar\,}\Bigr)
        \;\imp\;
        \Bigl(\thmT(G\cdot w) = \code{\mathbf{O}=\mathbf{s}\,\mathbf{O}}\Bigr),
  \tag{C}
\]
...
$\vdash (m \ge i)$ for $i = 1, 2, \dots$. The induction step
$\vdash(m\ge i)\Rightarrow\vdash(m\ge i{+}1)$ is exactly where Chaitin's
...
\[
  \vdash\;\Kc(\mathbf{x}_0; p_{r+1},\dots,p_N) \imp \bigl(\Kc(r)>\Lstar\bigr).
\]
...
$\vdash \bigl(\thmT(w)=\code{\Kc(r)>\Lstar}\bigr)
   \imp \bigl(\thmT(G\cdot w)=\code{\mathbf{O}=\mathbf{s}\,\mathbf{O}}\bigr)$.
Composing Steps~3--5 yields
$\vdash K_r(\mathbf{x}_0)=\mathbf{O}
   \imp \bigl(\thmT(h\cdot\mathbf{x}_0)=\code{\mathbf{O}=\mathbf{s}\,\mathbf{O}}\bigr)$.
...
$\mathit{Deriv}(S(r)) \to \mathit{Deriv}(S(r+1))$ and iterates it from
