...
-- T4.McodeInAlph -- Phase E4, step R4: every program code  mcode f  lies in
-- the  {O, ap1 s, ap2 pi}  fragment  InAlph  (T4.ProgParse), so the decoder
...
-- T4.KGodel1 / T4.KDiag.  Note: an application like  inAlph_mcode1 thmT  is
...
module T4.McodeInAlph where

open import T4.Base
open import T4.Tags  using ( tag_s ; tag_o ; tag_u ; tag_C ; tag_v ; tag_R )
