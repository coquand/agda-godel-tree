...
-- T4.KClash -- Phase E3 (CHAITIN-G1-STANDARD-DIRECTION.md SS6): the CLASH.
...
open import T4.KFormula using ( szLeqApp ; Kgt )
...
--   e := gLcode (var 0) = the NAME, n := nTerm (var 1)  produces  neg (the
-- closed conjunction) .  The program slot is now  ap1 parse (var 0)  (the
-- universal machine decodes the name), reducing to  ap1 parse gLcode ; the
-- subject  zTerm  and fuel  nTerm  are Closed, so their substitutions are
-- no-ops (closedAt).
...
kgtInstEq :
  ...
  Eq (substF 0 gLcode (substF 1 nTerm (Kgt L zTerm)))
     (neg (fAnd (eqF (szLeqApp L gLcode) (ap1 s O))
                (eqF (ap2 evalU (ap1 parse gLcode) nTerm) (ap1 s zTerm))))
kgtInstEq L gLcode nTerm zTerm clN clZ =
  let eN : Eq (substT 0 gLcode nTerm) nTerm
      ...
      qqBridge :
        Eq (eqF (ap2 evalU (ap1 parse gLcode) (substT 0 gLcode nTerm))
                (ap1 s (substT 0 gLcode (substT 1 nTerm zTerm))))
           (eqF (ap2 evalU (ap1 parse gLcode) nTerm) (ap1 s zTerm))
      qqBridge =
        eqTrans
          (eqCong (\ t -> eqF (ap2 evalU (ap1 parse gLcode) t)
                              (ap1 s (substT 0 gLcode (substT 1 nTerm zTerm)))) eN)
          (eqCong (\ t -> eqF (ap2 evalU (ap1 parse gLcode) nTerm) (ap1 s t)) eZ)
...
kr_clash :
  ...
  Deriv (eqF (ap2 evalU (ap1 parse gLcode) nTerm) (ap1 s zTerm)) ->     -- dEval (halts; program = parse(name))
