...
-- T4.ChaitinG1DischargeKdefImpN -- the number-code re-pointing of
-- T4.ChaitinG1DischargeKdefImp : the Carneiro-lifted (imp Rf) Discharge over the
-- honest p<N / runProgN recogniser.  Verbatim mirror, Kdef* -> Kdef*N , L -> predN.

open import T4.Base

module T4.ChaitinG1DischargeKdefImpN (predN : Term) where

open import T4.ThmT            using ( thmT )
open import T4.KdefRecogN predN using ( hitKdefN ; hitKdefN_le_one ; outKdefN )
open import T4.KdefRecogImpN predN using ( imp_hitKdefN_fires ; imp_dNeg_from_hitKdefN )
...
module DischargeKdefImpN
  ...
  imp_fired = imp_hitKdefN_fires Rf w x hyp_imp
  ...
  ----------------------------------------------------------------------
  -- k_max + substitution-stability witnesses ( CLOSED in Rf ).
  ...
  ----------------------------------------------------------------------
  -- imp_isHit_recog / imp_isHit / imp_missSucc / isFirst_recog .
  ...
  imp_isHit =
    let e1 : Deriv (eqF (ap1 gFun k_max) (ap1 isZero (ap1 p_recog k_max)))
        ...
        step1 : Deriv (imp Rf (eqF (ap1 gFun k_max)
                              (ap1 isZero (ap1 p_recog k_max))))
        step1 = impLift {Rf} e1
        step2 : Deriv (imp Rf (eqF (ap1 gFun k_max) (ap1 isZero (ap1 s O))))
  ...
  imp_dNeg_at_kmax :
    Deriv (imp Rf (eqF (ap1 thmT k_max) (ap1 KcodeN x')))
  imp_dNeg_at_kmax =
    imp_dNeg_from_hitKdefN Rf outKdefN k_max imp_isHit_recog
