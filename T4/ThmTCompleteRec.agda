...
-- T4.ThmTCompleteRec -- the unified, structurally-recursive
-- completeness theorem.  Composes the 17 per-constructor lemmas
-- of T4.ThmTCompleteAll .  No  Closed -witness premise :  every
-- per-axiom lemma is now Closed-free  (the multi-var axiom cases
-- use BRA3.RuleInst2.three_step_F  /  T4.RuleInst3.five_step_F
-- internally to bridge the sb-wrap cascade ; see
--  T4.ThmTCompleteAxClosed  and  T4.ThmTCompleteAxFunctor ).
--
--   thmT_complete_rec :
--     {P : Formula} (d : Deriv P) ->
--     Deriv (eqF (ap1 thmT (encode d)) (codeFormula P))

module T4.ThmTCompleteRec where
...
open import T4.ThmTCompleteAll
