...
-- T4.ChaitinG1ChainKdefConj -- BLOCK 4, part 4: the Chain step at the two-slot
-- conjunction K-shape.  A mechanical port of  T4.ChaitinG1ChainKdef , re-pointed
-- from  outKdef Lstar / gLcodeDef Lstar  to  outKdefConj enum N / gLcodeConj enum N
-- (T4.KdefDiagConj) and opening  DischargeKdefConj  instead of  DischargeKdef .
--
-- The 8 step segments (stepU_at_*, runs2, correct2, iter helpers) are GENERIC in
-- the diagonal program and the output projector, so the whole body is verbatim.
-- Output:  nTerm (the fuel) and  dEval_witness : evalU(parse(enc gLcodeConj), nTerm)
--          = s (outKdefConj N k_max) .

module T4.ChaitinG1ChainKdefConj where

open import T4.Base
open import T4.ThmT            using ( thmT )
open import T4.KdefConjRecog   using ( KcodeConj ; outKdefConj )
open import T4.KdefDiagConj    using ( gLcodeConj ; gCodeOfConj ; dRT_gLConj )
...
import T4.ChaitinG1DischargeKdefConj
...
------------------------------------------------------------------------
-- The conjunction-shape Chain module -- same setup as DischargeKdefConj.

module ChainKdefConj
  (enum     : Fun1)
  (N        : Nat)
  (w        : Term)
  (x        : Term)
  (h        : Deriv (eqF (ap1 thmT w) (ap1 (KcodeConj enum N) x)))
  ...
  open T4.ChaitinG1DischargeKdefConj.DischargeKdefConj enum N w x h sub0_w sub1_w sim_w
  ...
  outL : Fun1
  outL = outKdefConj enum N

  g_outer_code : Term
  g_outer_code = gCodeOfConj enum N      -- = mcode2 (Lift1 outL)

  h1c : Term
  h1c = mcodeMu (mcode1 (T4.ChaitinG1DischargeKdefConj.DischargeKdefConj.gFun
                          enum N w x h sub0_w sub1_w sim_w))
  ...
  c0  = cfgEV (gLcodeConj enum N) O konEmpty
  c1  = cfgEV h1c O K1
  c2  = cfgRT k_max K1
  ...
  initF_bridge : Deriv (eqF (ap1 initF (gLcodeConj enum N)) c0)
  initF_bridge = initF_eq (gLcodeConj enum N)

  evalU_at_gL :
    Deriv (eqF (ap2 evalU (gLcodeConj enum N) fuelN) (ap1 s (ap1 outL k_max)))
  evalU_at_gL =
    let unfold = evalU_unfold (gLcodeConj enum N) fuelN
        iterRw = congL (iter step) fuelN initF_bridge
        readRw = cong1 readout iterRw
    in ruleTrans unfold (ruleTrans readRw readout_chain)

  evalU_at_parse :
    Deriv (eqF (ap2 evalU (ap1 parse (enc (gLcodeConj enum N))) fuelN)
                (ap1 s (ap1 outL k_max)))
  evalU_at_parse =
    let parseEq = dRT_gLConj enum N
  ...
  dEval_witness :
    Deriv (eqF (ap2 evalU (ap1 parse (enc (gLcodeConj enum N))) nTerm)
                (ap1 s (ap1 (outKdefConj enum N) k_max)))
