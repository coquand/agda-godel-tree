...
-- T4.CoVSpecSpec -- the spec-preservation invariant for cov_spec.
--
...
-- the structure of  T4.CoVSpecFst.fst_cov_spec_eq , but proves the
-- spec-projection invariant (Fst Snd) instead of the counter-projection
-- invariant (Fst).
--
-- Used by  T4.SbF  to bridge  get_spec input_pkg = spec  inside
-- stepFun_sbf 's atomic branch.

module T4.CoVSpecSpec where
