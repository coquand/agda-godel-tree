...
-- T4.KdefDiag -- The diagonal program for the num-raw route.  Analog of
-- T4.KDiag, re-pointed from the OLD  hitK / out_L  to my num-raw  hitKdef /
-- outKdef  (T4.KdefRecog).  Establishes the program code  gLcodeDef L  whose
-- run is what  evalU(parse (enc gLcodeDef Lstar), n0) = s z  asserts.  This is
-- the substrate for the object-induction Mu (next).

module T4.KdefDiag where

open import T4.Base
open import T4.Tags        using ( tag_C )
open import T4.KdefRecog   using ( hitKdef ; outKdef )
