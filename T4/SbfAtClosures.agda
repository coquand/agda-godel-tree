...
-- T4.SbfAtClosures -- discharge of the three formula closures of
--  SbContract :
--
--   sbf_at_atomic :  ap2 sbf spec (pi (natCode tag_eq) (pi ca cb))
--                       = pi (natCode tag_eq) (pi (sbt spec ca) (sbt spec cb))
--   sbf_at_neg    :  ap2 sbf spec (pi (natCode tag_neg) cP)
--                       = pi (natCode tag_neg) (sbf spec cP)
--   sbf_at_imp    :  ap2 sbf spec (pi (natCode tag_imp) (pi cP cQ))
--                       = pi (natCode tag_imp) (pi (sbf spec cP) (sbf spec cQ))
--
-- Universal in the sub-positions  ca, cb, cP, cQ : Term -- no Closed
-- premise, no natCode hypothesis.
--
-- Architecture parallels  T4.SbtAtAp .  Tag dispatch + cov_spec_step_univ
-- + readOff + Post + stepBody_sbf cascade.  For tag_neg / tag_imp the
-- sub-positions are formulas and we use the sbf cov-table lookup +
-- stability bridge (HP_sbf_eq_sbf_under_leq).  For tag_eq the sub-positions
-- are TERMS and we use  C sbt get_spec get_ca_atom  + spec-preservation
-- (fstSnd_cov_spec_eq) -- no stability needed.

module T4.SbfAtClosures where
...
open import T4.CoVSpecSpec
open import T4.SbT          using ( sbt ; get_K ; get_inner ; get_spec
...
-- stepFun and work the same for sbf as for sbt.
...
private
  stepBody_sbf_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 stepBody_sbf input)
                (ap2 condFork
                  (ap1 (C pi atomic_branch_sbf neg_or_above) input)
                  (ap1 isEq input)))
  stepBody_sbf_unfold input =
    ax_C condFork (C pi atomic_branch_sbf neg_or_above) isEq input

  pi_atomic_neg_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 (C pi atomic_branch_sbf neg_or_above) input)
                (ap2 pi (ap1 atomic_branch_sbf input) (ap1 neg_or_above input)))
  pi_atomic_neg_unfold input =
    ax_C pi atomic_branch_sbf neg_or_above input

  neg_or_above_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 neg_or_above input)
                (ap2 condFork
                  (ap1 (C pi neg_branch_sbf imp_or_else) input)
                  (ap1 isNeg input)))
  neg_or_above_unfold input =
    ax_C condFork (C pi neg_branch_sbf imp_or_else) isNeg input

  pi_neg_imp_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 (C pi neg_branch_sbf imp_or_else) input)
                (ap2 pi (ap1 neg_branch_sbf input) (ap1 imp_or_else input)))
  pi_neg_imp_unfold input =
    ax_C pi neg_branch_sbf imp_or_else input

  imp_or_else_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 imp_or_else input)
                (ap2 condFork
                  (ap1 (C pi imp_branch_sbf else_branch_sbf) input)
                  (ap1 isImp input)))
  imp_or_else_unfold input =
    ax_C condFork (C pi imp_branch_sbf else_branch_sbf) isImp input

  pi_imp_else_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 (C pi imp_branch_sbf else_branch_sbf) input)
                (ap2 pi (ap1 imp_branch_sbf input) (ap1 else_branch_sbf input)))
  pi_imp_else_unfold input =
    ax_C pi imp_branch_sbf else_branch_sbf input
...
private
  stepBody_sbf_to_atomic :
    (input : Term) ->
    Deriv (eqF (ap1 isEq input) (ap1 s O)) ->
    Deriv (eqF (ap1 stepBody_sbf input) (ap1 atomic_branch_sbf input))
  stepBody_sbf_to_atomic input isEq_sO =
    let e1 :
          Deriv (eqF (ap1 stepBody_sbf input)
                      (ap2 condFork
                        (ap1 (C pi atomic_branch_sbf neg_or_above) input)
                        (ap1 isEq input)))
        e1 = stepBody_sbf_unfold input
        ...
        isEq_subst :
          Deriv (eqF (ap2 condFork
                      ...
                      (ap2 condFork
                       ...
                       (ap1 s O)))
        isEq_subst =
          congR condFork (ap1 (C pi atomic_branch_sbf neg_or_above) input) isEq_sO

        condFork_to_Fst :
          Deriv (eqF (ap2 condFork
                       (ap1 (C pi atomic_branch_sbf neg_or_above) input) (ap1 s O))
                      (ap1 Fst (ap1 (C pi atomic_branch_sbf neg_or_above) input)))
        condFork_to_Fst =
          condFork_true_nc (ap1 (C pi atomic_branch_sbf neg_or_above) input) O
        ...
        Fst_pi :
          Deriv (eqF (ap1 Fst (ap2 pi (ap1 atomic_branch_sbf input) (ap1 neg_or_above input)))
                      (ap1 atomic_branch_sbf input))
        Fst_pi = axFst (ap1 atomic_branch_sbf input) (ap1 neg_or_above input)
    in ruleTrans e1
         (ruleTrans isEq_subst
           (ruleTrans condFork_to_Fst
             (ruleTrans (cong1 Fst pi_eq) Fst_pi)))

  stepBody_sbf_to_neg_or_above :
    (input : Term) ->
    Deriv (eqF (ap1 isEq input) O) ->
    Deriv (eqF (ap1 stepBody_sbf input) (ap1 neg_or_above input))
  stepBody_sbf_to_neg_or_above input isEq_O =
    let e1 :
          Deriv (eqF (ap1 stepBody_sbf input)
                      (ap2 condFork
                        (ap1 (C pi atomic_branch_sbf neg_or_above) input)
        ...
        isEq_subst :
          Deriv (eqF (ap2 condFork
                      ...
                      (ap2 condFork
                       ...
                       O))
        isEq_subst =
          congR condFork (ap1 (C pi atomic_branch_sbf neg_or_above) input) isEq_O

        condFork_to_Snd :
          Deriv (eqF (ap2 condFork
                       (ap1 (C pi atomic_branch_sbf neg_or_above) input) O)
                      (ap1 Snd (ap1 (C pi atomic_branch_sbf neg_or_above) input)))
        condFork_to_Snd =
          condFork_false (ap1 (C pi atomic_branch_sbf neg_or_above) input)
        ...
        Snd_pi :
          Deriv (eqF (ap1 Snd (ap2 pi (ap1 atomic_branch_sbf input) (ap1 neg_or_above input)))
                      (ap1 neg_or_above input))
        Snd_pi = axSnd (ap1 atomic_branch_sbf input) (ap1 neg_or_above input)
    in ruleTrans e1
         (ruleTrans isEq_subst
  ...
  neg_or_above_to_neg :
    (input : Term) ->
    Deriv (eqF (ap1 isNeg input) (ap1 s O)) ->
    Deriv (eqF (ap1 neg_or_above input) (ap1 neg_branch_sbf input))
  neg_or_above_to_neg input isNeg_sO =
    let e1 :
        ...
        isNeg_subst :
          Deriv (eqF (ap2 condFork
                      ...
                      (ap2 condFork
                       ...
                       (ap1 s O)))
        isNeg_subst =
          congR condFork (ap1 (C pi neg_branch_sbf imp_or_else) input) isNeg_sO

        condFork_to_Fst :
          Deriv (eqF (ap2 condFork
                       (ap1 (C pi neg_branch_sbf imp_or_else) input) (ap1 s O))
                      (ap1 Fst (ap1 (C pi neg_branch_sbf imp_or_else) input)))
        condFork_to_Fst =
          condFork_true_nc (ap1 (C pi neg_branch_sbf imp_or_else) input) O
        ...
        Fst_pi :
          Deriv (eqF (ap1 Fst (ap2 pi (ap1 neg_branch_sbf input) (ap1 imp_or_else input)))
                      (ap1 neg_branch_sbf input))
        Fst_pi = axFst (ap1 neg_branch_sbf input) (ap1 imp_or_else input)
    in ruleTrans e1
  ...
  neg_or_above_to_imp :
    ...
    Deriv (eqF (ap1 isNeg input) O) ->
    Deriv (eqF (ap1 isImp input) (ap1 s O)) ->
    Deriv (eqF (ap1 neg_or_above input) (ap1 imp_branch_sbf input))
  neg_or_above_to_imp input isNeg_O isImp_sO =
    let e1 :
        ...
        condFork_to_Snd :
          Deriv (eqF (ap2 condFork
                       (ap1 (C pi neg_branch_sbf imp_or_else) input) O)
                      (ap1 Snd (ap1 (C pi neg_branch_sbf imp_or_else) input)))
        condFork_to_Snd =
        ...
        Snd_pi :
          Deriv (eqF (ap1 Snd (ap2 pi (ap1 neg_branch_sbf input) (ap1 imp_or_else input)))
        ...
        isImp_subst :
          Deriv (eqF (ap2 condFork
                      ...
                      (ap2 condFork
                       ...
                       (ap1 s O)))
        isImp_subst =
          congR condFork (ap1 (C pi imp_branch_sbf else_branch_sbf) input) isImp_sO

        condFork_to_Fst_imp :
          Deriv (eqF (ap2 condFork
                       (ap1 (C pi imp_branch_sbf else_branch_sbf) input) (ap1 s O))
                      (ap1 Fst (ap1 (C pi imp_branch_sbf else_branch_sbf) input)))
        condFork_to_Fst_imp =
          condFork_true_nc (ap1 (C pi imp_branch_sbf else_branch_sbf) input) O
        ...
        Fst_pi_imp :
          Deriv (eqF (ap1 Fst (ap2 pi (ap1 imp_branch_sbf input) (ap1 else_branch_sbf input)))
                      (ap1 imp_branch_sbf input))
        Fst_pi_imp = axFst (ap1 imp_branch_sbf input) (ap1 else_branch_sbf input)

        imp_or_else_to_imp :
          Deriv (eqF (ap1 imp_or_else input) (ap1 imp_branch_sbf input))
        imp_or_else_to_imp =
          ruleTrans e2
...
private
  neg_branch_sbf_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 neg_branch_sbf input)
                (ap2 pi (ap1 (constN tag_neg) input) (ap1 (lookupAt get_body) input)))
  neg_branch_sbf_unfold input =
    ax_C pi (constN tag_neg) (lookupAt get_body) input

  atomic_branch_sbf_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 atomic_branch_sbf input)
                (ap2 pi (ap1 (constN tag_eq) input)
                        (ap1 (C pi (C sbt get_spec get_ca_atom)
                                    (C sbt get_spec get_cb_atom)) input)))
  atomic_branch_sbf_unfold input =
    ax_C pi (constN tag_eq)
            (C pi (C sbt get_spec get_ca_atom) (C sbt get_spec get_cb_atom)) input

  atomic_pair_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 (C pi (C sbt get_spec get_ca_atom) (C sbt get_spec get_cb_atom)) input)
                (ap2 pi (ap1 (C sbt get_spec get_ca_atom) input)
                        (ap1 (C sbt get_spec get_cb_atom) input)))
  atomic_pair_unfold input =
    ax_C pi (C sbt get_spec get_ca_atom) (C sbt get_spec get_cb_atom) input

  C_sbt_ca_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 (C sbt get_spec get_ca_atom) input)
                (ap2 sbt (ap1 get_spec input) (ap1 get_ca_atom input)))
  C_sbt_ca_unfold input = ax_C sbt get_spec get_ca_atom input

  C_sbt_cb_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 (C sbt get_spec get_cb_atom) input)
                (ap2 sbt (ap1 get_spec input) (ap1 get_cb_atom input)))
  C_sbt_cb_unfold input = ax_C sbt get_spec get_cb_atom input

  imp_branch_sbf_unfold :
    (input : Term) ->
    Deriv (eqF (ap1 imp_branch_sbf input)
                (ap2 pi (ap1 (constN tag_imp) input)
                        (ap1 (C pi (lookupAt get_cP_imp) (lookupAt get_cQ_imp)) input)))
  imp_branch_sbf_unfold input =
...
private
  HP_sbf_eq_sbf_under_leq :
    (spec ct K : Term) ->
    Deriv (leq ct K) ->
    Deriv (eqF (HPsbt baseValue_sbf stepFun_sbf spec ct K) (ap2 sbf spec ct))
  HP_sbf_eq_sbf_under_leq spec ct K leq_ct_K =
    let stab :
          Deriv (eqF (HPsbt baseValue_sbf stepFun_sbf spec ct K)
                      (HPsbt baseValue_sbf stepFun_sbf spec ct ct))
        stab = mp (stabilityP_sbt_at baseValue_sbf stepFun_sbf spec ct K) leq_ct_K

        subCT_O : Deriv (eqF (ap2 sub ct ct) O)
        subCT_O = sub_self ct

        iter_arg :
          Deriv (eqF (ap2 (iter Snd) (HistP_sbt baseValue_sbf stepFun_sbf spec ct) (ap2 sub ct ct))
                      (ap2 (iter Snd) (HistP_sbt baseValue_sbf stepFun_sbf spec ct) O))
        iter_arg = congR (iter Snd) (HistP_sbt baseValue_sbf stepFun_sbf spec ct) subCT_O

        iter_base :
          Deriv (eqF (ap2 (iter Snd) (HistP_sbt baseValue_sbf stepFun_sbf spec ct) O)
                      (HistP_sbt baseValue_sbf stepFun_sbf spec ct))
        iter_base = iter_base_univ Snd (HistP_sbt baseValue_sbf stepFun_sbf spec ct)
        ...
        HP_at_ct :
          Deriv (eqF (HPsbt baseValue_sbf stepFun_sbf spec ct ct)
                      (ap1 Fst (HistP_sbt baseValue_sbf stepFun_sbf spec ct)))
        HP_at_ct = cong1 Fst iter_full

        readOff_eq :
          Deriv (eqF (ap1 readOff_spec (ap2 (cov_spec baseValue_sbf stepFun_sbf) spec ct))
                      (ap1 Fst (HistP_sbt baseValue_sbf stepFun_sbf spec ct)))
        readOff_eq = readOff_spec_eq (ap2 (cov_spec baseValue_sbf stepFun_sbf) spec ct)

        readOff_eq_sym :
          Deriv (eqF (ap1 Fst (HistP_sbt baseValue_sbf stepFun_sbf spec ct))
                      (ap1 readOff_spec (ap2 (cov_spec baseValue_sbf stepFun_sbf) spec ct)))
        readOff_eq_sym = ruleSym readOff_eq

        sbf_eq :
          Deriv (eqF (ap2 sbf spec ct) (ap1 readOff_spec (ap2 sbfState spec ct)))
        sbf_eq = sbf_unfold spec ct

        sbf_eq_sym :
          Deriv (eqF (ap1 readOff_spec (ap2 sbfState spec ct)) (ap2 sbf spec ct))
...
leq_cP_P_outer_imp cP cQ =
  let A : Term
      ...
      eqPi = ruleSym (T4.LeqMono.T114_at cP cQ)
...
leq_ca_P_outer_atom ca cb =
  let A : Term
      ...
      eqPi = ruleSym (T4.LeqMono.T114_at ca cb)
...
------------------------------------------------------------------------
-- sbf_at_neg :  the SbContract closure for neg.
...
sbf_at_neg k S cP =
  let spec : Term
      ...
      negor_to_neg :
        Deriv (eqF (ap1 neg_or_above input_pkg') (ap1 neg_branch_sbf input_pkg'))
      negor_to_neg = neg_or_above_to_neg input_pkg' isNeg_value

      stepBody_to_neg :
        Deriv (eqF (ap1 stepBody_sbf input_pkg') (ap1 neg_branch_sbf input_pkg'))
      stepBody_to_neg = ruleTrans stepBody_to_neg_or negor_to_neg

      -- neg_branch unfolding.
      negbr_unfold :
        Deriv (eqF (ap1 neg_branch_sbf input_pkg')
                    (ap2 pi (ap1 (constN tag_neg) input_pkg')
                            (ap1 (lookupAt get_body) input_pkg')))
      negbr_unfold = neg_branch_sbf_unfold input_pkg'
      ...
      -- lookupAt get_body input_pkg' = HPsbf spec cP P_outer = sbf spec cP.
      ...
      get_table_value :
        Deriv (eqF (ap1 get_table input_pkg') (HistP_sbt baseValue_sbf stepFun_sbf spec P_outer))
      ...
      iter_full :
        Deriv (eqF (ap2 (iter Snd) (ap1 get_table input_pkg')
                          (ap2 sub (ap1 get_K input_pkg') (ap1 get_body input_pkg')))
                    (ap2 (iter Snd) (HistP_sbt baseValue_sbf stepFun_sbf spec P_outer)
                          (ap2 sub P_outer cP)))
      iter_full = ruleTrans (congL (iter Snd) (ap2 sub (ap1 get_K input_pkg') (ap1 get_body input_pkg')) get_table_value)
                   (congR (iter Snd) (HistP_sbt baseValue_sbf stepFun_sbf spec P_outer) iter_arg)

      val_cP_unfold :
        Deriv (eqF (ap1 (lookupAt get_body) input_pkg')
                    (ap1 Fst (ap2 (iter Snd) (HistP_sbt baseValue_sbf stepFun_sbf spec P_outer)
                              (ap2 sub P_outer cP))))
      ...
      HP_to_sbf :
        Deriv (eqF (HPsbt baseValue_sbf stepFun_sbf spec cP P_outer) (ap2 sbf spec cP))
      HP_to_sbf = HP_sbf_eq_sbf_under_leq spec cP P_outer leq_cP_P

      val_cP_value :
        Deriv (eqF (ap1 (lookupAt get_body) input_pkg') (ap2 sbf spec cP))
      val_cP_value = ruleTrans val_cP_unfold HP_to_sbf

      -- Assemble pi (natCode tag_neg) (sbf spec cP).
      negbr_value :
        Deriv (eqF (ap1 neg_branch_sbf input_pkg')
                    (ap2 pi (natCode tag_neg) (ap2 sbf spec cP)))
      negbr_value =
      ...
      chain_to_negbr :
        Deriv (eqF (ap2 sbf spec input) (ap1 neg_branch_sbf input_pkg'))
      chain_to_negbr = ruleTrans chain_to_stepBody stepBody_to_neg
  in ruleTrans chain_to_negbr negbr_value

------------------------------------------------------------------------
-- sbf_at_imp :  the SbContract closure for imp.
--
...
sbf_at_imp :
  ...
  Deriv (eqF (ap2 sbf (ap2 pi (natCode k) S)
              ...
              (ap2 pi (natCode tag_imp)
                (ap2 pi
                  (ap2 sbf (ap2 pi (natCode k) S) cP)
                  (ap2 sbf (ap2 pi (natCode k) S) cQ))))
sbf_at_imp k S cP cQ =
  let spec : Term
      spec = ap2 pi (natCode k) S
      ...
      iter_full_cP = ruleTrans (congL (iter Snd) (ap2 sub (ap1 get_K input_pkg') (ap1 get_cP_imp input_pkg')) get_table_value)
                      (congR (iter Snd) (HistP_sbt baseValue_sbf stepFun_sbf spec P_outer) iter_arg_cP)

      val_cP_to_HP :
        Deriv (eqF (ap1 (lookupAt get_cP_imp) input_pkg')
                    (HPsbt baseValue_sbf stepFun_sbf spec cP P_outer))
      ...
      val_cP_value :
        Deriv (eqF (ap1 (lookupAt get_cP_imp) input_pkg') (ap2 sbf spec cP))
      ...
      iter_full_cQ = ruleTrans (congL (iter Snd) (ap2 sub (ap1 get_K input_pkg') (ap1 get_cQ_imp input_pkg')) get_table_value)
                      (congR (iter Snd) (HistP_sbt baseValue_sbf stepFun_sbf spec P_outer) iter_arg_cQ)

      val_cQ_to_HP :
        Deriv (eqF (ap1 (lookupAt get_cQ_imp) input_pkg')
                    (HPsbt baseValue_sbf stepFun_sbf spec cQ P_outer))
      ...
      chain_to_impbr :
        Deriv (eqF (ap2 sbf spec input) (ap1 imp_branch_sbf input_pkg'))
      chain_to_impbr = ruleTrans chain_to_stepBody stepBody_to_imp
  in ruleTrans chain_to_impbr impbr_value

------------------------------------------------------------------------
-- sbf_at_atomic :  the SbContract closure for atomic.
--
-- Direct sbt invocation in sub-positions via  C sbt get_spec get_ca_atom .
...
sbf_at_atomic k S ca cb =
  let spec : Term
      ...
      stepBody_to_atom :
        Deriv (eqF (ap1 stepBody_sbf input_pkg') (ap1 atomic_branch_sbf input_pkg'))
      stepBody_to_atom = stepBody_sbf_to_atomic input_pkg' isEq_value

      -- atomic_branch unfolding.
      atombr_unfold :
        Deriv (eqF (ap1 atomic_branch_sbf input_pkg')
                    (ap2 pi (ap1 (constN tag_eq) input_pkg')
                            (ap1 (C pi (C sbt get_spec get_ca_atom)
                                       (C sbt get_spec get_cb_atom)) input_pkg')))
      atombr_unfold = atomic_branch_sbf_unfold input_pkg'
      ...
      atombr_step1 = ruleTrans atombr_unfold
                       (congL pi (ap1 (C pi (C sbt get_spec get_ca_atom)
                                                (C sbt get_spec get_cb_atom)) input_pkg') constN_at)

      inner_C_unfold :
        Deriv (eqF (ap1 (C pi (C sbt get_spec get_ca_atom)
                              (C sbt get_spec get_cb_atom)) input_pkg')
                    (ap2 pi (ap1 (C sbt get_spec get_ca_atom) input_pkg')
                            (ap1 (C sbt get_spec get_cb_atom) input_pkg')))
      inner_C_unfold = atomic_pair_unfold input_pkg'

      -- C sbt get_spec get_ca_atom input_pkg' = sbt (get_spec input_pkg') (get_ca_atom input_pkg').
      sbt_ca_unfold :
        Deriv (eqF (ap1 (C sbt get_spec get_ca_atom) input_pkg')
                    (ap2 sbt (ap1 get_spec input_pkg') (ap1 get_ca_atom input_pkg')))
      sbt_ca_unfold = C_sbt_ca_unfold input_pkg'

      sbt_cb_unfold :
        Deriv (eqF (ap1 (C sbt get_spec get_cb_atom) input_pkg')
                    (ap2 sbt (ap1 get_spec input_pkg') (ap1 get_cb_atom input_pkg')))
      ...
      fstSnd_prev = fstSnd_cov_spec_eq baseValue_sbf stepFun_sbf spec P_outer
      ...
      -- sbt (get_spec ipkg) (get_ca_atom ipkg) = sbt spec ca.
      sbt_ca_value :
        Deriv (eqF (ap1 (C sbt get_spec get_ca_atom) input_pkg') (ap2 sbt spec ca))
      sbt_ca_value =
        ruleTrans sbt_ca_unfold
          (ruleTrans (congL sbt (ap1 get_ca_atom input_pkg') get_spec_value)
                      (congR sbt spec get_ca_value))

      sbt_cb_value :
        Deriv (eqF (ap1 (C sbt get_spec get_cb_atom) input_pkg') (ap2 sbt spec cb))
      sbt_cb_value =
        ruleTrans sbt_cb_unfold
          (ruleTrans (congL sbt (ap1 get_cb_atom input_pkg') get_spec_value)
                      (congR sbt spec get_cb_value))
      ...
      pair_val_value =
        ruleTrans inner_C_unfold
          (ruleTrans (congL pi (ap1 (C sbt get_spec get_cb_atom) input_pkg') sbt_ca_value)
                      (congR pi (ap2 sbt spec ca) sbt_cb_value))
      ...
      chain_to_atombr :
        Deriv (eqF (ap2 sbf spec input) (ap1 atomic_branch_sbf input_pkg'))
...
open import T4.SbT      using ( sbt_at_O )
open import T4.SbtAtVar using ( sbt_at_var_match ; sbt_at_var_nomatch )
open import T4.SbtAtAp  using ( sbt_at_ap1 ; sbt_at_ap2 )

sbContract : SbContract sbt sbf
sbContract = record
  { sbt_at_O           = sbt_at_O
  ; sbt_at_var_match   = sbt_at_var_match
  ; sbt_at_var_nomatch = sbt_at_var_nomatch
  ; sbt_at_ap1         = sbt_at_ap1
  ; sbt_at_ap2         = sbt_at_ap2
