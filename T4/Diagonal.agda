...
-- T4.Diagonal -- Goedel's diagonal lemma in the T4 Pair-encoding,
-- parametric on the substitution functor  sb : Fun2  (via
-- T4.SbContract) and our concrete  num : Fun1  (T4.Num,
-- contract from T4.NumContract).
--
...
--
-- ARCHITECTURE.  Mirrors BRA3.Thm.Thm11.Diagonal but uses T4.Code
-- (Pair-encoded codeFormula / codeTerm returning Term, not Nat) and
-- T4.NatBridge.codeFormula_eq to bridge between the Pair-tree
-- codeFormula H and its flattened natCode (codeFormulaNat H).

module T4.Diagonal where

open import T4.Base
open import T4.Tags
open import T4.Code
open import T4.NatBridge
open import T4.NumContract
open import T4.SbContract
open import T4.SbDerived
...
module Diagonal
  ...
  --
  -- Chain (Option B, in T4 Pair-encoding):
