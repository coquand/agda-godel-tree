...
-- T4.ChaitinG1DischargeKdefN -- the number-code re-pointing of
-- T4.ChaitinG1DischargeKdef : the mu-loop Discharge at the honest
-- p<N / runProgN recogniser ( hitKdefN / outKdefN , T4.KdefRecogN ; diagonal
-- predFlipDefN , T4.KdefDiagN ).  Verbatim mirror : the Discharge is generic in
-- the recogniser's 0/1 behaviour ( FirstHit / StepU2MuCorrect.Construct /
-- ChaitinG1Arith are all decoder-agnostic ), so only the  Kdef* -> Kdef*N  swap
-- is needed, with the  Lstar  threshold absorbed into the module parameter predN.

open import T4.Base

module T4.ChaitinG1DischargeKdefN (predN : Term) where

open import T4.ThmT            using ( thmT )
open import T4.KdefRecogN predN using
  ( hitKdefN ; hitKdefN_le_one ; hitKdefN_fires ; dNeg_from_hitKdefN ; outKdefN )
open import T4.KdefN  predN   using ( KcodeN )
open import T4.KdefDiagN predN using ( predFlipDefN )
...
------------------------------------------------------------------------
-- The Kdef-shape ( number-code ) Discharge module.

module DischargeKdefN
  (w        : Term)
  (x        : Term)
  ...
  ----------------------------------------------------------------------
  -- 5.  dNeg in num-raw KcodeN form, at the read-off subject  x' .

  x' : Term
  x' = ap1 outKdefN k_max

  dNeg_at_kmax : Deriv (eqF (ap1 thmT k_max) (ap1 KcodeN x'))
  dNeg_at_kmax = dNeg_from_hitKdefN outKdefN k_max isHit_recog
