...
-- T4.CgFunFun1 -- the genuine  Fun1 G  diagonal of Chaitin-Goedel-I.
--
-- =====================================================================
-- WHAT THIS FILE SHIPS.
-- =====================================================================
--
-- T4.CgFun.cgFun : Term -> Term  is a META term-transformer.  This
...
--  cw = closeW w  the only  w -dependence (T4.CgFun, lines 117-183).
...
--
--  So  E[-]  is a one-variable  Exp  (T4.AbsFun1) and
...
module T4.CgFunFun1 where

open import T4.Base
open import T4.AbsFun1
...
open import T4.StepU2CorrectAPI using ( Correct1 ; Correct2 ; fuelF ; fuelG )
open import T4.ChaitinG1CoreNumRaw using ( gLnameDef ; cSizeProofDef )
open import T4.CgFun      using ( cgFun )
open import T4.CgFalseImp using ( cgFalseImp ; cgFalseImpAtVar2 )
...
------------------------------------------------------------------------
-- Local constants -- VERBATIM copy of T4.CgFun's private block, so the
...
--  ap2 pi (natCode tag) (...)  cells (T4.ProgEnc.encApp).
...
-- skeletons -- T4.ConInj , T4.DefWit , T4.CgiClash , T4.EncodedProp).
...
------------------------------------------------------------------------
-- bodyExp -- a node-for-node transcription of  T4.CgFun.cgFun 's
...
--  var 2  ( T4.CgFalseImp.cgFalseImpAtVar2 ).   There  closeW (var 2) =
