...
-- T4.ChaitinG1DischargeKdefConj -- BLOCK 4, part 3: the  Discharge  step at
-- the two-slot conjunction K-shape.  A mechanical port of
-- T4.ChaitinG1DischargeKdef , re-pointed from the single-atom recogniser
-- ( hitKdef Lstar (outKdef Lstar) ) to the conjunction-shape recogniser
-- ( hitKdefConj enum N (outKdefConj enum N) , T4.KdefConjRecog) and the
-- conjunction diagonal predicate  predFlipConj enum N  (T4.KdefDiagConj).
--
...
-- whole body is verbatim; only the recogniser/predicate/code names change.
-- Output:  k_max , x' = outKdefConj N k_max , runs_mu (the mu-loop correctness),
-- and  dNeg_at_kmax :  thmT k_max = ap1 (KcodeConj N) x' .

module T4.ChaitinG1DischargeKdefConj where

open import T4.Base
open import T4.ThmT            using ( thmT )
open import T4.KdefConjRecog   using ( hitKdefConj ; hitKdefConj_le_one
                                       ; hitKdefConj_fires ; dNeg_from_hitKdefConj
                                       ; outKdefConj ; KcodeConj )
open import T4.KdefDiagConj    using ( predFlipConj )
...
------------------------------------------------------------------------
-- The conjunction-shape Discharge module.
...
module DischargeKdefConj
  ...
  x' : Term
  x' = ap1 (outKdefConj enum N) k_max

  dNeg_at_kmax : Deriv (eqF (ap1 thmT k_max) (ap1 (KcodeConj enum N) x'))
  dNeg_at_kmax = dNeg_from_hitKdefConj enum N (outKdefConj enum N) k_max isHit_recog
