...
-- T4.Counting -- the injectivity unit step for the arbitrary-injection
-- pigeonhole (Gate-1 route R3).  Builds on T4.PHP, which already has the
...
-- (T4/Skolem23.pdf):  [a < t+1] = [a < t] + [a = t] , summed over the
...
-- T4.RuleInst3) places the substituents at the leaves without ever
