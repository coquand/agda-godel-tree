...
$\overline n=\mathbf{s}^n\mathbf{O}$, and $\Thm{A}$ means ``$A$ is a
theorem of $T$''. We use the universal machine of the Chaitin
development.
...
only hypothesis used by the lower bounds is simple consistency,
\[
...
Assume $\Con_T$. There are at most $3^{\,L+1}$ numbers of complexity
$\le L$:
\[
...
Assume $\Con_T$. For every $L$ it is not the case that every $i\le
3^{\,L+1}$ has $\Kc(i)\le L$. Equivalently, some $i\le 3^{\,L+1}$ satisfies
$\Kc(i)>L$.
\end{theorem}

\begin{corollary}[$\Kc$ is unbounded]\label{cor:unbounded}
Assume $\Con_T$. For every $L$ it is not the case that $\Kc(i)\le L$ for
all $i$.
\end{corollary}
...
Assume $\Con_T$. No injective block of $3^{\,L+1}+1$ distinct numbers can
...
The library culminates in Chaitin's observation that $\Kc$ itself is not
computable. Say that a total combinator $K\!f\in\Fun_1$ \emph{computes}
$\Kc$ if it decides the complexity predicate:
\[
...
Assume $\Con_T$. No $K\!f\in\Fun_1$ computes $\Kc$.
\end{theorem}
...
uses $\Con_T$ as its sole hypothesis, through the single fact that one
program describes one value. The reference semantics $\ev$ is connected to
the object machine by a small reflection principle: an object equation
$f(\overline k)=\overline{\varphi(k)}$ together with machine soundness and
numeral injectivity (under $\Con_T$) pins the meta value
