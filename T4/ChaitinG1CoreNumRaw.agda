...
-- T4.ChaitinG1CoreNumRaw -- Chaitin-Goedel-I, num-raw form, CGI_core_num_raw.
--
-- Wires the shipped pieces:
--   * T4.ChaitinG1DischargeKdef.DischargeKdef  -- supplies  dNeg_at_kmax
--   * T4.ChaitinG1ChainKdef.ChainKdef          -- supplies  dEval_witness
--   * a PARAMETER  dLenStarDef                   -- the size atom for gLcodeDef Lstar
--   * T4.CgiClash.cgiClash                     -- the integrated single-conjunct clash
-- to produce the documented num-raw Chaitin-Goedel-I conclusion.
--
-- ============================================================
-- Answer to the "informal-argument" question (from
-- T4/NEXT-SESSION-CGI-NUM-RAW.md).
-- ============================================================
--
...
--       and  decode (num t) = t  holds for ANY Term t (T4.Decode.decode_num_id),
...
--       *** This is NOT realized. ***  T4.CgiClash.cgiClash performs the
...
--     of T4/dLenStarDef.agda when shipped),
...
module T4.ChaitinG1CoreNumRaw where
...
open import T4.dLenStarDef     using ( dLenStarDef )

import T4.ChaitinG1DischargeKdef
import T4.ChaitinG1ChainKdef

open import BRA3.RuleInst2       using ( simSubstT )
open import T4.CloseW          using ( closeW ; cl_w_sub0 ; cl_w_sub1 ; cl_w_sim )

------------------------------------------------------------------------
-- Local Sigma  (BRA3/T4 do not export a global Sigma; see Thm12.All for
-- the same pattern).
...
-- T4.CgiTheorem.dSize but at  gLnameDef := enc (gLcodeDef Lstar) .
--  dLenStarDef  (T4.dLenStarDef) discharges the residual size fact at
-- the Kdef-shape  Lstar  via the parallel  Size  module T4.GLCodePDef.
...
-- DischargeKdef wants on  w  are then trivial structurally (T4.CloseW
