...
-- T4.GLCodePDef -- the Kdef-shape parallel of T4.GLCodeP.Size .
--
-- T4.GLCodeP.Size builds the size accounting + dLen for the Kgt-shape
...
--     is SYNTACTICALLY the same as  pKgt L  (T4.KFormula);
...
open import T4.KFormula using ( szLeqApp ; pKgt )
open import T4.Kdef using ( runProg ; Kcode )
open import T4.KdefDiag using ( gLcodeDef ; gCodeOfDef ; predFlipDef )
open import T4.KdefRecog using ( hitKdef ; outKdef ; projKdef )
...
open import T4.GLCodeNodes using
...
module SizeDef (tc : Fun1) where
  ...
  outKdefP L = compose1U decode (compose1U (projKdef L) tc)
    where open import T4.Decode using ( decode )
