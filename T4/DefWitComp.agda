...
-- T4.DefWitComp -- the COMPUTATION-naming atom (the CORRECTED Chaitin
-- definability layer), the search-side / DefWit-compatible companion of the
-- kernel T4.CompressComp.  See T4/CHAITIN-KERNEL-THM13.md and
-- T4/NEXT-SESSION-CHAITIN-G1.md (Step 2a).
--
...
-- numeral code  num y  (one application of the shipped coder  T4.Num.num ).
...
module T4.DefWitComp where

open import T4.Base
open import T4.Tags using ( tag_eq ; tag_ap1 )
open import T4.Code using ( codeTerm ; codeFun1 )
open import T4.ThmT using ( thmT )
open import T4.LenR using ( lenR )
open import T4.Parse using ( parse )
open import T4.ParseRoundtrip using ( linTop ; parse_roundtrip_term )
open import T4.Num using ( num )
open import T4.IsNat using ( isNat ; num_eq_code )
open import T4.DefWit using ( cEqTm ; fAnd ; vFlin ; vPi )
...
--     (the last equality DEFINITIONAL, T4.Code.codeTerm on ap1);
