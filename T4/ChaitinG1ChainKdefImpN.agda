...
-- T4.ChaitinG1ChainKdefImpN -- the number-code re-pointing of
-- T4.ChaitinG1ChainKdefImp : the Carneiro-lifted (imp Rf) run chain.  Verbatim
-- mirror, Kdef* -> Kdef*N , L -> predN .

open import T4.Base

module T4.ChaitinG1ChainKdefImpN (predN : Term) where

open import T4.ThmT            using ( thmT )
open import T4.KdefN  predN   using ( KcodeN )
open import T4.KdefRecogN predN using ( outKdefN )
open import T4.KdefDiagN predN using ( gLcodeDefN ; gCodeOfDefN ; dRT_gLDefN ; predFlipDefN )
...
module ChainKdefImpN
  (Rf       : Formula)
  (w        : Term)
  (x        : Term)
  (hyp_imp  : Deriv (imp Rf (eqF (ap1 thmT w) (ap1 KcodeN x))))
  ...
  open T4.ChaitinG1DischargeKdefImpN.DischargeKdefImpN
         predN Rf w x hyp_imp sub0_w sub1_w sim_w
  ...
  module Chain
    ...
    c0  = cfgEV gLcodeDefN O konEmpty
    c1  = cfgEV h1c O K1
    c2  = cfgRT k_max K1
    ...
    seg1_evC = stepU_at_evC_code g_outer_code h1c (mcode1 u) O konEmpty
    ...
    seg7_bridge = congR pi (natCode tagRT) (congL pi konEmpty liftBridge)
    ...
    initF_bridge : Deriv (eqF (ap1 initF gLcodeDefN) c0)
    initF_bridge = initF_eq gLcodeDefN

    imp_evalU_at_gL :
      Deriv (imp Rf (eqF (ap2 evalU gLcodeDefN fuelN)
                          (ap1 s (ap1 outL k_max))))
    imp_evalU_at_gL =
      let unfold : Deriv (eqF (ap2 evalU gLcodeDefN fuelN)
                              (ap1 readout (ap2 (iter step)
                                              (ap1 initF gLcodeDefN) fuelN)))
          unfold = evalU_unfold gLcodeDefN fuelN
          iterRw : Deriv (eqF (ap2 (iter step) (ap1 initF gLcodeDefN) fuelN)
          ...
          closed_pre :
            Deriv (eqF (ap2 evalU gLcodeDefN fuelN)
                       (ap1 readout (ap2 (iter step) c0 fuelN)))
          closed_pre = ruleTrans unfold readRw
      in impEqTrans (ap2 evalU gLcodeDefN fuelN)
           (ap1 readout (ap2 (iter step) c0 fuelN))
           (ap1 s (ap1 outL k_max))
           (impLift {Rf} closed_pre) imp_readout_chain

    imp_evalU_at_parse :
      Deriv (imp Rf (eqF (ap2 evalU (ap1 parse (enc gLcodeDefN)) fuelN)
                          (ap1 s (ap1 outL k_max))))
    imp_evalU_at_parse =
      let parseEq : Deriv (eqF (ap1 parse (enc gLcodeDefN)) gLcodeDefN)
          parseEq = dRT_gLDefN
          evalRw : Deriv (eqF (ap2 evalU (ap1 parse (enc gLcodeDefN)) fuelN)
                              (ap2 evalU gLcodeDefN fuelN))
          evalRw = congL evalU fuelN parseEq
      in impEqTrans (ap2 evalU (ap1 parse (enc gLcodeDefN)) fuelN)
           (ap2 evalU gLcodeDefN fuelN) (ap1 s (ap1 outL k_max))
    ...
    imp_dEval_witness :
      Deriv (imp Rf (eqF (ap2 evalU (ap1 parse (enc gLcodeDefN)) nTerm)
                          (ap1 s (ap1 outKdefN k_max))))
