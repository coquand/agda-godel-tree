...
-- T4.CRGlueU -- shared conj3 assembly / decomposition for the bundled unsized
-- CR dispatch.  conj3 p = O  is built from the three facts
--   (V) wfRed (triF p) = O
--   (S) srcF (triF p) = tgtF p
--   (T) tgtF (triF p) = devF (srcF p)
-- and conversely decomposed at a child.
--
-- No holes, no postulates, no termination warnings; --safe --without-K
-- --exact-split.

module T4.CRGlueU where

open import T4.Base

open import T4.QCheckU using ( conj3 ; srcEqF ; tgtEqF )
...
------------------------------------------------------------------------
-- SECTION 1.  Unfolds of the cell Fun1s.
...
conj3_unfold : (p : Term) ->
  Deriv (eqF (ap1 conj3 p)
             (ap2 sigma (ap1 wfRed (ap1 triF p))
                        (ap2 sigma (eqDecO (ap1 srcF (ap1 triF p)) (ap1 tgtF p))
                                   (eqDecO (ap1 tgtF (ap1 triF p)) (ap1 devF (ap1 srcF p))))))
conj3_unfold p =
  ruleTrans (ax_C sigma (compose1U wfRed triF) (C sigma srcEqF tgtEqF) p)
    (ruleTrans (congL sigma (ap1 (C sigma srcEqF tgtEqF) p) (compose1U_eq wfRed triF p))
      (congR sigma (ap1 wfRed (ap1 triF p))
...
------------------------------------------------------------------------
-- SECTION 2.  Build conj3 p = O from the three facts.

buildConj3 : (p : Term) ->
  Deriv (eqF (ap1 wfRed (ap1 triF p)) O) ->
...
buildConj3 p hV hS hT =
  let sO : Deriv (eqF (eqDecO (ap1 srcF (ap1 triF p)) (ap1 tgtF p)) O)
      ...
      outer : Deriv (eqF (ap2 sigma (ap1 wfRed (ap1 triF p))
                            (ap2 sigma (eqDecO (ap1 srcF (ap1 triF p)) (ap1 tgtF p))
                                       (eqDecO (ap1 tgtF (ap1 triF p)) (ap1 devF (ap1 srcF p))))) O)
...
------------------------------------------------------------------------
-- SECTION 3.  Decompose conj3 c = O into the three child facts.

childV : (c : Term) -> Deriv (eqF (ap1 conj3 c) O) ->
  Deriv (eqF (ap1 wfRed (ap1 triF c)) O)
childV c h =
