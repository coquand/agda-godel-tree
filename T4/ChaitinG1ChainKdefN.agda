...
-- T4.ChaitinG1ChainKdefN -- the number-code re-pointing of T4.ChaitinG1ChainKdef.
-- Establishes the diagonal's RUN  evalU (parse (enc gLcodeDefN)) nTerm =
-- s (outKdefN k_max) .  Verbatim mirror : the universal-machine run chain is
-- decoder-agnostic ( the program-as-number / runProgN bridge lives in the clash,
-- T4.ParseN.runProgN_at_diag ); only the  Kdef* -> Kdef*N  swap is needed,
-- Lstar absorbed into predN .

open import T4.Base

module T4.ChaitinG1ChainKdefN (predN : Term) where

open import T4.ThmT            using ( thmT )
open import T4.KdefN  predN   using ( KcodeN )
open import T4.KdefRecogN predN using ( outKdefN )
open import T4.KdefDiagN predN using ( gLcodeDefN ; gCodeOfDefN ; dRT_gLDefN )
...
module ChainKdefN
  (w        : Term)
  (x        : Term)
  (h        : Deriv (eqF (ap1 thmT w) (ap1 KcodeN x)))
  ...
  open T4.ChaitinG1DischargeKdefN.DischargeKdefN predN w x h sub0_w sub1_w sim_w
  ...
  h1c : Term
  h1c = mcodeMu (mcode1 (T4.ChaitinG1DischargeKdefN.DischargeKdefN.gFun
                          predN w x h sub0_w sub1_w sim_w))
  ...
  evalU_at_gL :
    Deriv (eqF (ap2 evalU gLcodeDefN fuelN) (ap1 s (ap1 outL k_max)))
  evalU_at_gL =
    let unfold = evalU_unfold gLcodeDefN fuelN
        iterRw = congL (iter step) fuelN initF_bridge
        readRw = cong1 readout iterRw
    in ruleTrans unfold (ruleTrans readRw readout_chain)

  evalU_at_parse :
    Deriv (eqF (ap2 evalU (ap1 parse (enc gLcodeDefN)) fuelN)
                (ap1 s (ap1 outL k_max)))
  evalU_at_parse =
    let parseEq = dRT_gLDefN
  ...
  dEval_witness :
    Deriv (eqF (ap2 evalU (ap1 parse (enc gLcodeDefN)) nTerm)
