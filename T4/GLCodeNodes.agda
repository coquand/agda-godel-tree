...
-- T4.GLCodeNodes -- Phase R5, step (3): the thmT-FREE structural pieces of the
...
-- path (Cmcode), the size additivity and dLen are in T4.GLCodeP, PARAMETRIC
-- over an abstract checker  tc : Fun1  (so  nodes  never normalises  thmT ).

module T4.GLCodeNodes where

open import T4.Base
open import T4.Tags using ( tag_C ; tag_eq ; tag_ap1 )
...
open import T4.KFormula using ( pKgt )
