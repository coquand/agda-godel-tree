...
-- T4.ChaitinG1ChainBC -- the Chain step at the framework's  KdefBigConj
-- output-slot shape.  A mechanical port of  T4.ChaitinG1ChainKdefConj ,
-- re-pointed from  outKdefConj / gLcodeConj  to  outBC enum fuel M / gLcodeBC
-- enum M  ( T4.KdefDiagBC ) and opening  DischargeBC  instead of
-- DischargeKdefConj .
--
-- The 8 step segments are GENERIC in the diagonal program and the output
-- projector, so the whole body is verbatim.  Output:  nTerm (the fuel)
-- and  dEval_witness : evalU(parse(enc gLcodeBC), nTerm) = s (outBC M k_max) .

module T4.ChaitinG1ChainBC where

open import T4.Base
open import T4.ThmT            using ( thmT )
open import T4.KdefBigConjRecog using ( KcodeBC ; outBC )
open import T4.KdefDiagBC      using ( gLcodeBC ; gCodeOfBC ; dRT_gLBC )
...
------------------------------------------------------------------------
-- The output-slot-shape Chain module -- same setup as DischargeBC.
...
module ChainBC
  ...
  (w        : Term)
  (x        : Term)
  (h        : Deriv (eqF (ap1 thmT w) (ap1 (KcodeBC enum fuel M) x)))
  ...
  open T4.ChaitinG1DischargeBC.DischargeBC enum fuel M w x h sub0_w sub1_w sim_w
  ...
  outL : Fun1
  outL = outBC enum fuel M

  g_outer_code : Term
  g_outer_code = gCodeOfBC enum fuel M      -- = mcode2 (Lift1 outL)

  h1c : Term
  h1c = mcodeMu (mcode1 (T4.ChaitinG1DischargeBC.DischargeBC.gFun
                          enum fuel M w x h sub0_w sub1_w sim_w))
  ...
  c0  = cfgEV (gLcodeBC enum fuel M) O konEmpty
  c1  = cfgEV h1c O K1
  c2  = cfgRT k_max K1
  ...
  initF_bridge : Deriv (eqF (ap1 initF (gLcodeBC enum fuel M)) c0)
  initF_bridge = initF_eq (gLcodeBC enum fuel M)

  evalU_at_gL :
    Deriv (eqF (ap2 evalU (gLcodeBC enum fuel M) fuelN) (ap1 s (ap1 outL k_max)))
  evalU_at_gL =
    let unfold = evalU_unfold (gLcodeBC enum fuel M) fuelN
        iterRw = congL (iter step) fuelN initF_bridge
        readRw = cong1 readout iterRw
    in ruleTrans unfold (ruleTrans readRw readout_chain)

  evalU_at_parse :
    Deriv (eqF (ap2 evalU (ap1 parse (enc (gLcodeBC enum fuel M))) fuelN)
                (ap1 s (ap1 outL k_max)))
  evalU_at_parse =
    let parseEq = dRT_gLBC enum fuel M
  ...
  dEval_witness :
    Deriv (eqF (ap2 evalU (ap1 parse (enc (gLcodeBC enum fuel M))) nTerm)
                (ap1 s (ap1 (outBC enum fuel M) k_max)))
