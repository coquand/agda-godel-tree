...
-- T4.Thm12.ImpEncodedEqChain -- Hilbert-lifted (Carneiro) versions of
--  encoded_eqSym  and  encoded_eqTrans  from T4.Thm12.EncodedEqChain.
--
-- These are PUBLIC analogs of the  private  helpers used inside
--  T4.Thm12.PartRStep (for the thm12_R step proof).  They are pulled
-- out here so downstream clients (e.g. T4.Thm.Thm14Step3) can build
-- imp-encoded equality chains without depending on the heavy
-- PartRStep module.
--
-- Pattern : for each unconditional encoded_eq* in
--  T4.Thm12.EncodedEqChain , add an  imp P  wrapper around the two
-- inputs / one output.  The closed pieces (axes, refl) are lifted via
-- impLift ; the two mp applications use  imp_encoded_mp  from
--  T4.Thm12.EncodedMp .

module T4.Thm12.ImpEncodedEqChain where

open import T4.Base
open import T4.Tags
open import T4.Code
open import T4.ThmT                using ( thmT )
open import T4.SbStep              using ( InertU )
...
--
-- where  Df_eqSym_term  matches the T4.Thm12.EncodedEqChain encoding :
