...
-- T4.ChaitinG1DischargeBC -- the  Discharge  step at the framework's
-- KdefBigConj  output-slot shape.  A mechanical port of
-- T4.ChaitinG1DischargeKdefConj , re-pointed from the block-4 recogniser
-- ( hitKdefConj / outKdefConj / KcodeConj / predFlipConj ) to the
-- output-slot recogniser  ( hitBC / outBC / KcodeBC , T4.KdefBigConjRecog )
-- and the diagonal predicate  predFlipBC  ( T4.KdefDiagBC ).
--
...
-- Output:  k_max , x' = outBC M k_max , runs_mu (the mu-loop correctness),
-- and  dNeg_at_kmax :  thmT k_max = ap1 (KcodeBC M) x' .

module T4.ChaitinG1DischargeBC where

open import T4.Base
open import T4.ThmT            using ( thmT )
open import T4.KdefBigConjRecog using ( hitBC ; hitBC_le_one
                                       ; hitBC_fires ; dNeg_from_hitBC
                                       ; outBC ; KcodeBC )
open import T4.KdefDiagBC      using ( predFlipBC )
...
------------------------------------------------------------------------
-- The output-slot-shape Discharge module.
...
module DischargeBC
  ...
  ----------------------------------------------------------------------
  -- 5.  dNeg in num-raw KcodeBC form, at the read-off subject  x' .

  x' : Term
  x' = ap1 (outBC enum fuel M) k_max

  dNeg_at_kmax : Deriv (eqF (ap1 thmT k_max) (ap1 (KcodeBC enum fuel M) x'))
  dNeg_at_kmax = dNeg_from_hitBC enum fuel M (outBC enum fuel M) k_max isHit_recog
