...
-- T4.FoldRec -- a modular structural-fold recursor for T4, built by
-- FACTORING T4's own course-of-values machinery (T4.CoVSpec +
-- T4.Stability + T4.CoVSpecFst/Spec).  Phase A0 of
-- KRIVINE-BERRY-PLAN.md.
--
-- Goal: a clean fold-with-closures API on the T4 bare-Pair + explicit-
...
-- in T4.Stability / CoVSpecFst / CoVSpecSpec; FoldRec assembles those
...
-- induction lives once in  T4.Stability.stabilityP_sbt_v012  (already
-- parametric in base/step), which both bridges reuse.

module T4.FoldRec where
...
--
-- For a node  pi (s A) b  (every T4 code node is  pi (natCode tag) rest
...
--
-- Parametric analog of  T4.SbtAtAp.HP_sbt_eq_sbt_under_leq , stopping at
...
-- T4.SbT's get_* / lookupAt (reproduced here, public, so the Berry
