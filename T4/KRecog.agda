...
-- T4.KRecog -- Phase E2 (CHAITIN-G1-STANDARD-DIRECTION.md SS5): the search
-- RECOGNISER  hitK  (= hit_L) for the standard-route Kolmogorov formula, and
-- its bridge to  dNeg .  The standard-route analog of T4.ChaitinG1Neg's
-- hitNeg / dNeg_from_hitNeg, re-pointed from the provability atom to the
-- evalU K-formula  Kgt L x  (T4.KFormula).
--
...
-- hole projector  out : Fun1  (built concretely in E2.4 / T4.KOut).

module T4.KRecog where

open import T4.Base
open import T4.ThmT       using ( thmT )
open import T4.KFormula   using ( negKgtCodeOf )
