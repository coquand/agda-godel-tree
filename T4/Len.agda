...
-- T4.Len -- meta-level length (node-count) of Terms.
--
...
-- coding functions (T4.Code) expand every functor into a Pair-tree of
-- natCode tags, so  lenMeta (codeFormula F)  fully accounts for F's
-- structure including its functors.
--
-- KEY FACT (the motivation for compressed numerals, T4.Bin):
--
...
-- (binary) numerals (T4.Bin) give an  O(log m)  alternative.
